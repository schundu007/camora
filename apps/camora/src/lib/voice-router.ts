...
export function dispatchTranscript({
  ...
  if (activeTab === 'coding' || activeTab === 'design') {
    ...
    // Always hand the transcript to the sidebar; `autoSend` decides whether it
    // is asked or merely offered. Dropping it outright — which is what happened
    // before a solution was loaded, or when the phrasing missed isQuestion() —
    // left speech vanishing with no trace, so live dictation read as broken.
    // This module's contract was always "suppress auto-submission, never the
    // transcript"; the coding path just never kept it.
