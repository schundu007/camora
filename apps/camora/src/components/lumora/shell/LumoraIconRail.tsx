...
export type LumoraTab = 'session' | 'coding' | 'design' | 'cofix' | 'playground' | 'behavioral' | 'prepkit' | 'docs' | 'calendar' | 'sessions' | 'assistants' | 'profile' | 'credits' | 'ask';
...
const MAIN_ITEMS = [
  ...
  { id: 'playground', label: 'Playground', path: '/lumora/playground', icon: <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5"><path fillRule="evenodd" d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm3.293 1.293a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 01-1.414-1.414L7.586 10 5.293 7.707a1 1 0 010-1.414zM11 12a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd" /></svg> },
  { id: 'ask', label: 'Ask Sona', path: '/lumora/ask', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg> },
];
...
export const LumoraIconRail = ({ activeTab, sessionsOpen: _sessionsOpen, onToggleSessions: _onToggleSessions }: LumoraIconRailProps) => {
  ...
  const isActive = (id: string) => {
    ...
    if (id === 'ask') return activeTab === 'ask';
    return false;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {expanded && <span className="text-sm font-bold whitespace-nowrap text-[var(--text-hero)]" style={{ fontFamily: "var(--font-sans)" }}>Camora</span>}
      </Link>
