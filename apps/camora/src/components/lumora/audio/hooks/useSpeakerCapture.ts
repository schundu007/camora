...
/**
 * Universal speaker-audio capture hook. Replaces the
 ...
 *
 *   • mic-only — the user's mic is the only stream; we don't
 *     run a second capture. The hook never starts a stream in this
 *     mode — the user-mic capture (useAudioCapture) is the
 *     speaker source via server-side diarization.
 *
...
interface CaptureOptions {
  ...
  /** Used when method === 'room-mic' — the user's chosen mic.
   *  null = system default. */
...
async function acquireStream(
  ...
  if (method === 'electron-loopback' || method === 'tab-share') {
    if (!navigator.mediaDevices?.getDisplayMedia) {
      throw new Error('This browser cannot capture speaker audio. Use Chrome, Edge, or the Camora desktop app.');
    }
  ...
  if (method === 'room-mic') {
    // Room-mic captures the user's laptop mic and treats whatever
    // it picks up as the speaker source. Works for ANY external
    ...
    // is what splits the user's own voice from the room ambient.
    //
    // Important: keep echoCancellation/noiseSuppression OFF — both
    // would scrub speaker audio bleeding back through the mic.
    ...
    // Mirrors the user mic hook (useAudioCapture).
...
export function useSpeakerCapture(options: CaptureOptions) {
  const {
    method,
  ...
  const startCapture = useCallback(async () => {
    try {
      ...
      // Mic-only doesn't acquire a separate stream — the user-mic
