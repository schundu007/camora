...
export const linuxTopicCategoryMap = {
  'firmware-signing-secure-boot': 'security',
  'kernel-driver-builds': 'fundamentals',
  // Fundamentals
...
export const linuxTopics = [
  {
    id: "firmware-signing-secure-boot",
    title: "Firmware Signing & Secure Boot",
    icon: "shield",
    color: "#ef4444",
    questions: 6,
    description: "How firmware code signing, the UEFI Secure Boot chain of trust, and kernel module signing fit into a CI pipeline that never exposes the private signing key.",
    introduction: "## Firmware Signing and Secure Boot\n\nSecure boot exists to answer one question at every stage of startup: is the next piece of code allowed to run here? The answer comes from asymmetric cryptography. A private signing key produces a signature over an artifact; the matching public verification key, baked into read-only silicon or firmware, checks that signature before execution. The private key signs, the public key verifies, and the two are never interchangeable.\n\n## Why This Matters for a GPU CI Role\n\nAn AMD GPU stack ships firmware for the GPU, its microcontrollers, and sometimes a signed kernel driver. Every one of those artifacts must be signed before it is promoted to a release channel, and the signing has to happen inside CI without ever writing the private key to a build runner. Runners are ephemeral, shared, and frequently compromised, so the key lives in an `HSM` or a cloud `KMS` and the pipeline requests signatures over an authenticated channel rather than holding key material.\n\n## The Chain of Trust\n\nTrust is anchored in something immutable, typically a public key hash burned into one-time-programmable (`OTP`) fuses or mask ROM. That root verifies the first-stage bootloader, which verifies the next stage, and so on up to the kernel and its modules. Each link only runs code it can cryptographically verify, so a single unbroken chain from silicon to userspace is what secure boot actually guarantees.",
    whenToUse: [
      "You must add a signing step to a CI pipeline for firmware, a bootloader, or a kernel driver and cannot allow the private key to touch a build runner.",
      "You are debugging why a signed kernel module refuses to load, or a bootloader is rejected, on a host with UEFI Secure Boot enabled.",
      "A signing key or certificate is suspected compromised and you need to rotate it and revoke every artifact it signed via `dbx` or a CRL.",
      "You are adding a release gate that must independently verify an artifact's signature before it is promoted to a production channel.",
      "You are designing an offline or isolated signing service that a pipeline calls over the network to obtain signatures without ever exposing key material.",
    ],
    keyConcepts: [
      { term: "Asymmetric code signing", definition: "A signing scheme where a secret private key produces a signature over a digest of the artifact and a freely distributable public key verifies it. Only the private key can sign; possession of the public key never lets an attacker forge signatures. Tools like `openssl`, `sbsign`, and `pesign` implement this over `RSA` or `ECDSA`." },
      { term: "Detached signature", definition: "A signature stored as a separate file rather than embedded in the artifact, produced with commands like `openssl dgst -sign` or `gpg --detach-sign`. Detached signing lets a signing service return only the small signature while the large firmware blob stays on the CI runner, which is central to keeping the private key remote." },
      { term: "Certificate chain and root of trust", definition: "A hierarchy where a leaf signing certificate is signed by an intermediate CA, which chains up to a root whose public key (or its hash) is stored immutably in `OTP` fuses or mask ROM. Verification walks the chain from the artifact's signature up to that immutable anchor; if any link fails, execution is refused." },
      { term: "Anti-rollback version counter", definition: "A monotonic counter, held in fuses or a `TPM` NV index, that records the minimum acceptable firmware version. Boot firmware refuses any image whose embedded version is below the counter, so an attacker cannot downgrade to an older, validly signed but vulnerable image. The counter only ever increases and is bumped after a security fix." },
      { term: "UEFI Secure Boot key hierarchy", definition: "Four key databases in firmware: `PK` (Platform Key) owns the platform and authorizes changes to `KEK`; `KEK` (Key Exchange Key) authorizes updates to `db` and `dbx`; `db` holds keys and hashes allowed to boot; `dbx` is the forbidden list of revoked keys and hashes. A bootloader runs only if it matches `db` and does not match `dbx`." },
      { term: "Kernel module signing", definition: "With `CONFIG_MODULE_SIG` and `CONFIG_MODULE_SIG_FORCE`, the kernel appends a signature to each `.ko` using `scripts/sign-file` and verifies it against keys in the kernel keyring at load time. On a Secure Boot host with lockdown, an unsigned or wrongly signed module is rejected, so out-of-tree drivers must be signed by an enrolled key." },
      { term: "Measured boot versus secure boot", definition: "Secure boot enforces a policy: it halts if code fails verification. Measured boot enforces nothing at boot time; each stage hashes the next and extends the digest into a `TPM` PCR, producing an attestable record. Secure boot answers is this allowed to run, measured boot answers what actually ran, and the two are complementary." },
    ],
    pitfalls: [
      "Writing the private signing key to a CI runner, an environment variable, or the repo. Runners are ephemeral and shared; the key must live in an `HSM` or `KMS` and the pipeline should call a signing API, never hold key material.",
      "Enrolling a custom key into `db` but forgetting to update `dbx` when a key is retired. An old key stays trusted until it is explicitly revoked in `dbx`, so a leaked key remains valid for booting.",
      "Signing a kernel module but not enrolling the signing certificate via `mokutil --import` (or into `db`), so the module fails to load on a Secure Boot host even though the signature itself is valid.",
      "Omitting an anti-rollback counter bump after a security fix. Without raising the monotonic version, the old vulnerable image is still validly signed and can be flashed back by an attacker.",
      "Treating measured boot and secure boot as interchangeable. `TPM` PCR measurements record what ran but never block it; relying on PCRs alone leaves unsigned code free to execute.",
    ],
    keyQuestions: [
      { question: "How do you integrate firmware signing into a CI pipeline without ever exposing the private key on a runner?", answer: "## Principle\n\nThe private key never leaves the `HSM` or cloud `KMS`. The runner computes a digest of the firmware, sends only that digest (or the artifact reference) to the signing backend over an authenticated channel, and receives a detached signature back. The runner holds ciphertext and signatures, never key material.\n\n## Signing With AWS KMS\n\nCompute the digest locally, then ask `KMS` to sign it. The key material stays inside the `KMS` HSM boundary.\n\n```bash\nsha256sum firmware.bin | awk '{print $1}' > fw.sha256\nopenssl dgst -sha256 -binary firmware.bin > fw.digest\naws kms sign \\\n  --key-id alias/firmware-signing \\\n  --message fileb://fw.digest \\\n  --message-type DIGEST \\\n  --signing-algorithm RSASSA_PKCS1_V1_5_SHA_256 \\\n  --output text --query Signature | base64 -d > firmware.sig\n```\n\n## Signing Through a PKCS#11 HSM\n\nWhen the signer is a physical or network HSM, expose it through a `PKCS#11` provider so `openssl` never sees a raw key.\n\n```bash\nopenssl dgst -sha256 -sign \"pkcs11:token=fw-hsm;object=fw-signing-key\" \\\n  -keyform engine -engine pkcs11 \\\n  -out firmware.sig firmware.bin\n```\n\n## CI Job Shape\n\nThe pipeline authenticates to the signer with a short-lived OIDC token from the CI provider, not a stored secret.\n\n```bash\naws sts assume-role-with-web-identity \\\n  --role-arn \"$SIGNING_ROLE_ARN\" \\\n  --web-identity-token \"$CI_OIDC_TOKEN\" \\\n  --role-session-name ci-fw-sign > creds.json\n```\n\nThe key never appears in the job; only the ability to request one signature does." },
      { question: "Explain the secure-boot chain of trust and how you would validate it end to end.", answer: "## The Chain\n\nTrust starts at an immutable anchor: a public key hash burned into `OTP` fuses or mask ROM. That root verifies the first-stage bootloader's signature; the bootloader verifies the next stage; that stage verifies the kernel; the kernel verifies its modules. Every link runs only code it can cryptographically check against a key that ultimately chains to the fused root.\n\n## Validating The UEFI Portion\n\nConfirm Secure Boot is active and inspect the key databases.\n\n```bash\nmokutil --sb-state\nefi-readvar -v PK\nefi-readvar -v KEK\nefi-readvar -v db\nefi-readvar -v dbx\n```\n\n## Verifying A Signed Bootloader\n\nCheck that the bootloader carries a signature that chains to a key in `db`.\n\n```bash\nsbverify --list /boot/efi/EFI/BOOT/grubx64.efi\nsbverify --cert /etc/secureboot/db.crt /boot/efi/EFI/BOOT/grubx64.efi\npesign -S -i /boot/efi/EFI/BOOT/grubx64.efi\n```\n\n## Tracing To The Root\n\nWalk the certificate chain from the signing leaf up to the anchor and confirm the leaf is not present in `dbx`.\n\n```bash\nopenssl verify -CAfile root-ca.crt -untrusted intermediate.crt leaf-signing.crt\n```\n\nA valid chain means every stage from fused root to running kernel refused to execute anything it could not verify." },
      { question: "How do you sign a kernel module so it loads on a host with UEFI Secure Boot enabled?", answer: "## Why It Is Required\n\nWith Secure Boot on, the kernel enters lockdown and `CONFIG_MODULE_SIG_FORCE` rejects any module whose signature does not verify against a key in the kernel keyring. An out-of-tree AMD GPU driver must therefore be signed by a key the firmware and kernel already trust.\n\n## Generate A Machine Owner Key\n\n```bash\nopenssl req -new -x509 -newkey rsa:2048 -nodes \\\n  -keyout MOK.priv -outform DER -out MOK.der \\\n  -days 3650 -subj \"/CN=GPU Module Signing/\"\n```\n\n## Enroll It With mokutil\n\nEnrollment stages the key; the firmware `MokManager` UI confirms it on the next reboot.\n\n```bash\nmokutil --import MOK.der\nmokutil --list-enrolled\n```\n\n## Sign The Module\n\nUse the kernel's `scripts/sign-file` against the built `.ko`.\n\n```bash\n/usr/src/linux-headers-$(uname -r)/scripts/sign-file \\\n  sha256 MOK.priv MOK.der amdgpu.ko\nmodinfo amdgpu.ko | grep -E 'sig_id|signer|sig_key'\n```\n\n## Confirm It Loads\n\n```bash\ninsmod amdgpu.ko\ndmesg | grep -i amdgpu\n```\n\nIn CI, `sign-file` calls the remote signer instead of a local `MOK.priv`, and the enrolled certificate is shipped for hosts to import once." },
      { question: "A signing key is suspected compromised. How do you rotate and revoke it?", answer: "## Immediate Containment\n\nDisable the compromised key in the `HSM` or `KMS` so no new signatures can be produced, then generate a replacement key that never touched a runner.\n\n```bash\naws kms disable-key --key-id alias/firmware-signing-old\naws kms create-key --description firmware-signing-2026 --key-usage SIGN_VERIFY\n```\n\n## Revoke In UEFI\n\nAdd the compromised certificate or the hash of every artifact it signed to `dbx`. The update must itself be signed by `KEK`.\n\n```bash\nsbvarsign --key KEK.key --cert KEK.crt \\\n  --output dbx-update.auth dbx compromised.crt\nefi-updatevar -f dbx-update.auth dbx\nmokutil --revoke-import\n```\n\n## Revoke In A PKI Chain\n\nWhere the key is backed by a CA, publish a CRL or push an OCSP revocation so verifiers reject the leaf.\n\n```bash\nopenssl ca -revoke compromised.crt -config ca.cnf\nopenssl ca -gencrl -out current.crl -config ca.cnf\n```\n\n## Re-sign And Bump Rollback\n\nRe-sign current artifacts with the new key and raise the anti-rollback counter so downgrades to old-key images are refused.\n\n```bash\nsbsign --key new-signing.key --cert new-signing.crt \\\n  --output grubx64.signed.efi grubx64.efi\n```\n\nRotation is not complete until `dbx` (or the CRL) makes every old-key artifact unbootable." },
      { question: "How do you make signature verification a hard CI gate before promotion?", answer: "## Goal\n\nNo artifact reaches a release channel unless an independent CI stage re-verifies its signature against the trusted public key. The gate uses only the public key, so it can run on any runner.\n\n## Verify A Detached Firmware Signature\n\n```bash\nopenssl dgst -sha256 -verify fw-public.pem \\\n  -signature firmware.sig firmware.bin\nif [ $? -ne 0 ]; then\n  echo \"signature verification failed\"; exit 1\nfi\n```\n\n## Verify A Signed EFI Binary\n\n```bash\nsbverify --cert db.crt grubx64.signed.efi || exit 1\npesign -S -i grubx64.signed.efi\n```\n\n## Verify A Signed Kernel Module\n\n```bash\nmodinfo amdgpu.ko | grep -q 'sig_id' || { echo unsigned; exit 1; }\n```\n\n## Wire It Into Promotion\n\nThe promotion job depends on the verify job and refuses to publish on a nonzero exit.\n\n```bash\nset -euo pipefail\n./verify-signatures.sh\naws s3 cp firmware.bin s3://release-prod/firmware/ \\\n  --metadata verified=true\n```\n\nBecause the gate uses `set -euo pipefail` and explicit exit codes, a failed check aborts promotion rather than silently shipping an unverified build." },
      { question: "Design an offline signing enclave that a pipeline requests signatures from.", answer: "## Architecture\n\nThe signing key lives only inside an isolated enclave, an `HSM` or an air-gapped signer, that exposes a narrow API: give me a digest and an authorization, I return a signature. The enclave never receives the full artifact and never returns the key.\n\n## Request Flow\n\nThe CI runner authenticates with a short-lived OIDC token, sends a digest plus artifact metadata, and the enclave enforces policy before signing.\n\n```bash\nopenssl dgst -sha256 -binary firmware.bin > fw.digest\ncurl -sf https://signer.internal/sign \\\n  -H \"Authorization: Bearer $CI_OIDC_TOKEN\" \\\n  -H \"Content-Type: application/octet-stream\" \\\n  --data-binary @fw.digest \\\n  -o firmware.sig\n```\n\n## Policy Enforcement Inside The Enclave\n\nThe signer validates the caller, checks the artifact is from an approved branch, enforces the monotonic anti-rollback version, and logs every request to an append-only audit log before the `PKCS#11` sign call.\n\n```bash\npkcs11-tool --sign --mechanism RSA-PKCS-PSS \\\n  --input-file fw.digest --output-file firmware.sig \\\n  --token-label fw-hsm --login\n```\n\n## Why It Is Safe\n\nCompromising a runner yields at most the ability to request one policy-checked signature, not the key. Rate limits, per-artifact authorization, and the audit log bound the blast radius, and rotating the enclave's key invalidates nothing the runners hold because they hold nothing." },
    ],
  },
  {
    id: "kernel-driver-builds",
    title: "Kernel & Driver Build Environments",
    icon: "settings",
    color: "#3b82f6",
    questions: 6,
    description: "How to build the kernel driver and out-of-tree modules like amdgpu across kernel versions in CI, covering Kbuild, vermagic/ABI, DKMS, cross-compiling, and Secure Boot signing.",
    introduction: "## Building Against the Kernel, Not Against libc\n\nAn out-of-tree kernel module is not a normal C program. It has no `libc`, no dynamic linker, and no stable ABI. It is compiled against one specific prepared kernel tree and is only guaranteed to load into a kernel built from that exact tree, with that exact `.config` and compiler. This is why `amdgpu` built for `6.8.0-31-generic` refuses to load on `6.8.0-35-generic` with `invalid module format`, even though the two version strings look almost identical.\n\n## The Core Build Invocation\n\nEvery out-of-tree build delegates to the kernel's own build system, Kbuild. The canonical command is `make -C /lib/modules/$(uname -r)/build M=$PWD modules`. The `-C` switches into the prepared kernel build directory so the kernel's top-level `Makefile` and `.config` drive the compile; `M=$PWD` tells Kbuild to build the external module in the current directory using its small `Kbuild`/`Makefile` with `obj-m += mydriver.o`.\n\n## Why This Matters in CI\n\nIn an AMD GPU-stack CI role you never build just once. You build the same driver across a matrix of kernel versions and architectures, on runners where the headers arrive over NFS, where Secure Boot demands signed modules, and where a kernel upgrade on a test host must not silently break the driver. That is exactly what `kernel-devel` packages, `vermagic`, DKMS, `CROSS_COMPILE`, and module signing collectively solve. This topic walks the practical mechanics of each.",
    whenToUse: [
      "You maintain an out-of-tree driver (for example `amdgpu-dkms` or a custom module) that must build and load cleanly across many kernel versions in CI.",
      "A module compiled successfully but fails at `modprobe`/`insmod` with `invalid module format` or `disagrees about version of symbol`, and you must trace it to an ABI/`vermagic` mismatch.",
      "You are packaging a driver so it automatically rebuilds when a test host receives a kernel upgrade, rather than silently losing the device after reboot.",
      "You must cross-compile a driver for a different architecture (for example `aarch64`) from x86_64 build runners.",
    ...
    pitfalls: [
      "Building against `/lib/modules/$(uname -r)/build` in CI when the runner's booted kernel is not the target kernel, so you silently build for the wrong ABI. Always point `-C` at the explicit headers dir for the matrix version, never assume `uname -r`.",
      "Assuming matching version strings means the module will load. Two kernels with the same release can still differ in `.config` (SMP, `MODVERSIONS`, compiler), producing a `vermagic` or symbol-CRC mismatch and `invalid module format`.",
      "Mismatched compiler: building a module with a different `gcc` major version than the one that built the target kernel can trigger `module was compiled with a different compiler` warnings and, with strict ABI or signing, load failures. Pin the toolchain to match the kernel's.",
      "Forgetting to re-sign after every rebuild on Secure Boot hosts. A DKMS rebuild on kernel upgrade produces a fresh unsigned `.ko`; without a signing hook the module builds fine but silently fails to load after reboot.",
      "Non-reproducible builds from embedded timestamps and build paths. Without pinning `KBUILD_BUILD_TIMESTAMP` (and build user/host), identical source yields different binaries, breaking content-hash build caches and making artifact comparison across the matrix meaningless.",
    ],
    keyQuestions: [
      { question: "How do you build an out-of-tree module in CI across a matrix of kernel versions?", answer: "## The Principle\n\nNever build against `$(uname -r)` in CI. The runner's booted kernel is irrelevant; you build against an explicitly installed headers tree for each target version, so the same source is compiled once per matrix entry.\n\n## Minimal External Module\n\n```makefile\nobj-m += amdgpu_shim.o\namdgpu_shim-y := main.o helpers.o\n\nKVER ?= $(shell uname -r)\nKDIR ?= /lib/modules/$(KVER)/build\n\nall:\n\t$(MAKE) -C $(KDIR) M=$(PWD) modules\n\nclean:\n\t$(MAKE) -C $(KDIR) M=$(PWD) clean\n```\n\n## Driving the Matrix\n\nInstall the headers per version, then pass `KVER` explicitly so `KDIR` resolves to the right prepared tree.\n\n```bash\nset -euo pipefail\nfor KVER in 6.5.0-41-generic 6.8.0-35-generic 6.11.0-9-generic; do\n  KDIR=/lib/modules/${KVER}/build\n  test -d \"${KDIR}\" || { echo \"missing headers for ${KVER}\"; exit 1; }\n  make -C \"${KDIR}\" M=\"$PWD\" KVER=\"${KVER}\" clean\n  make -C \"${KDIR}\" M=\"$PWD\" KVER=\"${KVER}\" modules\n  modinfo ./amdgpu_shim.ko | grep -E 'vermagic|filename'\n  cp amdgpu_shim.ko \"artifacts/amdgpu_shim-${KVER}.ko\"\ndone\n```\n\n## In a CI Matrix\n\nExpose the version as a matrix axis and install headers in each job. On Debian/Ubuntu:\n\n```bash\napt-get install -y \"linux-headers-${KVER}\"\n```\n\nOn Fedora/RHEL:\n\n```bash\ndnf install -y \"kernel-devel-${KVER}\"\n```\n\n## Why This Holds Up\n\nEach job is hermetic: it depends only on the installed headers for `KVER`, not on what the runner booted. The `test -d` guard fails loudly when a headers package is missing instead of falling back to the wrong ABI. Emitting `vermagic` per artifact gives a durable record of exactly which kernel each `.ko` targets." },
      { question: "How do you package a driver with DKMS so it survives kernel upgrades?", answer: "## Why DKMS\n\nA plain `.ko` installed by hand is orphaned the moment a new kernel boots, because the module lives under the old kernel's `/lib/modules` tree. DKMS solves this by keeping the source registered and rebuilding automatically on every kernel install.\n\n## The `dkms.conf`\n\nSource lives at `/usr/src/<name>-<version>/` next to a `dkms.conf`.\n\n```bash\nPACKAGE_NAME=\"amdgpu_shim\"\nPACKAGE_VERSION=\"1.4.2\"\nBUILT_MODULE_NAME[0]=\"amdgpu_shim\"\nDEST_MODULE_LOCATION[0]=\"/updates/dkms\"\nMAKE[0]=\"make -C ${kernel_source_dir} M=${dkms_source_tree}/${PACKAGE_NAME}-${PACKAGE_VERSION} modules\"\nCLEAN=\"make -C ${kernel_source_dir} M=${dkms_source_tree}/${PACKAGE_NAME}-${PACKAGE_VERSION} clean\"\nAUTOINSTALL=\"yes\"\n```\n\n`AUTOINSTALL=yes` plus the kernel package post-install hooks are what rebuild the module on upgrade. `${kernel_source_dir}` is filled in by DKMS with the target kernel's build tree.\n\n## Register, Build, Install\n\n```bash\ncp -r ./amdgpu_shim /usr/src/amdgpu_shim-1.4.2\ndkms add     -m amdgpu_shim -v 1.4.2\ndkms build   -m amdgpu_shim -v 1.4.2\ndkms install -m amdgpu_shim -v 1.4.2\ndkms status\n```\n\n## Surviving the Upgrade\n\nWhen `apt`/`dnf` installs a new kernel, its DKMS trigger runs `dkms autoinstall`, which rebuilds `amdgpu_shim` against the new kernel's headers before reboot. You can target a specific kernel explicitly:\n\n```bash\ndkms install -m amdgpu_shim -v 1.4.2 -k 6.11.0-9-generic\n```\n\n## Building the Distributable Package\n\n```bash\ndkms mkdeb -m amdgpu_shim -v 1.4.2 --source-only\n```\n\nThis is the exact shape of AMD's `amdgpu-dkms`: source plus `dkms.conf`, rebuilt per kernel. Confirm the headers for each target kernel are present, or `dkms autoinstall` silently skips that kernel and the device disappears after reboot." },
      { question: "How do you diagnose a vermagic/ABI mismatch at load time?", answer: "## The Symptom\n\n```bash\ninsmod ./amdgpu_shim.ko\n# insmod: ERROR: could not insert module ./amdgpu_shim.ko: Invalid module format\n```\n\nor the same via `modprobe`, always with a `dmesg` line naming the real cause.\n\n## Step 1: Read the Kernel Log\n\n`dmesg` tells you which of the two gates failed.\n\n```bash\ndmesg | tail -5\n# amdgpu_shim: version magic '6.8.0-31-generic SMP mod_unload modversions'\n#   should be '6.8.0-35-generic SMP mod_unload modversions'\n```\n\nThat is a `vermagic` mismatch: the module was built against `6.8.0-31` headers but loaded on `6.8.0-35`.\n\n## Step 2: Compare vermagic Directly\n\n```bash\nmodinfo ./amdgpu_shim.ko | grep vermagic\nuname -r\ncat /proc/version\n```\n\nIf the `vermagic` release does not match `uname -r`, you built against the wrong headers tree. Rebuild with `-C /lib/modules/$(uname -r)/build`.\n\n## Step 3: When the Version Matches but It Still Fails\n\nA symbol-CRC (modversions) mismatch looks different:\n\n```bash\ndmesg | tail -3\n# amdgpu_shim: disagrees about version of symbol drm_dev_alloc\n```\n\nSame kernel release, different symbol CRC. The running kernel's `Module.symvers` differs from the one you built against (different `.config`, or an ABI-breaking backport). Diff the CRC for the offending symbol:\n\n```bash\ndiff <(grep drm_dev_alloc /path/to/build/Module.symvers) \\\n     <(grep drm_dev_alloc /lib/modules/$(uname -r)/build/Module.symvers)\n```\n\n## Step 4: Compiler Mismatch\n\n```bash\ncat /proc/version              # compiler that built the kernel\ngcc --version | head -1        # compiler you built the module with\n```\n\nA different `gcc` major version can shift ABI details; pin the toolchain to the one that built the target kernel and rebuild.\n\n## The Fix Pattern\n\nAlmost every case reduces to the same thing: you built against a different prepared tree than the kernel you are loading into. Rebuild against the exact target headers, confirm `modinfo` `vermagic` equals `uname -r`, then reload." },
      { question: "How do you cross-compile a driver for aarch64 from an x86_64 runner?", answer: "## What Cross-Compiling Requires\n\nThree things must all agree on the target: an `aarch64` toolchain, a kernel build tree prepared for `arm64`, and the `ARCH`/`CROSS_COMPILE` make variables pointing at both.\n\n## Install the Toolchain\n\n```bash\napt-get install -y gcc-aarch64-linux-gnu\naarch64-linux-gnu-gcc --version\n```\n\n## The Build Invocation\n\n`ARCH` selects the `arm64` subtree in the kernel tree; `CROSS_COMPILE` is the tool prefix.\n\n```bash\nexport ARCH=arm64\nexport CROSS_COMPILE=aarch64-linux-gnu-\n\nmake -C /path/to/linux-headers-arm64 \\\n     ARCH=arm64 \\\n     CROSS_COMPILE=aarch64-linux-gnu- \\\n     M=\"$PWD\" modules\n```\n\n## The Critical Constraint\n\nThe tree passed to `-C` must itself be an `arm64`-configured tree (its `.config` and `Module.symvers` built for `arm64`). You cannot point at the host's x86_64 `/lib/modules/$(uname -r)/build` and set `ARCH=arm64`, because the symbols and config are wrong. Install the cross headers explicitly (for example `linux-headers-arm64`) or prepare a tree with `make ARCH=arm64 modules_prepare`.\n\n## Verify the Output Architecture\n\n```bash\nfile amdgpu_shim.ko\n# amdgpu_shim.ko: ELF 64-bit LSB relocatable, ARM aarch64, ...\nmodinfo amdgpu_shim.ko | grep vermagic\n```\n\n`file` must report `ARM aarch64`, not `x86-64`. If it still says `x86-64`, `CROSS_COMPILE` was not applied to the recursive Kbuild invocation. Pass it both as an env export and on the `make` line to be safe.\n\n## Preparing an arm64 Tree From Source\n\nIf no cross-headers package exists:\n\n```bash\nmake ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- defconfig\nmake ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- modules_prepare\n```\n\n`modules_prepare` builds just enough (`scripts`, generated headers, `Module.symvers`) to compile external modules without a full kernel build." },
      { question: "How do you sign modules so they load on a Secure Boot host?", answer: "## Why Signing Is Needed\n\nUnder Secure Boot with kernel lockdown, the kernel refuses to load a module whose signature is not trusted, logging `Loading of unsigned module is rejected` (or tainting and blocking). You must sign each `.ko` with a key whose certificate is enrolled in the platform.\n\n## Generate a Signing Key\n\n```bash\nopenssl req -new -x509 -newkey rsa:2048 \\\n  -keyout MOK.priv -out MOK.der -outform DER \\\n  -nodes -days 36500 \\\n  -subj \"/CN=amdgpu CI signing key/\"\n```\n\n## Enroll the Certificate (MOK)\n\n```bash\nmokutil --import MOK.der\n# reboot; the MOK Manager prompts to enroll, then:\nmokutil --list-enrolled | grep -i amdgpu\n```\n\nThis is a one-time-per-host step; the enrolled key persists across kernel upgrades.\n\n## Sign the Module\n\nThe kernel ships `scripts/sign-file`, which appends the signature to the `.ko`.\n\n```bash\nKDIR=/lib/modules/$(uname -r)/build\n\"${KDIR}/scripts/sign-file\" sha256 MOK.priv MOK.der amdgpu_shim.ko\n\nmodinfo amdgpu_shim.ko | grep -E 'sig_id|signer|sig_key'\n```\n\n## Sign on Every Rebuild\n\nThe trap: DKMS rebuilds after a kernel upgrade and emits a fresh unsigned `.ko`. Wire signing into DKMS so it happens automatically.\n\n```bash\n# in dkms.conf\nPOST_BUILD=\"sign_dkms.sh\"\n```\n\n```bash\n#!/usr/bin/env bash\n# sign_dkms.sh\nset -euo pipefail\nKDIR=\"/lib/modules/${kernelver}/build\"\n\"${KDIR}/scripts/sign-file\" sha256 /root/keys/MOK.priv /root/keys/MOK.der \\\n  \"$(pwd)/amdgpu_shim.ko\"\n```\n\n## Verify It Loads\n\n```bash\nmodprobe amdgpu_shim\ndmesg | tail -2   # expect no 'unsigned module is rejected'\n```\n\nIf load still fails after signing, check `mokutil --sb-state` (Secure Boot enabled) and confirm the key that signed the module is the one enrolled. A rebuild that regenerated the key invalidates prior enrollment." },
      { question: "How do you handle NFS-mounted kernel headers and toolchains on the runner, and keep builds reproducible?", answer: "## The Infra Reality\n\nCI runners often mount `kernel-devel` trees and toolchains from a shared NFS export rather than installing packages locally. This saves disk and pins one blessed toolchain, but it introduces read-only mounts, path sensitivity, and cache concerns you must design around.\n\n## Point Kbuild at the NFS Tree\n\n```bash\nexport KHDR=/nfs/toolchains/linux-headers/6.8.0-35-generic\nexport PATH=/nfs/toolchains/gcc-13.2/bin:$PATH\n\nmake -C \"${KHDR}\" M=\"$PWD\" modules\n```\n\n## Never Write Into the Read-Only Mount\n\nKbuild normally writes object files under the module dir, which `M=$PWD` already keeps out of the headers tree. But if the module dir itself is on NFS, redirect the build to local scratch to avoid write failures, latency, and file-locking stalls that surface as a hung `modpost`.\n\n```bash\nmkdir -p /var/tmp/build\ncp -r ./src/* /var/tmp/build/\nmake -C \"${KHDR}\" M=/var/tmp/build modules\n```\n\n## Pin the Toolchain Explicitly\n\nDo not rely on the runner's default `gcc`. The module's compiler should match the kernel's; source the exact NFS toolchain and verify:\n\n```bash\ngcc --version | head -1\ncat \"${KHDR}/include/config/kernel.release\"\n```\n\n## Make the Build Reproducible\n\nEmbedded timestamps and build paths make otherwise-identical builds differ, defeating artifact caches. Pin them:\n\n```bash\nexport KBUILD_BUILD_TIMESTAMP='2026-01-01T00:00:00Z'\nexport KBUILD_BUILD_USER=ci\nexport KBUILD_BUILD_HOST=ci-runner\nexport SOURCE_DATE_EPOCH=1735689600\n\nmake -C \"${KHDR}\" M=/var/tmp/build modules\nsha256sum /var/tmp/build/amdgpu_shim.ko\n```\n\nWith these pinned, the same source plus the same NFS headers and toolchain yield a byte-identical `.ko`, so a content-hash cache can safely skip rebuilds across the matrix.\n\n## Cache Strategy and Mount Guards\n\nKey the build cache on `sha256(source) + kernel.release + toolchain version + config hash`. Because the headers and toolchain come from a versioned NFS path, that path is part of the key, so a toolchain bump changes the path and correctly invalidates the cache. Guard the mount at job start:\n\n```bash\nmountpoint -q /nfs/toolchains || { echo 'NFS toolchains not mounted'; exit 1; }\ntest -d \"${KHDR}\" || { echo 'headers tree missing'; exit 1; }\n```\n\nFailing loudly here beats silently building against a stale or half-mounted tree." },
    ],
  },
  // ─── FUNDAMENTALS ──────────────────────────────────────────────────────────
  {
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
Understanding the Linux boot process helps diagnose startup failures and tune boot performance.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
The Linux kernel is monolithic: all core subsystems (memory management, process scheduling, filesystems, networking, device drivers) run in kernel space at the same privilege level. Unlike microkernels (where subsystems run as separate processes), this gives better performance at the cost of isolation — a kernel bug can crash the entire system.
...
\`\`\``,
  ...
  // ─── SHELL (additional) ────────────────────────────────────────────────────
  {
    ...
    introduction: `## Overview
Bash variables and the environment are fundamental to shell scripting and interactive use. Getting them wrong causes subtle, hard-to-debug bugs.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
Bash provides multiple conditional constructs, and choosing the right one is critical for correctness and portability.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
Bash functions allow code reuse and organization within scripts. They behave like mini-scripts within your script, with their own positional parameters but sharing the script's variable scope by default.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
The Unix text processing toolkit is a core skill for Linux administration and DevOps. These tools are composable via pipes and form the basis of shell-based data processing pipelines.
...
| grep -F | Fixed strings — performance, or when pattern has regex metacharacters |`,
  ...
  },
  {
    ...
    introduction: `## Overview
Bash job control allows managing multiple processes from a single terminal session. Understanding it is essential for running background tasks, keeping processes alive after logout, and debugging pipeline behavior.
...
\`\`\``,
  ...
  // ─── NETWORKING (additional) ───────────────────────────────────────────────
  {
    ...
    introduction: `## Overview
Linux kernel routing determines where outgoing packets are sent. Every packet consults the routing table, and the kernel applies longest prefix match to select the best route.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
ss (socket statistics) is the modern replacement for netstat. It reads socket information directly from the kernel via netlink socket rather than parsing /proc/net/tcp, making it significantly faster on systems with many connections.
...
CLOSE_WAIT connections cannot be fixed by kernel tuning. The application must be fixed to call close() on connections when done. Workaround: restart the app (clears all sockets). Permanent fix: add proper connection cleanup in error paths.`,
  ...
  },
  {
    ...
    introduction: `## Overview
tcpdump is the essential command-line packet capture tool. It uses libpcap and BPF (Berkeley Packet Filter) to capture and filter network packets at the kernel level, minimizing the data sent to userspace.
...
- Connection RST after Certificate: client rejected the cert`,
  ...
  },
  {
    ...
    introduction: `## Overview
DNS debugging is a critical skill — many service failures are ultimately DNS failures. The tools and configuration files involved form a layered system.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
curl is the Swiss Army knife for HTTP debugging and API testing. It supports nearly every HTTP feature and provides detailed output of the full request/response cycle.
...
\`\`\``,
  ...
  // ─── PERFORMANCE (additional) ──────────────────────────────────────────────
  {
    ...
    introduction: `## Overview
top and htop are the first tools you reach for when investigating system performance. Understanding what you're looking at is critical — the numbers can be misleading without context.
...
| Fix: faster disk, I/O optimization | Fix: profile hot code, optimize algorithm |`,
  ...
  },
  {
    ...
    introduction: `## Overview
vmstat and iostat provide system-wide and per-device performance statistics at a level of detail that top doesn't offer.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
perf is the Linux performance profiling Swiss Army knife, built directly into the kernel subsystem. It uses hardware performance counters and software events to profile at near-zero overhead.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
strace intercepts every interaction between a process and the Linux kernel. Since all I/O, file access, memory allocation (ultimately), and process management goes through system calls, strace reveals exactly what a program is doing at the lowest level.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
lsof (List Open Files) is uniquely powerful on Linux because "everything is a file" — regular files, directories, sockets, pipes, devices, and more all appear in lsof output. It's an essential tool for diagnosing resource leaks, connection issues, and deleted-file problems.
...
\`\`\``,
  ...
  },
  {
    ...
    introduction: `## Overview
The Linux CPU scheduler determines which process runs on which CPU at any given moment. Understanding the scheduler helps optimize performance for both latency-sensitive and throughput-oriented workloads.
