...
export const linuxTopics = [
  ...
  // ─── FUNDAMENTALS (additional) ──────────────────────────────────────────────
  {
    ...
    visualizations: [],
    introduction: `File permissions are the foundation of Linux security. Every file and directory has an owner (user), a group, and a permission set for three classes: owner, group, and others. Each class gets three bits: read (r=4), write (w=2), and execute (x=1).

## Octal vs Symbolic Notation

chmod accepts octal or symbolic notation:
...
umask is a mask subtracted from the default permissions when new files are created. Default file permissions are 666, directories 777. With umask 022: files get 644, directories get 755. umask 027 gives files 640/directories 750.

## Special Bits

- SUID (4xxx): When set on an executable, it runs as the file owner (e.g., /usr/bin/passwd runs as root). chmod u+s file or chmod 4755 file.
- SGID (2xxx): On executables, runs as the file group. On directories, new files inherit the directory group.
- Sticky bit (1xxx): On directories (e.g., /tmp), only the file owner or root can delete files.

## POSIX ACLs

Standard ugo permissions only allow one user and one group. ACLs allow per-user and per-group rules:

\`\`\`bash
setfacl -m u:alice:rw file.txt   # grant alice read+write
setfacl -m g:devs:r  file.txt   # grant devs group read
getfacl file.txt                 # view all ACL entries
\`\`\`

A + at the end of ls -l output means an ACL is present.`,
    whenToUse: [
      'Diagnosing "Permission denied" errors on files or directories',
      'Setting up shared project directories where a team needs group write access with SGID',
      'Hardening a system by auditing SUID/SGID binaries with find / -perm /6000',
      'Granting a single user access without changing group ownership using setfacl',
    ],
    keyConcepts: [
      { term: 'Octal notation', definition: 'Three digits (e.g., 755) each representing owner/group/others. Each digit sums r(4)+w(2)+x(1). chmod 644 = owner rw, group r, others r.' },
      { term: 'umask', definition: 'A subtraction mask for default permissions. umask 022 means new files get 666-022=644, new dirs get 777-022=755. Set persistently in ~/.bashrc or /etc/profile.' },
      { term: 'SUID bit', definition: 'Set User ID on an executable: the process runs with the file owner\'s effective UID. ls -l shows s in the owner execute column. Critical for tools like passwd and sudo.' },
      { term: 'SGID on directories', definition: 'New files inside inherit the directory\'s group. Set with chmod g+s dir. Critical for shared team directories where all members need access to each other\'s files.' },
      { term: 'Sticky bit', definition: 'On a directory, prevents users from deleting files they don\'t own. ls -ld /tmp shows t in the others execute column. Essential for world-writable directories.' },
      { term: 'POSIX ACLs', definition: 'Per-user/per-group permissions beyond the nine-bit model. setfacl -m u:alice:rw file grants alice rw without changing group. Files with ACLs show + in ls -l.' },
    ],
    pitfalls: [
      'Setting 777 on directories to "fix permissions" — this lets any user delete anyone else\'s files. Always pair world-write with the sticky bit.',
      'Expecting SUID to work on shell scripts — Linux ignores SUID on interpreted scripts. It only works on compiled ELF binaries.',
      'Overlooking the ACL mask — even if setfacl grants rwx, the mask entry caps the effective permissions. Run getfacl to see effective permissions.',
      'Using chmod -R 777 recursively on /var/www — makes every file executable and removes all access control, a serious security risk.',
    ],
    keyQuestions: [
      {
        question: 'What does chmod 4755 do, and why is SUID on /usr/bin/passwd necessary?',
        answer: `## chmod 4755

The leading 4 sets the SUID bit. 755 gives owner rwx, group r-x, others r-x. The file runs with the file owner's UID regardless of who executes it.
...
The s in the owner execute column confirms SUID is set.

## Why passwd needs SUID

/etc/shadow (which stores hashed passwords) is readable only by root:
...
When a normal user runs passwd, the process needs to write to /etc/shadow. Because SUID is set and the owner is root, the process runs with effective UID 0 (root). Without SUID, normal users could never change their own passwords.
...
\`\`\``,
      ...
      {
        question: 'How does umask work, and how would you set it so new files are 640 and directories are 750?',
        answer: `## umask mechanics

umask is a permission mask applied when the kernel creates new files. The kernel starts with defaults (666 for files, 777 for dirs) and subtracts the umask:
...
# Dirs:   777 - 027 = 750  (owner rwx, group rx, others none)
\`\`\`

Set permanently in /etc/profile (system-wide) or ~/.bashrc (user-specific).

## Verify

\`\`\`bash
umask          # show current umask (027)
umask -S       # symbolic form: u=rwx,g=rx,o=
touch testfile && ls -l testfile    # should show -rw-r-----
mkdir testdir  && ls -ld testdir    # should show drwxr-x---
\`\`\``,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#3b82f6',
    questions: 6,
    description: '/etc/passwd, /etc/shadow, useradd/usermod, primary vs supplementary groups, and su vs sudo.',
    visualizations: [],
    introduction: `User and group management is foundational to Linux security. The kernel identifies users by UID and groups by GID — names are just human-readable aliases stored in text files.
...
The x in field 2 means the password hash is in /etc/shadow. UIDs below 1000 are reserved for system accounts.

## /etc/shadow — Password Hashing

Readable only by root. Contains hashed passwords in $algorithm$salt$hash format ($6$ = SHA-512). An ! or * prefix means the account is locked.

## useradd vs adduser

On Debian/Ubuntu, adduser is an interactive Perl wrapper. useradd is the low-level POSIX tool:

\`\`\`bash
useradd -m -s /bin/bash -G sudo,docker alice
# -m: create home dir, -s: shell, -G: supplementary groups
usermod -aG docker alice   # -a is CRITICAL: append, don't replace groups
...
Every user has one primary group (in /etc/passwd field 4) and zero or more supplementary groups (in /etc/group). Use id alice to see all. New files get the creator's primary group unless SGID is set on the directory.

## su vs sudo

su switches to another user, requiring that user's password. sudo runs a single command with elevated privileges using the caller's own password, with full audit logging to /var/log/auth.log.`,
    whenToUse: [
      'Provisioning new user accounts on a Linux server',
      'Debugging "operation not permitted" errors caused by missing group membership',
      'Auditing who has sudo access: grep -r NOPASSWD /etc/sudoers /etc/sudoers.d/',
      'Setting up service accounts with nologin shell for running daemons',
    ],
    keyConcepts: [
      { term: '/etc/passwd format', definition: 'Seven fields: username:x:UID:GID:GECOS:home:shell. The x means password is in /etc/shadow. UID 0 = root. System accounts use /sbin/nologin as shell.' },
      { term: '/etc/shadow', definition: 'Root-only file with password hashes ($6$=SHA-512) and aging policy. An ! prefix locks the account. passwd -l username adds the lock prefix.' },
      { term: 'usermod -aG', definition: 'Append the user to a supplementary group without removing existing groups. Omitting -a replaces ALL groups — a classic destructive mistake that removes sudo access.' },
      { term: 'nologin shell', definition: 'Setting /usr/sbin/nologin or /bin/false as the shell prevents interactive login while allowing the account to own files and run processes.' },
      { term: 'su vs sudo', definition: 'su requires knowing the target user\'s password. sudo uses the caller\'s own password and logs every command. Prefer sudo for auditability.' },
    ],
    pitfalls: [
      'Running usermod -G without -a — replaces ALL supplementary groups, potentially removing sudo access instantly.',
      'Adding a user to a group and expecting immediate effect in the current session — the user must log out and back in (or run newgrp) for the change to apply.',
      'Editing /etc/passwd or /etc/shadow with a plain text editor — use vipw/vigr to prevent concurrent write corruption.',
      'Enabling root SSH login — use PermitRootLogin no and require sudo for escalation and full auditability.',
    ],
    keyQuestions: [
      {
        question: 'What is the difference between su and sudo, and why should root login be disabled in production?',
        answer: `## su — Substitute User

su alice starts a shell as alice using alice's password. su - gives a full login shell. Problems: requires sharing the target user's password, no per-command audit trail.

## sudo — Privileged Single Command

sudo command runs as root using the caller's own password. Every invocation is logged:
...
1. Accountability: every sudo command is logged with the calling username
2. Attack surface: root is a known username targeted by brute force attacks
3. Least privilege: scoped sudo rules limit blast radius of compromised account

\`\`\`bash
# /etc/ssh/sshd_config
PermitRootLogin no

# /etc/sudoers — grant only needed commands
alice ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart myapp
\`\`\``,
      },
      {
        question: 'How would you create a service account for running a web application daemon?',
        answer: `\`\`\`bash
# Create system user: no home, nologin shell, UID < 1000
useradd --system --no-create-home --shell /usr/sbin/nologin webappd

# Set ownership of app files
chown -R webappd:webappd /opt/webapp
chmod 750 /opt/webapp

# Lock the account further
passwd -l webappd
...
The systemd service runs as webappd. Even if the application is compromised, the attacker gets only webappd's limited privileges — no shell, no home directory, no sudo access.`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#3b82f6',
    questions: 6,
    description: 'apt/dpkg, yum/dnf/rpm, snap packages, and building from source with configure/make.',
    visualizations: [],
    introduction: `Package management is how Linux systems install, update, and remove software while tracking dependencies.

## Debian/Ubuntu — APT and dpkg

apt resolves dependencies and downloads packages from repositories. dpkg is the low-level tool.

\`\`\`bash
apt update                   # Refresh package index from sources
apt upgrade                  # Upgrade installed packages
apt dist-upgrade             # Smart upgrade — handles dependency changes
apt install nginx
...
## RHEL/CentOS — DNF and RPM

\`\`\`bash
dnf update
dnf install httpd
dnf remove httpd
rpm -qa | grep httpd         # List installed RPMs
rpm -qi httpd                # Package info
rpm -ql httpd                # Files in package
rpm -qf /etc/httpd/conf/httpd.conf  # Which package owns this file
\`\`\`

## Repositories

APT repos in /etc/apt/sources.list and /etc/apt/sources.list.d/. PPAs add third-party repos via add-apt-repository ppa:name/ppa. RHEL repos in /etc/yum.repos.d/.

## snap Packages

Self-contained with bundled dependencies. Strict confinement via AppArmor/seccomp:

\`\`\`bash
snap install code --classic    # classic = less restricted
snap list
...
\`\`\``,
    whenToUse: [
      'Pinning a package version to prevent upgrades: apt-mark hold or dnf versionlock',
      'Finding which package owns a config file: dpkg -S or rpm -qf',
      'Diagnosing broken package state after failed install: dpkg --configure -a',
      'Installing a specific version not in repos — build from source or download .deb/.rpm directly',
    ],
    keyConcepts: [
      { term: 'apt vs dpkg', definition: 'apt resolves dependencies and downloads from repos. dpkg installs .deb files directly — no dependency resolution. Broken installs fixed with dpkg --configure -a.' },
      { term: 'apt update vs upgrade', definition: 'apt update refreshes the local package index. apt upgrade installs newer versions. dist-upgrade can also remove packages to satisfy new dependencies.' },
      { term: 'dpkg -S / rpm -qf', definition: 'Find which package owns a file: dpkg -S /usr/bin/curl (Debian) or rpm -qf /usr/bin/curl (RHEL). Essential for understanding what installed a binary.' },
      { term: 'Package hold / versionlock', definition: 'Prevent upgrades: apt-mark hold nginx on Debian; dnf versionlock add nginx on RHEL. Critical before a planned maintenance window.' },
    ],
    pitfalls: [
      'Running apt upgrade without apt update first — the local index is stale and you may get wrong versions or dependency errors.',
      'Using dpkg -i for a package with dependencies — use "apt install ./package.deb" instead which resolves deps.',
      'Forgetting apt update before apt install in a CI container — the stale index resolves old versions or fails entirely.',
      'Using rpm -i instead of rpm -U for upgrades — installs a second copy alongside the old version.',
    ],
    keyQuestions: [
      {
        question: 'How do you find which package owns a specific file, and how do you hold a package at a specific version?',
        answer: `## Finding Package Owner

\`\`\`bash
# Debian/Ubuntu
dpkg -S /usr/bin/nginx
apt-file search /usr/bin/nginx   # also searches non-installed packages
...
Holds prevent accidental upgrades during apt upgrade that could break production services. In production, always hold critical library versions (like openssl, glibc) at a tested version.`,
      },
      {
        question: 'What is the difference between apt upgrade and apt dist-upgrade?',
        answer: `## apt upgrade

Upgrades packages but never removes installed packages or installs new ones to satisfy dependencies. If upgrading package A requires a new package B or removing package C, apt upgrade skips A entirely. Safe and conservative.

## apt dist-upgrade (now: apt full-upgrade)

Intelligently handles dependency changes: will install new packages and remove old ones to resolve the upgrade. Used for major system upgrades and when held-back packages appear in apt upgrade.

\`\`\`bash
# See what would be upgraded vs held back
apt list --upgradable

# Conservative: won't remove anything
apt upgrade

# Full: resolves all dependency changes
apt full-upgrade   # modern alias for dist-upgrade
\`\`\`

## When to use each

- apt upgrade: routine security updates on production servers
- apt full-upgrade: when packages are "held back", distribution upgrades, or kernel updates`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#3b82f6',
    questions: 6,
    description: 'POST to UEFI to GRUB2 to kernel to initramfs to systemd: the complete boot sequence from power-on to login.',
    visualizations: [],
    introduction: `Understanding the Linux boot process is essential for debugging boot failures, optimizing startup time, and understanding how system components fit together.

## Phase 1: Firmware (BIOS/UEFI)

At power-on, the CPU executes firmware. BIOS (legacy) performs POST (Power-On Self Test), then reads the MBR (first 512 bytes). UEFI (modern) reads the EFI System Partition (ESP, FAT32) for .efi bootloader files. UEFI also supports Secure Boot — cryptographic verification of the boot chain.

## Phase 2: Bootloader (GRUB2)

GRUB2 presents the boot menu, loads the kernel (vmlinuz) and initramfs into memory, and passes kernel parameters. Configuration lives at /etc/default/grub — never edit /boot/grub/grub.cfg directly:
...
The kernel decompresses itself, detects hardware, and mounts the initramfs as a temporary rootfs.

## Phase 4: initramfs

The initramfs (initial RAM filesystem) is a compressed CPIO archive containing drivers needed to mount the real root — handling LUKS encryption, LVM, RAID, NFS root. After mounting real root, the kernel performs pivot_root to switch from initramfs to the real filesystem, then executes /sbin/init as PID 1.

## Phase 5: systemd

systemd reads unit files and activates the default target:
- multi-user.target — non-graphical (approx runlevel 3)
- graphical.target — with display manager (approx runlevel 5)
- rescue.target — single user for repair

\`\`\`bash
systemd-analyze time           # Total boot time breakdown
systemd-analyze blame          # Time per unit, sorted descending
systemd-analyze critical-chain # Dependency chain determining boot time
\`\`\``,
    whenToUse: [
      'Recovering a system that won\'t boot — boot to rescue target, chroot, fix grub or fstab',
      'Optimizing cloud VM startup time with systemd-analyze blame',
      'Adding kernel parameters for hardware issues: nomodeset, acpi=off, console=ttyS0',
      'Diagnosing a "dracut emergency shell" at boot — usually a missing block device in fstab',
    ],
    keyConcepts: [
      { term: 'initramfs', definition: 'Temporary rootfs in RAM containing enough drivers to mount the real root filesystem (LVM, LUKS, NFS). Built by dracut or initramfs-tools. Discarded after pivot_root.' },
      { term: 'GRUB2 configuration', definition: 'Edit /etc/default/grub and run update-grub to regenerate /boot/grub/grub.cfg. Never edit grub.cfg directly — it\'s overwritten on every kernel update.' },
      { term: 'Kernel cmdline parameters', definition: 'Passed by GRUB. Key ones: ro (read-only initial mount), quiet (suppress messages), single or systemd.unit=rescue.target for recovery. View with cat /proc/cmdline.' },
      { term: 'systemd targets', definition: 'Replace SysV runlevels. multi-user.target = runlevel 3, graphical.target = runlevel 5. Set default with systemctl set-default multi-user.target.' },
      { term: 'systemd-analyze blame', definition: 'Lists units sorted by initialization time. Identifies slow services delaying boot. Pair with systemd-analyze critical-chain for dependency context.' },
    ],
    pitfalls: [
      'Editing /boot/grub/grub.cfg directly — it is regenerated by update-grub and your changes are lost on the next kernel update.',
      'Forgetting to rebuild initramfs after adding a kernel module needed at boot — the system drops to an emergency shell.',
      'Confusing BIOS MBR booting with UEFI ESP booting — installing GRUB for the wrong firmware type results in an unbootable system.',
      'Using "runlevel 3" as a systemd target — the correct target is multi-user.target.',
    ],
    keyQuestions: [
      {
        question: 'What is initramfs and why does Linux need it even when the kernel supports ext4 natively?',
        answer: `## The Chicken-and-Egg Problem

Even with ext4 built into the kernel, initramfs solves a fundamental bootstrap problem for complex storage stacks:

- Root on LVM: LVM tools must run before mounting root
- Root on LUKS: disk must be decrypted (needs passphrase prompt) before mounting
- Root on RAID: md arrays must be assembled first
- Root on iSCSI/NFS: network must be configured first

The kernel can't do these at early boot — it needs userspace tools. initramfs provides that minimal userspace environment.

\`\`\`bash
# View initramfs contents
lsinitramfs /boot/initrd.img-$(uname -r) | head -30

# Rebuild after adding drivers
update-initramfs -u -k all     # Debian/Ubuntu
dracut --force                 # RHEL/CentOS
\`\`\``,
      },
      {
        question: 'How do you recover a system where GRUB is broken?',
        answer: `\`\`\`bash
...
For quick kernel parameter edits: at the GRUB menu press e, find the linux line, add single or init=/bin/bash, then Ctrl+X to boot into a root shell.`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#3b82f6',
    questions: 6,
    description: 'Monolithic kernel architecture, loadable modules, /proc and /sys virtual filesystems, and sysctl tuning.',
    visualizations: [],
    introduction: `The Linux kernel is a monolithic kernel — all core OS services (process scheduling, memory management, filesystems, device drivers, networking) run in a single privileged address space. This differs from microkernels where services run in userspace. Linux's monolithic design provides low overhead but means a kernel module bug can crash the entire system.

## Loadable Kernel Modules

Linux's practical answer is Loadable Kernel Modules (LKMs) — driver code that can be dynamically loaded/unloaded without rebooting. Modules live in /lib/modules/$(uname -r)/:

\`\`\`bash
lsmod                        # List loaded modules and their dependencies
modprobe nf_conntrack        # Load a module and its dependencies
modprobe -r nf_conntrack     # Unload a module
modinfo nf_conntrack         # Module metadata: params, author, dependencies
insmod /path/to/module.ko    # Load directly by path (no dep resolution)
rmmod nf_conntrack           # Remove (fails if module is in use)
\`\`\`

Persistent loading: /etc/modules-load.d/ (load at boot), parameters in /etc/modprobe.d/.

## /proc — Process and Kernel State

/proc is a virtual filesystem — no disk I/O, all data generated live by the kernel:

- /proc/cpuinfo — CPU model, cores, flags
- /proc/meminfo — MemTotal, MemFree, MemAvailable, Cached, SwapUsed
- /proc/sys/ — tunable kernel parameters (readable AND writable)
- /proc/PID/ — per-process: cmdline, environ, fd/, maps, status

## /sys — Device and Driver Attributes

/sys (sysfs) exposes the kernel's device/driver model as a directory tree. Used for device configuration:
...
sysctl -w net.ipv4.ip_forward=1  # Set (not persistent)
sysctl -p /etc/sysctl.conf   # Apply persistent settings
\`\`\`

Critical parameters for cloud/container engineering:
- net.ipv4.ip_forward=1 — required for container networking
- vm.swappiness=10 — reduce swap for latency-sensitive workloads
- net.core.somaxconn=65535 — increase listen backlog for high-traffic servers`,
    whenToUse: [
      'Enabling IP forwarding for container networking: sysctl -w net.ipv4.ip_forward=1',
      'Tuning vm.swappiness to minimize swap on database servers',
      'Diagnosing a service that cannot open more files: check /proc/sys/fs/file-max and ulimit -n',
      'Investigating a process\'s open file descriptors via /proc/PID/fd',
    ],
    keyConcepts: [
      { term: 'Monolithic kernel', definition: 'All OS services run in one kernel address space. Fast due to no IPC overhead, but a faulty module crashes the whole kernel. Linux uses loadable modules to add drivers without rebooting.' },
      { term: 'modprobe vs insmod', definition: 'modprobe loads a module with dependency resolution (reads modules.dep). insmod loads a specific .ko file directly with no dependency handling — fails if deps are missing.' },
      { term: 'MemAvailable vs MemFree', definition: 'MemFree is completely unused RAM — typically low because the kernel uses free RAM for page cache. MemAvailable is the kernel\'s estimate of RAM available for new allocations including reclaimable cache. Always monitor MemAvailable.' },
      { term: 'sysctl persistence', definition: 'sysctl -w changes are lost on reboot. For persistence, add to /etc/sysctl.d/99-custom.conf and run sysctl -p.' },
      { term: 'net.ipv4.ip_forward', definition: 'Must be 1 for the kernel to forward packets between interfaces. Required for Docker, Kubernetes, and routing. Disabled (0) by default for security.' },
    ],
    pitfalls: [
      'Using sysctl -w without adding to /etc/sysctl.d/ — changes are lost on reboot and the system silently regresses.',
      'Setting vm.swappiness=0 on Kubernetes nodes — the OOM killer fires instead of swapping, causing arbitrary pod kills.',
      'Using insmod instead of modprobe — insmod has no dependency resolution, causing cryptic "unknown symbol" errors.',
      'Not checking /proc/PID/fd when a process holds a deleted file open — the file occupies disk space that df cannot account for.',
    ],
    keyQuestions: [
      {
...
\`\`\``,
      ...
      {
        question: 'How do you load a kernel module persistently and pass parameters to it?',
        answer: `\`\`\`bash
...
options br_netfilter nf_call_iptables=1
EOF

cat > /etc/modprobe.d/blacklist-nouveau.conf << 'EOF'
blacklist nouveau
EOF
# Then rebuild initramfs: update-initramfs -u -k all
\`\`\``,
      },
    ],
    references: [
  ...
  // ─── SHELL ──────────────────────────────────────────────────────────────────
  {
    ...
    color: '#22c55e',
    questions: 6,
    description: 'Variable scoping, export, env vs printenv, PATH, IFS, and special variables $?, $$, $!, $@.',
    visualizations: [],
    introduction: `Understanding how the shell manages variables and the environment is fundamental to writing correct scripts. Variables in bash exist in two namespaces: shell variables (local to the current shell) and environment variables (exported to child processes).

## Shell Variables vs Environment Variables

\`\`\`bash
MESSAGE="hello"       # Shell variable — children cannot see this
export MESSAGE        # Now it is in the environment
export DB_HOST="db.prod.example.com"  # Create and export in one step
\`\`\`

## env, printenv, and set

- env — shows only exported environment variables
- printenv VAR — print a single env var
- set — shows all shell variables, functions, and values

## PATH — Command Search Order

bash searches directories in PATH left to right, stopping at the first match:

\`\`\`bash
echo $PATH
export PATH="/opt/myapp/bin:$PATH"   # prepend — takes priority
type -a ls               # Shows all matches including aliases and builtins
\`\`\`
...
- $* — All args as one word`,
    whenToUse: [
      'Debugging a script that behaves differently in cron — missing environment variables',
      'Parsing /etc/passwd or CSV data with IFS manipulation and while read',
      'Checking $? after a critical command before proceeding',
      'Writing safe scripts with set -euo pipefail and parameter expansion defaults',
    ],
    keyConcepts: [
      { term: 'export', definition: 'Marks a shell variable as an environment variable so child processes inherit it. Without export, subshells get it (they fork the parent) but exec\'d programs do not.' },
      { term: '$? — exit code', definition: 'Exit code of the last command. 0 = success; non-zero = error. Save immediately: rc=$? before any other command overwrites it.' },
      { term: '$$ — current PID', definition: 'PID of the running shell. Used for unique temp file names: TMPFILE=/tmp/myscript.$$ avoids collisions between concurrent script runs.' },
      { term: '"$@" vs "$*"', definition: '"$@" expands to separate words, one per argument — preserves spaces inside arguments. "$*" joins all into one word. Always use "$@" when forwarding arguments.' },
    ],
    pitfalls: [
      'Spaces around = in assignment: "name = value" runs "name" as a command. Must be "name=value" with no spaces.',
      '$@ and $* behave the same unquoted but differently when quoted — "$@" gives separate words, "$*" gives one string.',
      'Forgetting export before running a script — the script sees an empty string and may silently use a default.',
      'Setting IFS globally in a script without restoring it — later read and for loops split on unexpected delimiters.',
    ],
    keyQuestions: [
      {
        question: 'A script works interactively but fails when run from cron. What are the likely causes?',
        answer: `Cron runs in a minimal environment — smaller than an interactive shell.

## 1. Missing PATH

Cron's default PATH is typically /usr/bin:/bin. Fix: use absolute paths or set PATH at the top:
...
Variables like HOME, JAVA_HOME, DATABASE_URL are not set in cron. Fix:
...
Cron starts in HOME. Use absolute paths throughout.

## Debugging

\`\`\`bash
# Redirect all output to a log
* * * * * /path/to/script.sh >> /tmp/cronlog.txt 2>&1

# Capture the cron environment
* * * * * env > /tmp/cronenv.txt
\`\`\``,
      },
      {
        question: 'How do you safely use default values and detect unset variables in bash scripts?',
        answer: `\`\`\`bash
#!/bin/bash
set -euo pipefail
...
\`\`\``,
  ...
  },
  {
    ...
    color: '#22c55e',
    questions: 6,
    description: 'if/elif/else, case, for/while/until, and the critical difference between [, [[, and (( )).',
    visualizations: [],
    introduction: `Control flow in bash has multiple syntaxes accumulated over decades. Understanding the exact semantics of each test construct separates scripts that work reliably from ones that silently misbehave.

## Test Constructs: [ vs [[ vs (( ))

[ ... ] is the portable POSIX test command — requires careful quoting:

\`\`\`bash
[ "$count" -gt 0 ]    # numeric — must use -gt, not >
[ -f "$file" ]        # file test
[ "$name" = "alice" ] # string equality — must quote
\`\`\`

[[ ... ]] is a bash keyword — safer and more powerful:
- No word splitting — quoting is optional for single variables
- Supports == for pattern matching, =~ for regex
- Supports && and || inside the test

(( ... )) is for arithmetic evaluation:

\`\`\`bash
(( count > 0 ))          # integer comparison
(( count++ ))            # increment
...
\`\`\`bash
case "$1" in
  start|begin) systemctl start myapp ;;
  stop|end)    systemctl stop myapp ;;
...
# while read — process a file line by line (canonical form)
while IFS= read -r line; do
  echo "Line: $line"
done < /etc/hosts

# until — run until condition is true
until ping -c1 db.prod.example.com &>/dev/null; do
  echo "Waiting for DB..."; sleep 5
done
\`\`\``,
    whenToUse: [
      'Polling until a health check passes: until curl -sf http://localhost:8080/health; do sleep 2; done',
      'Processing each line of a log file safely with while IFS= read -r line',
      'Writing a control script for a service: case "$1" in start|stop|restart|status)',
      'Checking file existence before operating: [[ -f config.yml ]] || exit 1',
    ],
    keyConcepts: [
      { term: '[[ ]] vs [ ]', definition: '[[ ]] is a bash keyword: safer quoting, supports =~ regex, && and || inside. [ ] is POSIX test: portable to /bin/sh but requires careful quoting.' },
      { term: '(( )) arithmetic', definition: 'Evaluates arithmetic expressions. Returns 0 (true) if non-zero result. Allows C-style syntax: (( i++ )), (( a * b > 100 )).' },
      { term: 'while IFS= read -r', definition: 'The canonical way to read a file line by line. IFS= prevents trimming leading/trailing whitespace. -r prevents backslash interpretation.' },
      { term: 'Short-circuit && ||', definition: 'cmd1 && cmd2 runs cmd2 only if cmd1 succeeds. cmd1 || cmd2 runs cmd2 only if cmd1 fails.' },
    ],
    pitfalls: [
      'Using = for numeric comparison in [ ] — = is string comparison. Use -eq for numbers: [ "$count" -eq 0 ].',
      'Forgetting to quote variables in [ ] — [ $name == foo ] breaks if name is empty or contains spaces.',
      'Using && or || inside [ ] — they are not supported; use [[ ]] or separate [ ] tests.',
      'Reading a file with for line in $(cat file) — word-splits on spaces and expands globs. Use while IFS= read -r line instead.',
    ],
    keyQuestions: [
      {
        question: 'What is the difference between [ ], [[ ]], and (( ))? When do you use each?',
        answer: `## [ ] — POSIX test

Available in all POSIX shells (sh, dash, bash). Variables must be quoted:

\`\`\`bash
[ "$count" -gt 0 ]           # numeric comparison — must use -gt
[ -f "$file" ]               # file test
...
Bash keyword — prevents word splitting. More powerful:

\`\`\`bash
[[ -n $var && $var == prod* ]]   # both tests in one expression
[[ "$version" =~ ^[0-9]+\\.[0-9]+ ]]  # regex matching
...
- Portability to /bin/sh: use [ ]
- Bash scripts: use [[ ]] for strings/files, (( )) for numbers`,
      },
      {
        question: 'Write a bash loop that retries a command up to 5 times with exponential backoff.',
        answer: `\`\`\`bash
#!/usr/bin/env bash
set -euo pipefail
...
retry() {
  ...
  while (( attempt <= max_attempts )); do
    echo "[$(date +%T)] Attempt $attempt/$max_attempts..."
    if "$@"; then
...
retry curl -fsSL https://api.example.com/health
\`\`\``,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#22c55e',
    questions: 5,
    description: 'Function declaration, local variables, return codes, $FUNCNAME, and function libraries via source.',
    visualizations: [],
    introduction: `Functions are the primary mechanism for code reuse in bash. A bash function runs in the current shell context — not a subshell — which has important implications for variable scoping and side effects.
...
Variables inside functions are global by default — they affect the calling scope. Always use local:
...
## Return Codes vs Values

return N sets the function's exit code (0-255). It does NOT return a string — use echo + command substitution:
...
host="$(get_hostname)"
if is_running nginx; then echo "nginx is up"; fi
\`\`\`
...
\`\`\`bash
die() {
  echo "[\${FUNCNAME[1]:-main}] ERROR: $*" >&2
  exit 1
}
\`\`\`

## Function Libraries via source

\`\`\`bash
source /opt/scripts/lib/logging.sh
. /opt/scripts/lib/aws.sh
\`\`\``,
    whenToUse: [
      'Building deploy scripts with shared logging, error handling, and retry logic in a lib/ directory',
      'Wrapping complex commands with consistent error reporting',
      'Creating control scripts with subcommands — each is a function called by a case statement',
      'Writing recursive functions for directory traversal or tree processing',
    ],
    keyConcepts: [
      { term: 'local keyword', definition: 'Scopes a variable to the current function. Without local, bash variables are global and a function silently overwrites the caller\'s variables.' },
      { term: 'return vs exit', definition: 'return N exits only the function (sets $?). exit N exits the entire script. Never call exit from a library function — the caller cannot handle it.' },
      { term: '$FUNCNAME', definition: 'Array of the call stack of function names. ${FUNCNAME[0]} = current function. ${FUNCNAME[1]} = its caller. Use in error messages to show where failure originated.' },
      { term: 'source / .', definition: 'Load a file into the current shell: source lib.sh or . lib.sh. Functions defined in the file become available. All variables are shared — use local inside each function.' },
    ],
    pitfalls: [
      'Omitting local for variables inside functions — they silently modify the caller\'s scope, causing hard-to-trace bugs.',
      'Calling exit inside a library function — the caller cannot recover. Use return with a non-zero code instead.',
      'Using return to pass values — return only handles exit codes 0-255. Use echo + command substitution for string values.',
      'Defining functions after the first call — bash is interpreted top-to-bottom; functions must be defined before they are called.',
    ],
    keyQuestions: [
      {
        question: 'Write a bash function library with consistent logging and error handling.',
        answer: `\`\`\`bash
#!/usr/bin/env bash
# lib/common.sh
...
retry() {
  ...
  until "$@"; do
    (( attempt++ ))
    (( attempt > max )) && { log_error "All $max retries failed"; return 1; }
    log_info "Retry $((attempt-1))/$max..."
    sleep $(( attempt * 2 ))
  done
}
\`\`\``,
      ...
      {
        question: 'Why can a bash function modify the caller\'s variables, and how does local fix that?',
        answer: `## The Problem

Bash functions run in the same shell process as the caller. Unlike subshells ( ) which fork, functions share the caller's variable namespace:
...
file="important.txt"
process
echo "$file"           # Prints: temp.txt — OOPS!
\`\`\`
...
file="important.txt"
process
echo "$file"           # Prints: important.txt — caller preserved
\`\`\``,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#22c55e',
    questions: 7,
    description: 'grep, sed, awk, cut, sort, uniq, tr, wc, head/tail, and xargs for SRE data pipelines.',
    visualizations: [],
    introduction: `The Unix philosophy — small tools that do one thing well, composed via pipes — makes bash a powerful text processing environment. Mastering these tools lets you answer most data-extraction questions without writing full programs.
...
grep -l "pattern" /etc/*.conf    # Filenames only, not content
grep -n "Exception" *.log        # Prepend line numbers
grep -c "404" access.log         # Count matching lines
grep -A 3 -B 1 "FATAL" app.log   # 3 lines After, 1 Before match
\`\`\`

## sed — Stream Editor

\`\`\`bash
sed 's/ERROR/CRITICAL/g' app.log           # Global substitution
sed -i 's/localhost/db.prod/g' config.yml  # In-place edit
...
\`\`\``,
    whenToUse: [
      'Extracting the top 10 IP addresses from an nginx access log',
      'Finding ERROR lines in the last 100 lines: tail -100 app.log | grep ERROR',
      'Mass-operating on files found by find: find . -name "*.py" | xargs grep "import os"',
      'Removing Windows line endings from a config file: tr -d \'\\r\'',
    ],
    keyConcepts: [
      { term: 'grep -E (extended regex)', definition: 'Enables alternation (|), groups (()), quantifiers (+, ?). Without -E, these must be escaped. egrep is equivalent to grep -E.' },
      { term: 'sed -i in-place', definition: 'Edits the file directly. On macOS requires an extension: sed -i \'\' \'s/old/new/g\'. Use -i.bak for a backup: sed -i.bak \'s/old/new/g\' file.' },
      { term: 'awk NR and NF', definition: 'Built-in variables: NR = current record number (line number), NF = number of fields in current record. $NF = the last field.' },
      { term: 'sort | uniq -c', definition: 'The canonical frequency counting pipeline: sort brings duplicates together, uniq -c counts them. Add sort -rn to get most frequent first.' },
      { term: 'xargs -I{}', definition: 'Placeholder substitution: find . -name "*.log" | xargs -I{} cp {} /backup/{} uses {} anywhere in the command.' },
    ],
    pitfalls: [
      'Using grep without -E for alternation — grep "ERROR|WARN" matches the literal string "ERROR|WARN". Use grep -E.',
      'Running sed -i on macOS without an empty string extension — macOS sed requires sed -i \'\' (with empty string).',
      'For-loop over find output: for f in $(find...); — splits on whitespace including filenames with spaces. Use find -print0 | xargs -0.',
      'awk field numbering: $0 is the entire line, $1 is the first field. Cut uses 1-based indexing for -f as well.',
    ],
    keyQuestions: [
      {
        question: 'Show the top 10 IP addresses from an nginx access log with request counts.',
        answer: `\`\`\`bash
...
Steps: awk extracts IP (field 1), sort brings identical IPs adjacent, uniq -c counts them, sort -rn orders by highest count, head -10 takes top 10.

## Filtering to 4xx/5xx errors only

\`\`\`bash
awk '$9 ~ /^[45]/ {print $1}' /var/log/nginx/access.log \\
...
Default grep uses BRE. Special characters +, ?, |, (, ) must be escaped:

\`\`\`bash
grep 'error\\|warn' file
grep 'err.*critical' file
\`\`\`

## egrep / grep -E — Extended Regular Expressions

\`\`\`bash
grep -E 'error|warn|critical' file
grep -E '(ERROR|WARN): .+' file
\`\`\`

## fgrep / grep -F — Fixed Strings

Treats pattern as literal — no regex. Fastest for large inputs:

\`\`\`bash
grep -F 'User[0] connected' logfile
grep -F '(null)' error.log
\`\`\`

Rule: use grep -F for literal strings, grep -E for regex patterns.`,
      },
    ],
    references: [
  ...
  {
    id: 'bash-job-control',
    title: 'Job Control & Process Substitution',
    icon: 'terminal',
    color: '#22c55e',
    questions: 5,
    description: 'fg/bg/jobs, nohup, disown, Ctrl+C/Z, process substitution <(), and pipefail.',
    visualizations: [],
    introduction: `Job control lets you manage multiple processes within a single shell session — pausing, resuming, backgrounding, and detaching them.

## Signals and Job Control Keys

- Ctrl+C — sends SIGINT (signal 2) to the foreground process group. Most programs terminate.
- Ctrl+Z — sends SIGTSTP (signal 20) which suspends the foreground process. It becomes a stopped background job.

## Jobs, fg, and bg

\`\`\`bash
./long-backup.sh &         # Start in background
...
## nohup and disown — Surviving Logout

When a shell exits, it sends SIGHUP to all its job-controlled processes.
...
disown removes a running job from the job table:
\`\`\`bash
./long-process.sh &
disown %1
disown -h %1     # Keep in table but immune to SIGHUP
\`\`\`
...
grep -Ff <(cut -d: -f1 /etc/passwd) data.txt
\`\`\`
...
\`\`\``,
    whenToUse: [
      'Running a long backup over SSH that must survive SSH disconnect — nohup or tmux',
      'Comparing two sorted command outputs without temp files: diff <(cmd1|sort) <(cmd2|sort)',
      'Launching parallel tasks and waiting: launch with & then use wait',
      'Diagnosing why a pipeline appears to succeed but the first command failed — PIPESTATUS',
    ],
    keyConcepts: [
      { term: 'SIGHUP on logout', definition: 'When a shell exits, it sends SIGHUP to its process group. nohup makes the process immune; disown removes it from the job table so the shell doesn\'t send it.' },
      { term: 'nohup vs disown', definition: 'nohup: starts a new command immune to SIGHUP. disown: detaches an already-running job from the shell. Use nohup for new jobs, disown when you forgot.' },
      { term: 'Process substitution <()', definition: '<(cmd) creates a named pipe containing cmd\'s stdout. Passed to commands that expect filenames. Avoids temp files. Only available in bash/zsh.' },
      { term: 'PIPESTATUS array', definition: 'Bash array of exit codes of the last pipeline\'s commands. ${PIPESTATUS[0]} = first command, ${PIPESTATUS[1]} = second. Overwritten by the next command.' },
    ],
    pitfalls: [
      'Starting a long job over SSH without nohup — if the connection drops, SIGHUP terminates the job.',
      'Using nohup without redirecting stdout — nohup.out grows unbounded. Always: nohup cmd > /path/to/log 2>&1 &',
      'Forgetting set -o pipefail — grep "pattern" massive.log | head -1 exits 0 even if grep found nothing.',
      'Process substitution <() requires bash — it does not work in /bin/sh, dash, or POSIX sh.',
    ],
    keyQuestions: [
      {
        question: 'What is the difference between nohup and disown, and when do you use each?',
        answer: `## nohup — Immune from SIGHUP Before Starting

\`\`\`bash
nohup python3 train.py --epochs 100 > /var/log/training.log 2>&1 &
echo "PID: $!"
\`\`\`

Use when: starting a new long-running process knowing you'll log out before it finishes.
...
Use when: you forgot to use nohup and the job is already running.

## Key Difference

- nohup: the process ignores SIGHUP signal entirely
- disown: the shell won't send SIGHUP but the process could still receive it from another source

## Modern Alternative: tmux
...
\`\`\``,
  ...
  {
    id: 'linux-ip-routing',
    title: 'IP Routing & Route Tables',
    icon: 'globe',
    color: '#06b6d4',
    questions: 6,
    description: 'ip route, routing tables, default gateway, policy routing with ip rule, and ECMP for multi-path forwarding.',
    visualizations: [],
    introduction: `The Linux kernel's routing subsystem determines which interface and next-hop gateway to use for every outgoing packet. Routing issues are among the most common causes of partial network outages and asymmetric traffic.

## The iproute2 Suite

The modern tool is ip from the iproute2 package. The older route command from net-tools is deprecated:
...
The kernel performs longest-prefix match (LPM) — the most specific matching route wins. A /32 host route beats /24 beats /0 (default):
...
Linux supports multiple routing tables (numbered 1-252, main=254). The ip rule command defines routing policy:

\`\`\`bash
ip rule list
ip route show table 200
ip rule add from 10.0.0.5 table 200
\`\`\`

## ECMP

\`\`\`bash
ip route add 0.0.0.0/0 nexthop via 10.0.0.1 dev eth0 weight 1 \\
                        nexthop via 10.0.0.2 dev eth1 weight 1
\`\`\``,
    whenToUse: [
      'Debugging why traffic takes an unexpected path or fails to reach a destination',
      'Configuring multi-homed servers with two ISPs requiring per-source routing',
      'Setting up ECMP for load distribution across multiple gateways',
      'Verifying a new static route before removing the old one with ip route get',
    ],
    keyConcepts: [
      { term: 'Longest-Prefix Match (LPM)', definition: 'The kernel selects the route with the most specific subnet mask. /32 beats /24 beats /0. Use ip route get IP to see exactly which route the kernel selects.' },
      { term: 'ip route get', definition: 'A kernel-level route lookup showing which route, interface, and source IP would be used for a real packet. Test without sending traffic: ip route get 8.8.8.8.' },
      { term: 'Policy routing (ip rule)', definition: 'Linux maintains multiple routing tables. ip rule defines ordered policies — "if source IP is X, look up table 200." Enables per-source, per-mark routing.' },
      { term: 'ECMP', definition: 'Equal-Cost Multi-Path: distribute flows across multiple next-hops with the same metric. Flow hashing is 5-tuple based — the same flow always takes the same path.' },
    ],
    pitfalls: [
      'Forgetting that ip route add changes are not persistent — write routes to /etc/network/interfaces, netplan, or systemd-networkd for permanence.',
      'Ignoring routing table when troubleshooting asymmetric routing — traffic may enter on eth0 but reply goes out eth1, causing stateful firewall drops.',
      'Using metric without understanding priority — lower metric wins. Accidentally adding a more specific low-metric route silently overrides intended routing.',
    ],
    keyQuestions: [
      {
        question: 'How do you configure policy routing on a multi-homed server with two ISPs?',
        answer: `\`\`\`bash
echo "200 isp_a" >> /etc/iproute2/rt_tables
echo "201 isp_b" >> /etc/iproute2/rt_tables

ip route add default via 10.0.0.1 dev eth0 table isp_a
ip route add 10.0.0.0/24 dev eth0 src 10.0.0.5 table isp_a

ip route add default via 172.16.0.1 dev eth1 table isp_b
ip route add 172.16.0.0/24 dev eth1 src 172.16.0.5 table isp_b

ip rule add from 10.0.0.5 table isp_a priority 100
ip rule add from 172.16.0.5 table isp_b priority 101

ip rule show
ip route show table isp_a
\`\`\`

Reply packets now egress the same interface they ingressed on, preserving symmetric routing.`,
      },
      {
        question: 'How does the Linux kernel select which route to use for an outgoing packet?',
        answer: `The kernel scans the routing table and selects the entry whose destination prefix is the most specific (longest) match for the packet's destination IP.

\`\`\`bash
ip route show
ip route get 8.8.8.8
# Output: 8.8.8.8 via 10.0.0.1 dev eth0 src 10.0.0.5

ip route add 10.10.10.10/32 via 10.0.0.2

ip route get 10.10.10.10
# Now uses 10.0.0.2 instead of the /24 gateway
\`\`\`

Before consulting the routing table, the kernel evaluates policy rules (ip rule list) in ascending priority order. If no route matches, the kernel sends ICMP "network unreachable" back to the sender.`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    visualizations: [],
    introduction: `Every production connectivity issue leads to the same first step: inspect open sockets. ss (socket statistics) from iproute2 is the authoritative modern tool. It communicates directly with the kernel via netlink, making it dramatically faster than the legacy netstat which parsed /proc/net/tcp.

## Why ss Replaced netstat

On busy servers with tens of thousands of connections, netstat -an can take seconds and holds read locks on /proc. ss returns the same data in milliseconds.
...
- TIME_WAIT — connection closed, waiting 2xMSL (~60-120s) to absorb delayed packets
- CLOSE_WAIT — remote sent FIN, but local app hasn't called close() yet
- SYN_RECV — server received SYN, sent SYN-ACK, waiting for ACK

## TIME_WAIT vs CLOSE_WAIT

TIME_WAIT is normal — the kernel holds the port pair to prevent stale packets from corrupting new connections.

CLOSE_WAIT is always an application bug — the remote end closed but your code hasn't called close(). These accumulate indefinitely, consuming file descriptors until the process crashes.
...
\`\`\``,
    whenToUse: [
      'Checking which process is listening on a port before deploying a new service',
      'Diagnosing "address already in use" errors during service restarts',
      'Investigating TIME_WAIT accumulation on high-throughput HTTP servers',
      'Identifying CLOSE_WAIT socket leaks caused by application connection handling bugs',
    ],
    keyConcepts: [
      { term: 'ss -tulnp flags', definition: 't=TCP, u=UDP, l=listening only, n=numeric IPs/ports, p=show process. Drop -l to also see established connections.' },
      { term: 'TIME_WAIT (2MSL)', definition: 'Normal closure state held for 2 x Maximum Segment Lifetime (~60s). Prevents delayed packets from corrupting new connections on the same 4-tuple. High counts are normal — not a bug.' },
      { term: 'CLOSE_WAIT', definition: 'Remote sent FIN but local app hasn\'t called close(). Always an application bug — missed EOF handler or connection pool leak. Accumulates indefinitely consuming file descriptors.' },
      { term: 'Netlink vs /proc/net/tcp', definition: 'ss uses the netlink socket API to query the kernel directly — fast and O(1) per socket. netstat opens /proc/net/tcp and parses ASCII hex — O(n) with string parsing overhead.' },
    ],
    pitfalls: [
      'Running netstat on a loaded server with 50k+ connections — can take 30+ seconds and spike CPU.',
      'Treating CLOSE_WAIT as a kernel issue — always caused by application code failing to call close() after EOF.',
      'Confusing TIME_WAIT with a problem — high TIME_WAIT is normal on HTTP servers; only act if you exhaust ephemeral ports.',
      'Forgetting -n flag — without it, ss tries to resolve each IP and port via DNS, adding seconds of latency.',
    ],
    keyQuestions: [
      {
        question: 'What is the difference between TIME_WAIT and CLOSE_WAIT, and how do you diagnose each?',
        answer: `## TIME_WAIT — Normal and Intentional

Entered by the active closer after sending FIN. Held for 2xMSL (typically 60s). Purpose: absorb delayed duplicates from the old connection.
...
Remote sent FIN but local application hasn't called close(). Persists until the process is restarted.
...
Common root causes: HTTP client that doesn't drain response body before returning connection to pool; missing connection.close() call in error paths.

Remediation: CLOSE_WAIT requires fixing application code. Restarting clears them temporarily.`,
      },
    ],
    references: [
      'https://man7.org/linux/man-pages/man8/ss.8.html',
  ...
  },
  {
    ...
    color: '#06b6d4',
    questions: 5,
    description: 'tcpdump BPF filters, capture syntax, writing pcap files, and tshark for terminal analysis.',
    visualizations: [],
    introduction: `When ss, curl, and logs don't tell the full story, packet capture provides ground truth. tcpdump captures raw packets from network interfaces using the kernel's Berkeley Packet Filter (BPF) engine and can run on any Linux server without a GUI.
...
BPF runs in kernel space — packets that don't match are dropped before reaching userspace:

\`\`\`bash
tcpdump -i eth0 'host 10.0.0.1'
tcpdump -i eth0 'port 443'
tcpdump -i eth0 'tcp port 80 and not host 127.0.0.1'
...
# -C 100: rotate every 100MB, -W 10: keep only 10 files
\`\`\`

## tshark — Terminal Wireshark

\`\`\`bash
tshark -r file.pcap -T fields -e http.request.method -e http.host
tshark -r file.pcap -Y 'http.response.code == 404'
tshark -r tls.pcap -T fields -e tls.handshake.extensions_server_name
\`\`\``,
    whenToUse: [
      'Verifying that a TCP handshake completes between two services when higher-level tools report connection refused',
      'Capturing DNS queries to diagnose resolver misconfiguration or NXDOMAIN floods',
      'Debugging TLS handshake failures by confirming ClientHello/ServerHello exchange',
      'Writing a pcap file during an incident for post-mortem analysis in Wireshark',
    ],
    keyConcepts: [
      { term: 'BPF Filter Expressions', definition: 'Packet matching compiled to bytecode and run in-kernel. Primitives: host, port, tcp, udp, net, src, dst. Combine with and, or, not.' },
      { term: 'Snaplen (-s)', definition: 'Bytes captured per packet. Default in modern tcpdump is 65535 (full). Use -s 96 to capture only headers. Truncated packets show "[Packet size limited]" in Wireshark.' },
      { term: 'Rolling captures (-C, -W)', definition: '-C 100 rotates pcap file every 100MB. -W 10 keeps only 10 files (ring buffer). Combined: keeps last ~1GB of traffic — essential for capturing intermittent issues.' },
      { term: 'tshark', definition: 'Wireshark\'s command-line equivalent. Decodes protocols beyond tcpdump\'s ASCII output. Extract specific fields: tshark -r file.pcap -T fields -e dns.qry.name.' },
    ],
    pitfalls: [
      'Running tcpdump without -nn on a production host — DNS reverse lookups for every IP generate a flood of DNS queries and slow output dramatically.',
      'Forgetting -s 0 when you need full payload — truncated packets prevent Wireshark from reassembling HTTP bodies or TLS records.',
      'Using tcpdump on a bond interface — traffic may not appear; verify which member interface carries it with ip link show first.',
      'Writing to disk without a size limit — tcpdump with -w and no -C/-W can fill the filesystem and crash the server.',
    ],
    keyQuestions: [
      {
        question: 'How do you debug a hanging HTTP connection with tcpdump?',
        answer: `\`\`\`bash
tcpdump -i eth0 -nn 'host 10.0.1.50 and tcp port 80' -w /tmp/debug.pcap

curl -v http://10.0.1.50/api/health

tcpdump -r /tmp/debug.pcap -nn -v
\`\`\`
...
Interpreting:
- Only [S], no [S.]: SYN sent but no SYN-ACK — firewall dropping inbound or server not listening
- [P.] then timeout: HTTP request sent, server received but never responded — application-level issue
- [R]: connection actively refused — port not open

\`\`\`bash
tcpdump -i eth0 -nn -A 'tcp port 80 and host 10.0.1.50'
\`\`\``,
      },
    ],
    references: [
  ...
  },
  {
    ...
    visualizations: [],
    introduction: `Broken DNS is the root cause of a surprisingly large fraction of production incidents. Every SRE needs fluency with the Linux DNS query toolchain.

## The Linux Resolver Stack

1. nsswitch.conf — /etc/nsswitch.conf defines lookup order for the hosts database. Typically: files dns myhostname. The files module checks /etc/hosts first.
2. resolv.conf — /etc/resolv.conf specifies nameserver IPs, search domains, and options.
3. systemd-resolved stub — listens on 127.0.0.53:53, caches responses, and forwards to upstream resolvers.

## dig — Primary DNS Debugging Tool
...
\`\`\``,
    whenToUse: [
      'Verifying that a newly deployed DNS record propagated to authoritative nameservers',
      'Tracing the full delegation chain to diagnose NXDOMAIN or SERVFAIL with dig +trace',
      'Checking whether systemd-resolved is caching a stale record — resolvectl flush-caches',
      'Diagnosing why /etc/hosts overrides are being ignored — check nsswitch.conf host order',
    ],
    keyConcepts: [
      { term: 'dig @server domain type', definition: 'Query a specific nameserver directly, bypassing the system resolver. dig @8.8.8.8 example.com A asks Google\'s resolver. Compare authoritative vs cached answers.' },
      { term: 'dig +trace', definition: 'Performs iterative resolution from root servers, following NS referrals at each level. Shows every delegation step — invaluable for diagnosing delegation breakage or lame delegations.' },
      { term: 'ANSWER vs AUTHORITY sections', definition: 'ANSWER contains requested records. AUTHORITY lists nameservers for the zone (present on NXDOMAIN). Empty ANSWER + AUTHORITY present = NXDOMAIN or no-data response.' },
      { term: 'systemd-resolved stub', definition: 'On modern Ubuntu/Debian/RHEL, listens on 127.0.0.53:53 as a caching stub. Configure resolvers via systemd-networkd or netplan — NOT by editing /etc/resolv.conf directly.' },
      { term: 'nsswitch.conf hosts order', definition: 'If files comes before dns, /etc/hosts entries override DNS silently. This causes confusion when a hostname resolves differently on different machines.' },
    ],
    pitfalls: [
      'Editing /etc/resolv.conf directly on a systemd-resolved system — it is a symlink and will be overwritten on network changes.',
      'Relying on dig from a recursive resolver to verify propagation — it may be caching an old answer. Always dig the authoritative nameserver with @ to confirm.',
      'Ignoring nsswitch.conf files-before-dns order — /etc/hosts entries silently override DNS.',
      'Forgetting that CNAME records cannot coexist with other record types at the same name — zone apex cannot be a CNAME.',
    ],
    keyQuestions: [
      {
        question: 'How do you use dig +trace to diagnose a DNS delegation problem?',
        answer: `\`\`\`bash
dig +trace example.com A

for ns in $(dig example.com NS +short); do
  echo -n "$ns: "; dig @$ns example.com SOA +short | awk '{print $3}'
done
\`\`\`

Common problems revealed:
1. Lame delegation: parent NS points to a server that doesn't know the zone — SERVFAIL
2. Missing glue records: NS listed but no A record in ADDITIONAL section
3. Mismatched serials between primary and secondary — stale records on some nameservers`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#06b6d4',
    questions: 5,
    description: 'curl -v for HTTP debugging, custom headers, authentication, TLS inspection, and timing breakdown.',
    visualizations: [],
    introduction: `curl is the Swiss Army knife of HTTP debugging. curl -v shows the full request/response exchange including TLS negotiation, request headers, response headers, and body.
...
curl -H 'Authorization: Bearer eyJhbGc...' URL
...
\`\`\`bash
curl --cacert /path/to/ca-bundle.pem URL
curl -k URL
...
\`\`\``,
    whenToUse: [
      'Testing an API endpoint with a specific header or auth token before writing client code',
      'Measuring DNS/TCP/TLS/TTFB latency breakdown with -w timing format',
      'Verifying that a TLS certificate is valid and presents the correct CN/SAN',
      'Debugging redirect chains to confirm a URL reaches the intended destination',
    ],
    keyConcepts: [
      { term: 'curl -v (verbose)', definition: 'Prints the full exchange: TLS handshake prefixed with *, request headers with >, response headers with <, body to stdout. Use -o /dev/null to suppress body.' },
      { term: '-w timing format', definition: 'Write-out option with timing variables: time_namelookup, time_connect (TCP), time_appconnect (TLS), time_starttransfer (TTFB), time_total. Use with -o /dev/null -s.' },
      { term: '-L (follow redirects)', definition: 'curl does not follow redirects by default. Add -L to follow 3xx responses. Combined with -v you see each hop in the redirect chain.' },
      { term: '--cacert and -k', definition: '--cacert /path/ca.pem adds a custom CA for internal PKI. -k/--insecure disables all TLS verification. Never use -k in automation — it silently enables MITM attacks.' },
    ],
    pitfalls: [
      'Using -k/--insecure in scripts — it disables all TLS validation silently. Always configure a proper CA bundle instead.',
      'Forgetting -L when testing redirected URLs — curl reports 301/302 as success (exit 0), misleading you.',
      'Not accounting for HTTP/2 — add --http1.1 to force HTTP/1.1 if your proxy behaves differently per protocol.',
      'Hardcoding credentials in curl commands that end up in shell history or CI logs.',
    ],
    keyQuestions: [
      {
        question: 'How do you send a POST request with JSON, Bearer auth, and inspect the full request/response?',
        answer: `\`\`\`bash
TOKEN=$(cat /var/run/secrets/api-token)
...
- Lines starting with * — TLS/connection info
- Lines starting with > — request headers sent
- Lines starting with < — response headers received

Common gotcha: -d automatically sets Content-Type to application/x-www-form-urlencoded. Always explicitly set -H 'Content-Type: application/json' when sending JSON.`,
      },
    ],
    references: [
  ...
  // ─── PERFORMANCE ────────────────────────────────────────────────────────────
  {
    ...
    visualizations: [],
    introduction: `top is the canonical Linux process monitor — a real-time view of system health that every engineer reaches for first when a server is slow. htop is its modern replacement: color-coded, mouse-capable, and far easier to navigate.

## Load Average — The Most Misunderstood Metric

The three numbers (1/5/15 minute) represent the average number of processes in the run queue or waiting for I/O. Interpretation depends on CPU count:

- On a 1-core machine: load 1.0 = 100% utilized; load 2.0 = 200% saturated
- On a 4-core machine: load 4.0 = 100% utilized; load 8.0 = 200% saturated

Check CPU count with nproc.
...
- %ni — niced processes
- %id — idle
- %wa — iowait (CPU idle waiting for I/O)
- %hi — hardware IRQ handlers
- %si — software IRQ
- %st — steal (CPU stolen by hypervisor — cloud VMs)
...
- q — quit`,
    whenToUse: [
      'First response to a slow server — immediate picture of CPU, memory, load, and top consumers',
      'Identifying which process is consuming unexpected CPU during a production incident',
      'Checking load average trend (1/5/15 min) to see if a spike is recovering or worsening',
      'Spotting zombie processes that indicate broken process management',
    ],
    keyConcepts: [
      { term: 'Load Average vs CPU Usage', definition: 'Load average counts processes runnable or waiting for I/O — not CPU %. A server can have 5% CPU but load 8 if 8 processes are blocked on disk I/O. Always divide by CPU count from nproc.' },
      { term: '%wa (iowait)', definition: 'Percentage of time CPU was idle because at least one process was waiting for I/O. High %wa (>20-30%) indicates disk bottleneck — drill in with iostat -x 1.' },
      { term: '%st (steal time)', definition: 'On cloud VMs, CPU cycles the hypervisor gave to another VM. >5% steal = noisy neighbor or undersized instance. Sustained high steal requires instance resize or migration.' },
      { term: 'Zombie processes (Z state)', definition: 'Processes that exited but weren\'t reaped by their parent (parent hasn\'t called wait()). Consume no CPU/memory — just a PID entry. Indicate a parent process bug.' },
    ],
    pitfalls: [
      'Interpreting load average without dividing by CPU count — load 4.0 is a crisis on 1 CPU and trivial on 32 CPUs.',
      'Confusing iowait with I/O throughput — iowait is idle CPU time attributed to waiting processes, not an I/O rate.',
      'Trying to kill zombie processes directly — zombies are already dead; kill the parent process to reap them.',
      'Ignoring the 15-minute load average — it shows whether the system was already under load before the incident started.',
    ],
    keyQuestions: [
      {
        question: 'A server has load average 24 with 32 CPUs. top shows 40% iowait and 10% user CPU. What is happening?',
        answer: `## Diagnosis: I/O Bound Workload

Load 24 on 32 CPUs = 75% saturation. With 40% iowait and only 10% user CPU, the system spends most of its time waiting for I/O.

### Step 1: Find processes in D state (uninterruptible I/O wait)

\`\`\`bash
ps aux | awk '$8 == "D" {print $0}'
\`\`\`

### Step 2: Find the saturated disk device

\`\`\`bash
iostat -x 1 5
\`\`\`

### Step 3: Find which files D-state processes access

\`\`\`bash
for pid in $(ps aux | awk '$8 == "D" {print $2}' | head -5); do
  echo "=== PID $pid ==="
  ls -la /proc/$pid/fd/ 2>/dev/null | tail -5
done
\`\`\`

Common causes: runaway log writing saturating disk, NFS mount with high latency, memory pressure forcing excessive swap I/O.`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#f97316',
    questions: 5,
    description: 'vmstat fields for system-level stats, iostat -x for disk I/O deep-dive, and await vs %util.',
    visualizations: [],
    introduction: `When a performance issue is not clearly CPU or disk, vmstat and iostat provide the system-level view. These tools from the sysstat package are the backbone of command-line performance analysis.

## vmstat — Virtual Memory Statistics

vmstat N samples every N seconds. The first row is a boot average — always ignore it and watch subsequent rows.

Key columns:
- r: processes in run queue (high = CPU saturation)
- b: processes in uninterruptible sleep (high + high wa = I/O bottleneck)
- si/so: swap in/out per second (any nonzero = problem)
- bi/bo: blocks read/written from disk
...
Key columns in -x mode:
- r/s, w/s: reads/writes per second
- rMB/s, wMB/s: throughput
- await: average I/O time in ms (queue + service)
- %util: device utilization (>90% = saturated for HDDs)

## await vs %util

await is time from I/O submission to completion. %util is how busy the device is. For SSDs/NVMe, %util can be 100% while still performing well — look at await instead.`,
    whenToUse: [
      'Determining whether a slow server is CPU-bound, memory-swapping, or I/O-bound in a single glance',
      'Detecting swap I/O (si/so) which causes severe latency spikes even at low memory pressure',
      'Measuring context switch rates to diagnose lock contention or excessive thread count',
      'Identifying which disk device is saturated before escalating to the storage team',
    ],
    keyConcepts: [
      { term: 'vmstat r column (run queue)', definition: 'Processes ready to run but waiting for a CPU. Sustained r > number of CPUs = CPU saturation. Spikes are normal; chronic high r needs more CPUs or reduced workload.' },
      { term: 'vmstat si/so (swap I/O)', definition: 'si = pages swapped in from disk; so = pages swapped out. Any nonzero si/so in production is a problem — swap I/O adds milliseconds to memory accesses that normally take nanoseconds.' },
      { term: 'vmstat cs (context switches)', definition: 'Context switches per second. Moderate (1k-10k/s) is normal. Very high (>100k/s) indicates excessive threading, mutex contention, or busy-polling. Correlate with %sy.' },
      { term: 'iostat %util', definition: 'Percentage of time the device was busy. >90% = saturated for HDDs. For SSDs/NVMe, %util alone is misleading — look at await for actual latency.' },
      { term: 'iostat await', definition: 'Average I/O completion time in ms. Target: <1ms NVMe, <5ms SSD, <20ms HDD. Values above these thresholds cause visible application latency.' },
    ],
    pitfalls: [
      'Reading the first vmstat output row as real-time data — the first row is always a system-wide boot average.',
      'Using iostat without -x — basic iostat output lacks await and %util, the two most important diagnostic columns.',
      'Ignoring swap activity — a server with 20% memory free but si=100 is actively swapping and will show 100ms+ latency spikes.',
      'Treating %util 100% on NVMe as a problem — NVMe handles massive parallelism; look at await instead of %util for flash storage.',
    ],
    keyQuestions: [
      {
        question: 'How do you use vmstat and iostat together to determine if a performance problem is CPU, memory, or I/O bound?',
        answer: `\`\`\`bash
vmstat 1      # in terminal 1
iostat -x 1   # in terminal 2
\`\`\`

CPU-bound: high r, high us/sy, low wa, low iostat %util. Profile with perf top.

I/O-bound: high b, high wa, high iostat %util/await. Find top writers with iotop or pidstat -d.

Memory/swap thrashing: nonzero si/so, high wa. Check free -h, reduce vm.swappiness or add RAM.
...
\`\`\``,
  ...
  },
  {
    ...
    visualizations: [],
    introduction: `perf is the Linux kernel's built-in performance analysis framework. Unlike application-level profilers, perf works at the hardware counter and PMU level, giving visibility into CPU microarchitecture events — cache misses, branch mispredictions, instruction throughput.

## perf stat — Hardware Counter Summary

\`\`\`bash
perf stat ./myapp arg1
perf stat -p $PID -- sleep 10
\`\`\`

Key counters: cycles, instructions, IPC (instructions/cycles), cache-misses, branch-misses. IPC below 0.5 suggests heavy memory stalls.
...
Reading flame graphs: x-axis = proportion of total samples (width = CPU time). y-axis = call stack depth. Wide frame at top = that function itself is hot.`,
    whenToUse: [
      'Identifying which function consumes the most CPU before optimizing',
      'Measuring IPC and cache miss rate to determine if high CPU is compute or memory bound',
      'Generating a flame graph during a high-CPU incident to share with developers',
      'Off-CPU analysis: finding where a process spends time blocked on I/O or mutexes',
    ],
    keyConcepts: [
      { term: 'Hardware Performance Counters', definition: 'CPU registers counting microarchitecture events: cycles, instructions, cache-misses, branch-misses. IPC = instructions/cycles — healthy apps run 1-3 IPC; below 0.5 = memory stalls.' },
      { term: 'perf record -g', definition: 'Records the full call stack at each sample. Without -g, flame graphs are flat. For binaries compiled without frame pointers use --call-graph dwarf.' },
      { term: 'Flame graph width = CPU time', definition: 'Width represents CPU time proportion — wider = more time spent. Height = call stack depth. Wide base narrowing above = the function itself is hot.' },
      { term: 'Off-CPU profiling', definition: 'On-CPU profiling only captures running time. Off-CPU analysis captures why a process is NOT running — blocking on mutex, I/O, or sleep. Use perf record -e sched:sched_switch or BPF tools.' },
    ],
    pitfalls: [
      'Running perf without -g and wondering why flame graphs are flat — call graph recording is not default due to overhead.',
      'Profiling Java/Python with basic perf record — JIT frames show as [unknown]; use async-profiler for Java or py-spy for Python.',
      'Profiling at very high frequency (9999Hz) on a latency-sensitive production service — adds ~5% overhead; limit duration.',
    ],
    keyQuestions: [
      {
        question: 'Walk through the full workflow of generating a CPU flame graph for a running process.',
        answer: `\`\`\`bash
apt-get install -y linux-perf linux-tools-common
...
# For C++ compiled without frame pointers:
...
\`\`\``,
  ...
  },
  {
    ...
    color: '#f97316',
    questions: 5,
    description: 'Tracing system calls and library calls to debug mysterious application behavior without source code.',
    visualizations: [],
    introduction: `When you need to know exactly what a process is doing at the OS boundary, strace is your tool. It intercepts every system call — file opens, reads, writes, network connections, memory mappings — with arguments and return values. No source code access required.
...
strace -c ./myapp
strace -T -p $PID
strace -o /tmp/trace.out -p $PID
\`\`\`

## syscall Categories (-e trace=)

- file: open, read, write, stat, unlink, rename, chmod
...
strace -p $PID -T 2>&1 | awk '$NF > 0.01 {print}'
\`\`\`

## ltrace — Library Call Tracing

ltrace traces dynamic library function calls (malloc, fopen, SSL_connect, pthread_mutex_lock).`,
    whenToUse: [
      'Diagnosing why a process is slow without source code — identify blocking I/O via syscall patterns',
      'Finding which files a binary reads at startup to understand config dependencies',
      'Debugging ENOENT errors where a service claims a file does not exist — strace shows the exact path',
      'Confirming that a process is not unexpectedly making network connections',
    ],
    keyConcepts: [
      { term: 'strace -p (attach)', definition: 'Attaches to a running process without restarting it. Detach with Ctrl+C — the process continues. Use -p PID1 -p PID2 to trace multiple simultaneously.' },
      { term: '-c (count and summarize)', definition: 'Prints a summary table: syscall name, call count, total time, average time. Best first pass — identifies top consumers before diving into individual calls.' },
      { term: '-T (per-syscall timing)', definition: 'Appends wall-clock time to each syscall: read(5, ...) = 100 <0.087>. A read() taking 87ms means blocking I/O. Combine with -e trace=file to see only file calls with timing.' },
      { term: 'ENOENT / EACCES / EAGAIN', definition: 'ENOENT = file not found (exact path shown); EACCES = permission denied; EAGAIN = try again (non-blocking I/O not ready). EAGAIN in a tight loop = busy-polling causing high CPU.' },
    ],
    pitfalls: [
      'Running strace without -T on a performance problem — you see every syscall but cannot tell which ones are slow.',
      'Using strace -f on a high-throughput multithreaded service without -c first — adds 2-10x overhead multiplied by thread count.',
      'Missing child process syscalls without -f flag — strace only traces the specified process without following forks.',
      'Confusing ltrace and strace — ltrace shows library function calls (malloc, fopen); strace shows kernel syscalls (mmap, openat).',
    ],
    keyQuestions: [
      {
        question: 'How do you use strace to find why a service is slow without source code?',
        answer: `\`\`\`bash
strace -c -p $PID -f -- sleep 10 2>&1

strace -p $PID -e trace=read,write -T 2>&1 | awk '$NF > 0.01 {print}'

strace -p $PID -T -o /tmp/strace.out -e trace=file &
sleep 30
kill %1
grep 'E[A-Z]' /tmp/strace.out | sort | uniq -c | sort -rn
\`\`\`

Common patterns:

\`\`\`bash
grep 'stat.*ENOENT' /tmp/strace.out | sort | uniq -c | sort -rn

strace -p $PID -e trace=network -T 2>&1 | grep 'ECONNREFUSED'

strace -p $PID -e trace=nanosleep,select,poll,epoll_wait -T 2>&1
\`\`\``,
      },
    ],
    references: [
  ...
  },
  {
    ...
    color: '#f97316',
    questions: 5,
    description: 'lsof for open files/sockets, network connections, file descriptor leaks, and recovering deleted files.',
    visualizations: [],
    introduction: `On Unix, everything is a file — regular files, directories, sockets, pipes, device nodes. lsof (List Open Files) shows every open file descriptor across all processes on the system.
...
- 3r, 4w, 5u — numbered file descriptors with access mode
...
\`\`\``,
    whenToUse: [
      'Finding which process has a file open before unmounting — avoids "device is busy" errors',
      'Recovering data from a deleted file still held open via /proc/PID/fd',
      'Diagnosing file descriptor leaks by watching FD count grow over time',
      'Finding which process is listening on port 8080 without ss or netstat',
    ],
    keyConcepts: [
      { term: 'lsof -i (network)', definition: 'Lists all processes with open internet sockets. Filter: lsof -i :80, lsof -i TCP:22, lsof -i @10.0.0.1. Shows process name, PID, user, socket state.' },
      { term: 'lsof +D (directory)', definition: 'Finds all processes with any file open under that directory (recursive). Essential before umount — "device is busy" means lsof +D /mountpoint shows the blocking process.' },
      { term: '/proc/PID/fd recovery', definition: 'When a file is deleted but still open, access data via /proc/PID/fd/N. The FD number comes from lsof -p PID — look for the DEL type entry.' },
      { term: 'lsof -p (per-process)', definition: 'Shows all file descriptors for a process. Count with lsof -p PID | wc -l. System default FD limit: ulimit -n (default 1024). Approaching limit = EMFILE errors.' },
    ],
    pitfalls: [
      'Running lsof without -n on a host with many connections — DNS resolution for each network socket can take minutes.',
      'Stopping a production service to free disk space from a deleted file — recover via /proc/PID/fd instead.',
      'lsof gives a point-in-time snapshot — rapidly-opening processes may not show all FDs in a single run.',
    ],
    keyQuestions: [
      {
        question: 'How do you diagnose and fix a "too many open files" EMFILE error?',
        answer: `\`\`\`bash
journalctl -u myservice | grep -i 'too many open'
...
\`\`\``,
      ...
      {
        question: 'How do you recover a deleted log file that is still being written?',
        answer: `\`\`\`bash
lsof | grep '(deleted)'
# nginx  1234  www  5w  REG  deleted  /var/log/nginx/access.log (deleted)
...
This is common with incorrect log rotation — the old file is deleted but the FD keeps growing the inode indefinitely.`,
      },
    ],
    references: [
  ...
  },
  {
    ...
    visualizations: [],
    introduction: `Linux uses the Completely Fair Scheduler (CFS) as its default algorithm since kernel 2.6.23. CFS aims to give every runnable process a fair share of CPU time, weighted by priority.

## The Completely Fair Scheduler (CFS)

CFS tracks each process's virtual runtime (vruntime) — CPU time consumed, weighted by priority. The process with the lowest vruntime runs next.
...
\`\`\`bash
ionice -c 3 -p $PID          # class 3 = idle
ionice -c 2 -n 0 -p $PID    # class 2 best-effort, highest priority
\`\`\`

Classes: 1=realtime, 2=best-effort (default), 3=idle.
...
chrt -r 20 ./realtime-app
\`\`\``,
    whenToUse: [
      'Preventing background batch jobs from starving foreground services by nicing them to +15 or +19',
      'Pinning a latency-critical service to dedicated CPU cores with taskset',
      'Binding a database server to NUMA node 0 with numactl for local DRAM access',
      'Setting ionice -c 3 for background backup jobs that should never contend with production I/O',
    ],
    keyConcepts: [
      { term: 'CFS vruntime fairness', definition: 'CFS tracks vruntime — CPU time weighted by priority. The process with the lowest vruntime runs next. Nice -20 accumulates vruntime ~3x slower than nice 0, so it gets scheduled 3x more often.' },
      { term: 'nice range -20 to +19', definition: '-20 = highest priority (needs root). +19 = lowest priority. When competing for CPU, nice +19 gets ~1.5% CPU vs nice 0\'s ~98.5%.' },
      { term: 'ionice classes', definition: '1=realtime (guaranteed I/O bandwidth). 2=best-effort (default, priority 0-7). 3=idle (I/O only when disk is idle — perfect for background backups).' },
      { term: 'taskset CPU affinity', definition: 'Pins a process to specific CPUs via scheduling affinity. Reduces L1/L2 cache misses from CPU migrations. taskset -c 0,1 command binds to CPUs 0 and 1.' },
      { term: 'NUMA topology', definition: 'On multi-socket servers, memory access time depends on whether RAM is local or remote to the CPU socket. Remote NUMA access adds 30-60% latency. numactl --membind=0 ensures local DRAM allocation.' },
    ],
    pitfalls: [
      'Relying on nice values when the system is not CPU-saturated — nice only affects scheduling when processes compete. On an idle system, nice +19 runs at full speed.',
      'Using SCHED_FIFO without a watchdog — a FIFO process in an infinite loop preempts all other processes including the shell, requiring a hardware reset.',
      'Pinning a multithreaded process to a single CPU — all threads share that CPU, creating worse contention than scheduler migration.',
      'Ignoring NUMA on multi-socket servers — a process allocated on node 1 but running on node 0 CPUs experiences 30-60% higher memory latency for every cache miss.',
    ],
    keyQuestions: [
      {
        ...
        answer: `\`\`\`bash
BATCH_PID=$(pgrep -f batch-etl)
renice -n 19 -p $BATCH_PID
ionice -c 3 -p $BATCH_PID

SERVICE_PID=$(pgrep -f web-service)
taskset -c 0-7 -p $SERVICE_PID

taskset -c 8-15 -p $BATCH_PID

numactl --cpunodebind=0 --membind=0 ./web-service &
numactl --cpunodebind=1 --membind=1 ./batch-etl &

ps -eo pid,ni,psr,comm | grep -E 'web-service|batch-etl'
taskset -p $SERVICE_PID
taskset -p $BATCH_PID
\`\`\``,
      },
    ],
