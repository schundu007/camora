...
The correct approach is to set timeouts at or slightly above the p99 latency of the healthy dependency — often 200–500ms for internal calls. Anything beyond that budget should either fail fast (circuit breaker) or be pushed to an async queue. A 30-second timeout is appropriate only for human-initiated, low-frequency operations like file uploads.`,
    ...
    quickFire: [
      { q: 'Root cause of the cascade?', a: 'A single slow payment processor combined with 30-second synchronous timeouts -- threads backed up faster than they returned, exhausting every service\'s thread pool.' },
      { q: 'What detection would have caught this earlier?', a: 'Thread pool saturation metrics and per-dependency latency p99 alerts -- a rising p99 on the payment processor call would have triggered before thread pools exhausted.' },
      { q: 'Key mitigation applied?', a: 'Per-dependency bulkhead thread pools so one slow dependency cannot starve all others, plus circuit breakers to fast-fail calls once the dependency degrades.' },
      { q: 'What runbook item was missing?', a: 'A step to open circuit breakers manually and shed load when a downstream dependency is degraded -- the team had no playbook for partial dependency failure.' },
      { q: 'Why is 30-second timeout dangerous in a synchronous service call?', a: 'Each waiting thread holds a socket and pool slot for the full duration. A 60-second degradation causes hundreds of threads to pile up, exhausting the pool before recovery.' },
      { q: 'What is the bulkhead pattern?', a: 'Isolating different downstream calls into separate thread pools so a slow dependency can only exhaust its own pool, not the entire process.' },
      { q: 'How does a retry storm amplify a cascading failure?', a: 'Clients receiving errors immediately retry, multiplying load on an already-degraded service and preventing it from recovering.' },
      { q: 'What is correct timeout sizing for internal service calls?', a: 'Set at or slightly above the p99 latency of the healthy dependency -- typically 200-500ms for internal calls. 30 seconds is only appropriate for user-initiated file uploads.' },
    ],
    references: [
...
Third, add a circuit breaker and local cache: if the introspection endpoint is slow, serve from the last-known-valid cached result for a configurable grace period (30–60 seconds) rather than blocking the request.`,
    ...
    quickFire: [
      { q: 'Root cause of the global outage?', a: 'A single-region token introspection endpoint in us-east-1 was called inline on every authenticated request in all four regions -- a hidden single-region hard dependency.' },
      { q: 'What detection would have caught this earlier?', a: 'Dependency mapping of all outbound network calls per service tagged by region. Any single-region tag on an inline request path is a documented risk.' },
      { q: 'Key mitigation applied?', a: 'Switch from online token introspection to offline JWT validation using cached public keys -- each region validates tokens locally with no cross-region network call.' },
      { q: 'What runbook item was missing?', a: 'A dependency inventory that maps each service\'s inline request-path calls to their AWS region. The 23 hidden us-east-1 dependencies were unknown until the postmortem.' },
      { q: 'What is the difference between data plane and control plane?', a: 'Data plane handles actual user requests (serving). Control plane handles management (auth, config, orchestration). Inline control-plane calls in the data plane make outages worse.' },
      { q: 'What is token caching and why does it help?', a: 'Storing validated JWT results in a short-lived local cache so auth introspection calls are not made per-request -- one stale result window is acceptable to avoid per-request network calls.' },
      { q: 'How do you audit existing systems for hidden single-region dependencies?', a: 'Query CloudTrail for API calls where caller region differs from resource region, plus VPC flow logs for cross-region traffic patterns.' },
    ],
    references: [
...
The vendor acknowledged the change in their changelog — an entry added three days earlier under "improvement." No breaking-change notice was sent. The API contract had not formally changed (it still returned HTTP 200 with a country code), so the integration test passed.`,
    ...
    quickFire: [
      { q: 'Root cause of the incident?', a: 'A third-party geolocation API silently changed its fallback behavior -- returning "US" for all unresolvable codes instead of an error, breaking international pricing without any HTTP error signals.' },
      { q: 'What detection would have caught this earlier?', a: 'Value distribution monitoring -- instrumenting the integration to emit a counter per unique return value. "US" rising from 15% to 100% of responses is an anomaly that should alert.' },
      { q: 'Key mitigation applied?', a: 'Adding canary requests that periodically send known test inputs (ZIP codes from multiple countries) and assert expected outputs -- alerts immediately on any behavioral change.' },
      { q: 'What runbook item was missing?', a: 'A third-party API behavioral monitoring checklist -- any integration relying on semantic correctness (not just HTTP 200) needs value-distribution or canary monitoring.' },
      { q: 'Why did integration tests pass despite the broken behavior?', a: 'The API still returned HTTP 200 with a valid country code schema. Contract tests only validated status code and schema, not the semantic correctness of the returned value.' },
      { q: 'What is consumer-driven contract testing?', a: 'The consumer defines what it expects from a provider (including specific values), and the provider runs those expectations in its test suite -- catches silent behavioral changes.' },
    ],
    references: [
      'https://docs.pact.io/',
    ],
...
The RDS primary received 47x its normal query rate within 90 seconds. Read replicas, which normally handled 80% of traffic, were not warmed either — connection pools to replicas filled and began queuing. By 3:17 AM, p99 latency on the main API had risen from 45ms to 8 seconds. By 3:19 AM, the RDS instance hit its max_connections limit and began refusing new connections. The site effectively went down for 22 minutes during what was supposed to be a zero-downtime maintenance window.`,
    ...
    quickFire: [
      { q: 'Root cause of the thundering herd?', a: 'Replacing the entire ElastiCache cluster at once caused a simultaneous cold cache across all application nodes, sending 47x normal query rate to RDS within 90 seconds.' },
      { q: 'What detection would have caught this earlier?', a: 'Cache hit ratio and RDS connection count alerts -- a drop from 94% to 0% hit ratio is an instant anomaly signal that would trigger before RDS saturates.' },
      { q: 'Key mitigation applied?', a: 'Blue-green cache replacement -- provision and warm a new cluster while the old one serves traffic, then switch the endpoint atomically once the new cluster is warm.' },
      { q: 'What runbook item was missing?', a: 'A cache warming procedure in the maintenance runbook -- the team had no documented process for pre-populating cache before directing traffic to a new cluster.' },
      { q: 'What is request coalescing?', a: 'Deduplicating concurrent cache-miss requests for the same key so only one request reaches the database -- all waiters receive the same result, bounding DB load to unique key rate.' },
      { q: 'Why can low-traffic windows paradoxically expose failure modes?', a: 'Low traffic means less organic cache warming before the maintenance window. The same cold cache at 3 AM still saturates the database because cache hit rate was high even at low volume.' },
      { q: 'What is rolling node replacement for ElastiCache?', a: 'Replacing one node at a time -- consistent hashing means only 1/N of the keyspace goes cold per step instead of 100%, limiting the blast radius of each node replacement.' },
    ],
    references: [
...
The database held for 4 minutes before the most popular queries repopulated the cache and the stampede subsided. Less popular pages took 18 minutes. Total impact: 11 minutes of elevated error rates during the peak of the flash sale.`,
    ...
    quickFire: [
      { q: 'Root cause of the cache stampede?', a: 'All product cache keys were set with an identical TTL and cached at the same time -- they all expired simultaneously, sending 4,000 concurrent queries to PostgreSQL in 500ms.' },
      { q: 'What detection would have caught this earlier?', a: 'Cache hit ratio monitoring -- a sudden drop from high hit rate to near-zero is an instant signal that a stampede or cold cache event is occurring.' },
      { q: 'Key mitigation applied?', a: 'TTL jitter -- adding a random offset to each key\'s TTL so keys expire across a window rather than at a single point in time.' },
      { q: 'What runbook item was missing?', a: 'A pre-event cache warming procedure -- before the flash sale, explicitly warm all affected product keys with a batch job rather than relying on organic cache population.' },
      { q: 'What is stale-while-revalidate?', a: 'Serving the expired cached value while asynchronously refreshing the cache in the background -- eliminates synchronous miss latency and prevents stampede entirely.' },
      { q: 'What is a cache lock pattern?', a: 'The first miss for a key acquires a distributed lock (Redis SET NX). Lock holders query the DB and write the cache; non-holders serve stale content or wait.' },
    ],
    references: [
...
For critical tables, the combination of PITR + application-level soft delete provides both near-zero RPO and fast recovery (a single UPDATE to un-delete soft-deleted rows).`,
    ...
    quickFire: [
      { q: 'Root cause of the data loss?', a: 'A cleanup script pointed to the production database via a misconfigured environment variable fallback, executing DELETE without a WHERE clause on 2.3 million rows in 4 seconds.' },
      { q: 'What detection would have caught this earlier?', a: 'A row count drop alert on the user_preferences table -- losing 2.3 million rows in 4 seconds is a dramatic anomaly that Datadog or CloudWatch can detect.' },
      { q: 'Key mitigation applied?', a: 'Point-in-time recovery (PITR) combined with binary log replay to recover to just before the DELETE event -- restored to within minutes of the incident.' },
      { q: 'What runbook item was missing?', a: 'A DRY_RUN mode requirement for all destructive scripts, plus a row count confirmation prompt before executing large DELETEs.' },
      { q: 'Difference between RPO and RTO?', a: 'RPO is the maximum acceptable data loss (how old the recovery point can be). RTO is the maximum acceptable downtime (how long recovery takes). PITR improves RPO; replicas improve RTO.' },
      { q: 'How does soft delete protect against accidental deletions?', a: 'Marks rows as deleted with a flag and timestamp instead of issuing DELETE. Recovery is a single UPDATE to clear the flag -- no backup restore required.' },
      { q: 'What is point-in-time recovery in RDS?', a: 'Restores a database to any second within the backup retention window using automated snapshots plus transaction log replay. Creates a new DB instance -- not an in-place restore.' },
      { q: 'What safety controls prevent accidental prod database access?', a: 'Separate IAM roles and credentials per environment, explicit DSN validation in scripts with environment confirmation prompts, and read-only default credentials for non-prod workflows.' },
    ],
    references: [
...
The affected rows totaled $847,000 in under-recorded revenue. Recovery required replaying original transaction events from the event store, which the team was fortunate to have. Without the event store, the corruption would have been unrecoverable.`,
    ...
    quickFire: [
      { q: 'Root cause of the corruption?', a: 'Integer division in Python (amount / 100) truncated monetary values instead of rounding -- $19.999... became $19.99 instead of $20.00 across 14 million rows.' },
      { q: 'What detection would have caught this earlier?', a: 'A post-migration sum comparison -- comparing the total of the new column against the expected total from the original column would immediately show a discrepancy.' },
      { q: 'Key mitigation applied?', a: 'Replaying original transaction events from the event store to reconstruct correct values -- without event sourcing, the corruption would have been unrecoverable.' },
      { q: 'What runbook item was missing?', a: 'A mandatory post-migration data quality check: compare sum/min/max/average of the transformed column against the source, and sample 1,000 rows for manual verification.' },
      { q: 'Why should financial amounts never use floating point?', a: 'Binary floating point cannot represent many decimal fractions exactly (0.1 is a repeating binary fraction). Store amounts as integers (cents) to avoid rounding errors entirely.' },
      { q: 'What is a migration dry run with comparison?', a: 'Running the migration against a production replica and comparing before/after value distributions (sum, min, max, sample rows) before applying to production.' },
      { q: 'What is event sourcing and how did it enable recovery?', a: 'Every state change is stored as an immutable event. Current state is rebuilt by replaying events -- providing a recovery path to reconstruct correct values even after table-level corruption.' },
    ],
    references: [
...
For 4 minutes, order status pages showed orders as "pending" that had already shipped. More critically, a batch job that read order states from the replica and triggered fulfillment re-triggered 847 orders that had already been fulfilled.`,
    ...
    quickFire: [
      { q: 'Root cause of the data loss?', a: 'Automatic RDS failover promoted the correct Multi-AZ standby, but the application\'s read connections pointed to an async analytics replica that was 4 minutes behind the primary.' },
      { q: 'What detection would have caught this earlier?', a: 'Replication lag monitoring with a paging alert -- an alert when replica lag exceeds 30 seconds would have flagged the analytics replica before it was trusted for business-critical reads.' },
      { q: 'Key mitigation applied?', a: 'Separating read endpoints by staleness tolerance -- business-critical reads route to the primary; only analytics queries with known lag tolerance use the async replica.' },
      { q: 'What runbook item was missing?', a: 'A failover checklist verifying that all application connection strings point to the correct post-failover endpoint, not stale replica aliases.' },
      { q: 'Difference between RDS Multi-AZ standby and a read replica?', a: 'Multi-AZ uses synchronous replication, is promoted automatically on failure, and has no lag. Read replicas use async replication with potential lag and are not promoted automatically.' },
      { q: 'Why should batch jobs always read from the primary?', a: 'Business-critical idempotency requires fresh data. Reading from a lagging replica can cause double-processing of records that appear un-processed but were already handled.' },
      { q: 'How does Aurora handle replica lag for reader endpoints?', a: 'Aurora\'s reader endpoint automatically routes reads to the primary when replica lag exceeds a configurable threshold -- preventing stale reads without manual application logic.' },
    ],
    references: [
...
The team had never run a restore drill. There was no monitoring on the backup bucket's object count or size. The only backup that existed was a weekly RDS automated snapshot — 6 days old at the time of the disk failure — which meant 6 days of data was unrecoverable.`,
    ...
    quickFire: [
      { q: 'Root cause of the backup failure?', a: 'A config change 14 months prior pointed the backup output path to a deleted S3 bucket. The script treated any non-exception exit as success and posted green notifications regardless.' },
      { q: 'What detection would have caught this earlier?', a: 'A dead man\'s switch alert -- fire if no new backup object appears in the S3 bucket within 1.5x the backup interval. This catches silent job failures regardless of exit code.' },
      { q: 'Key mitigation applied?', a: 'After emergency recovery from the 6-day-old RDS snapshot, the team implemented object validation: the backup script now verifies the written object size, checksum, and readability before posting success.' },
      { q: 'What runbook item was missing?', a: 'A scheduled restore drill -- monthly restoration of the most recent backup to a test environment. The team had never verified a restore worked in 14 months of "successful" backups.' },
      { q: 'What is the difference between backup creation success and backup validity?', a: 'A backup job can log success (process ran without exception) while writing zero bytes. Validity requires confirming the output exists, has correct size, and is actually restorable.' },
      { q: 'How should backup monitoring be layered?', a: 'Three layers: dead man\'s switch (alert if no new object in 1.5x interval), object validation (size threshold + checksum after each write), and periodic restore drills.' },
    ],
    references: [
...
Prevention: Remove the file from git history using git filter-branch or BFG Repo Cleaner (note: for public repos, assume the credentials are permanently exposed even after history rewrite — scanners index commits quickly). Replace with IAM roles and AWS Secrets Manager. Add pre-commit hooks (detect-secrets) to the repo.`,
    ...
    quickFire: [
      { q: 'Root cause of the incident?', a: 'A developer committed a real AWS access key in a config file to a public GitHub repository. Automated scanners found it within 4 minutes and began mining cryptocurrency.' },
      { q: 'What detection would have caught this earlier?', a: 'A pre-commit hook using git-secrets or detect-secrets that blocks commits containing credential patterns before they ever reach the remote repository.' },
      { q: 'Key mitigation applied?', a: 'Immediate key revocation via aws iam delete-access-key, then CloudTrail audit of all actions taken with the key, followed by terminating all unauthorized resources.' },
      { q: 'What runbook item was missing?', a: 'A credential incident response playbook covering: revoke key, audit CloudTrail, check for new IAM users, scan all regions for unauthorized resources, file AWS Security Incident Report.' },
      { q: 'What should replace long-lived IAM access keys in CI?', a: 'OIDC-based short-lived credentials -- GitHub Actions OIDC issues temporary AWS credentials scoped to the specific job, automatically revoked when the job ends.' },
      { q: 'Does deleting the file from git history contain the exposure?', a: 'No -- for public repos, assume credentials are permanently compromised. Automated scanners index commits within minutes. Revoke immediately regardless of history rewrite.' },
      { q: 'What should developers use instead of access keys for local AWS access?', a: 'IAM Identity Center (SSO) with short-lived session credentials, or EC2 instance profiles and ECS task roles that use temporary STS credentials automatically.' },
    ],
    references: [
...
The exposure was discovered during an automated compliance audit 8 months later. By then, the bucket had 847 GB of log data. AWS Macie scan of the bucket found 3.2 million distinct email addresses and 180,000 instances of what it classified as credit card-like patterns.`,
    ...
    quickFire: [
      { q: 'Root cause of the exposure?', a: 'A logging bucket created with Block Public Access unchecked during testing was never locked down. Application logs containing PII were publicly accessible for 8 months.' },
      { q: 'What detection would have caught this earlier?', a: 'AWS Config rule s3-bucket-public-read-prohibited with auto-remediation -- would have flagged or auto-fixed the bucket within minutes of creation.' },
      { q: 'Key mitigation applied?', a: 'Enabling S3 Block Public Access at the AWS account level -- this overrides any bucket-level setting and prevents any bucket from being made public regardless of individual configuration.' },
      { q: 'What runbook item was missing?', a: 'A post-creation bucket compliance check in the IaC pipeline and a requirement that all new buckets declare explicit BlockPublicAcls/BlockPublicPolicy settings.' },
      { q: 'Why are logging buckets higher risk than they appear?', a: 'Application logs often contain full request payloads, which include PII, credentials in query strings, and session tokens. Logging buckets frequently hold more sensitive data than application buckets.' },
      { q: 'What is AWS Macie?', a: 'A managed service that uses ML to automatically discover PII, credentials, and financial data in S3 buckets. Should run continuously on all buckets containing log data.' },
      { q: 'What is data minimization in logging?', a: 'Logging only fields necessary for debugging -- masking or excluding PII (emails, names, payment data) from logs by default so log storage cannot become a PII exposure vector.' },
    ],
    references: [
...
The finding required revoking iam:PassRole from the developer's policy and implementing a permission boundary on all roles that could be passed to Lambda — restricting what the Lambda could actually do even if passed an admin role.`,
    ...
    quickFire: [
      { q: 'Root cause of the privilege escalation risk?', a: 'A developer had iam:PassRole + lambda:CreateFunction + lambda:InvokeFunction -- three individually reasonable permissions that together enable creating a Lambda with an admin role to issue new IAM credentials.' },
      { q: 'What detection would have caught this earlier?', a: 'Automated IAM escalation scanning (Cloudsplaining, PMapper) that maps permission combinations to escalation paths -- run in CI when IAM policies change.' },
      { q: 'Key mitigation applied?', a: 'Permission boundaries on all Lambda execution roles -- even if a Lambda is passed an admin role, the boundary limits effective permissions to a safe maximum.' },
      { q: 'What runbook item was missing?', a: 'An IAM policy review process that evaluates permission combinations, not just individual permissions in isolation -- escalation paths arise from combinations.' },
      { q: 'What is iam:PassRole and why is it dangerous?', a: 'Allows attaching an IAM role to a service. Combined with resource creation (Lambda, EC2, ECS), it lets the user launch resources with higher permissions than they personally hold.' },
      { q: 'What is a permission boundary?', a: 'An IAM policy that sets the maximum permissions a role can ever have -- even if a broader policy is attached, the boundary limits effective permissions.' },
      { q: 'How do you restrict which roles can be passed with iam:PassRole?', a: 'Add a Condition block: StringEquals iam:PassedToService: lambda.amazonaws.com plus a resource ARN condition restricting the specific roles that can be passed.' },
    ],
    references: [
...
The CI keys had S3 read/write and CloudFront invalidation permissions. The attacker used them to replace the CDN-served JavaScript bundle with a malicious version over a 6-hour window before the keys were rotated.`,
    ...
    quickFire: [
      { q: 'Root cause of the supply chain attack?', a: 'A typosquatted npm package (lodash-utils vs lodash) with a postinstall script that read process.env and exfiltrated all environment variables including CI AWS credentials.' },
      { q: 'What detection would have caught this earlier?', a: 'npm audit and Socket.dev in CI flagging packages with postinstall scripts or newly published packages with suspicious behaviors before installation.' },
      { q: 'Key mitigation applied?', a: 'Replacing long-lived CI access keys with GitHub Actions OIDC -- temporary credentials scoped to the specific job, automatically revoked when the job ends.' },
      { q: 'What runbook item was missing?', a: 'A package review checklist for new dependencies: verify publisher, check publication date, audit postinstall scripts, confirm package name matches the intended library exactly.' },
      { q: 'What does npm ci --ignore-scripts do?', a: 'Installs exact versions from the lock file without running any lifecycle scripts (postinstall, preinstall). Prevents malicious postinstall code from executing in CI.' },
      { q: 'What is a lock file and why is npm ci better than npm install in CI?', a: 'Lock files pin exact versions and checksums. npm ci installs from the lock file and fails if package.json and lock file are inconsistent -- prevents unexpected package substitution.' },
      { q: 'How do you scope CI credentials to minimum permissions?', a: 'Create a dedicated IAM role per CI job with only the permissions that job needs (e.g., CloudFront invalidation only) -- a compromised token for one job cannot access unrelated resources.' },
    ],
    references: [
...
Monitor with a CloudWatch/Datadog metric for process.memoryUsage().heapUsed and alert when it exceeds 80% of available memory, giving time to investigate before OOM.`,
    ...
    quickFire: [
      { q: 'Root cause of the memory leak?', a: 'Each incoming HTTP request added a close event listener to a shared Redis client without ever removing it -- after 10,000 requests, 10,000 listeners each held a closure reference to a request object.' },
      { q: 'What detection would have caught this earlier?', a: 'Monitoring process.memoryUsage().heapUsed as a Datadog/CloudWatch metric and alerting when it exceeds 80% of available memory -- heap growth precedes OOM by hours.' },
      { q: 'Key mitigation applied?', a: 'A single removeListener call in the response end handler, releasing the closure reference so the GC could reclaim the request objects.' },
      { q: 'What runbook item was missing?', a: 'Heap snapshot comparison in the performance runbook -- a procedure for taking two snapshots 30 minutes apart and comparing them in Chrome DevTools to identify growing objects.' },
      { q: 'What is event listener accumulation?', a: 'Adding listeners to emitters inside request handlers without matching removeListener calls. Node.js emits a MaxListeners warning after 10 listeners on one event -- a useful early signal.' },
      { q: 'Why did the team accept nightly restarts as normal?', a: 'The crash happened at 3 AM at low traffic with only 30 seconds of downtime. Once scaled to 8 instances with staggered starts, crashes spread across the day and became visible.' },
      { q: 'What are three common Node.js memory leak patterns?', a: 'Event listener accumulation, global Maps/Sets without size bounds or TTL, and setInterval callbacks capturing large objects in closures without clearInterval.' },
    ],
    references: [
...
Monitor: add a per-request query count metric to APM. Alert if any endpoint averages more than 20 queries per request in production.`,
    ...
    quickFire: [
      { q: 'Root cause of the N+1 collapse?', a: 'ORM lazy loading generated one query per order for product and customer associations. A user with 840 orders triggered 1,681 queries per request, exhausting the 20-connection pool with 3 concurrent power users.' },
      { q: 'What detection would have caught this earlier?', a: 'Per-request query count in APM -- a list endpoint issuing 100+ queries per request is an immediate N+1 flag visible in Datadog trace details.' },
      { q: 'Key mitigation applied?', a: 'Switching ORM lazy loading to eager loading with JOIN for the endpoint, reducing 1,681 queries per request to 2.' },
      { q: 'What runbook item was missing?', a: 'SQL query count logging in development -- a developer mode that counts queries per request and warns when a single request issues more than 10 queries.' },
      { q: 'Why did staging never catch this?', a: 'Test data had at most 20 orders per user. The N+1 problem only becomes catastrophic at production scale (hundreds to thousands of orders per power user).' },
      { q: 'What is the DataLoader pattern?', a: 'Collect all IDs needed across a batch of items, then issue one batched query (WHERE id IN (...)) to fetch them all. Reduces N queries to 1 without requiring ORM eager loading.' },
      { q: 'What metric signals N+1 without query-level tracing?', a: 'p99 and p99.9 latency -- N+1 problems only affect users with large datasets. Average latency looks fine while tail latency is extremely slow.' },
    ],
    references: [
...
For index creation: always use CREATE INDEX CONCURRENTLY — it builds the index without a table lock, taking slightly longer but not blocking reads or writes.`,
    ...
    quickFire: [
      { q: 'Root cause of the connection pool exhaustion?', a: 'A blocking ALTER TABLE on a 300-million-row table held a table-level lock for 18 minutes. All 12 services queuing on that lock exhausted their pools in 4 minutes.' },
      { q: 'What detection would have caught this earlier?', a: 'pg_stat_activity monitoring for queries waiting on Lock events -- a growing queue of lock-waiting connections is visible seconds after a blocking DDL starts.' },
      { q: 'Key mitigation applied?', a: 'Killing the blocking migration and using an online schema change approach (ADD COLUMN as nullable, backfill in batches, add constraint with NOT VALID + VALIDATE).' },
      { q: 'What runbook item was missing?', a: 'A DDL review gate requiring all schema changes on tables over 1 million rows to use CONCURRENTLY or batch-backfill patterns before production execution.' },
      { q: 'Why was ALTER TABLE ... ADD COLUMN blocking?', a: 'The column had a non-constant default (jsonb), requiring PostgreSQL to rewrite all 300 million rows to add the default value -- taking a table-level lock for the full duration.' },
      { q: 'What is the safe way to add a column with a default in PostgreSQL?', a: 'Add column as NULLABLE (instant), backfill in batches with sleep, then add NOT NULL constraint with NOT VALID + separate VALIDATE CONSTRAINT -- never a full table lock.' },
      { q: 'What is PgBouncer and why is it needed for 12 microservices sharing one database?', a: 'A connection pooler that multiplexes many application connections onto fewer database connections. Prevents 12 services each with pool-max-10 from hitting max_connections simultaneously.' },
    ],
    references: [
...
DynamoDB partitions are limited to 3,000 read capacity units (RCU) per second per partition. The hero product's reads concentrated exclusively on a single partition. At peak, the sale page received 2,200 requests/second, each triggering 3 DynamoDB reads. The partition received 6,600 reads/second — more than twice its limit. DynamoDB returned ProvisionedThroughputExceededException errors. The application had no retry logic for DynamoDB errors on the read path, so product pages began failing for all users, not just users experiencing throttling.`,
    ...
    quickFire: [
      { q: 'Root cause of the DynamoDB throttling?', a: 'The hero product\'s product_id concentrated all flash-sale reads on a single partition, exceeding the 3,000 RCU/second per-partition limit at 6,600 reads/second peak.' },
      { q: 'What detection would have caught this earlier?', a: 'DynamoDB CloudWatch metric ProvisionedThroughputExceededException per partition -- non-zero values on a specific partition before the sale would signal a hot partition risk.' },
      { q: 'Key mitigation applied?', a: 'Adding ElastiCache in front of DynamoDB for product reads with a 5-second TTL -- the cache absorbs 99% of reads while keeping stock reasonably fresh.' },
      { q: 'What runbook item was missing?', a: 'A pre-event access pattern analysis: project peak reads per item, compare against the 3,000 RCU per partition limit, and add a caching layer for any item expected to exceed it.' },
      { q: 'What is a hot partition in DynamoDB?', a: 'A partition receiving a disproportionate share of traffic that hits the 3,000 RCU or 1,000 WCU per partition per second limit, causing throttling for items on that partition.' },
      { q: 'What is write sharding in DynamoDB?', a: 'Appending a random suffix (product_id#1 through product_id#10) to distribute writes across N partitions. Reads must query all N shards and merge -- used for high-write hot items.' },
      { q: 'Does DynamoDB on-demand mode eliminate hot partition throttling?', a: 'No -- per-partition limits (3,000 RCU, 1,000 WCU per second) still apply in on-demand mode. On-demand only removes the need to pre-provision total table capacity.' },
    ],
    references: [
...
This pattern means: any application version can be deployed between phases without breaking the database. It costs extra deployments but eliminates the rollback trap.`,
    ...
    quickFire: [
      { q: 'Root cause of the unrollbackable deploy?', a: 'A column rename migration was bundled in the same deployment as the application code. Rolling back the app code broke against the renamed column; rolling back the column broke the new app code.' },
      { q: 'What detection would have caught this earlier?', a: 'Automated rollback testing in staging: after deploying the new version, attempt to roll back to the previous version and run the test suite against the current database state.' },
      { q: 'Key mitigation applied?', a: 'A database view as a compatibility shim -- presenting the old column name while the underlying table used the new name, allowing both app versions to work simultaneously.' },
      { q: 'What runbook item was missing?', a: 'A deployment checklist requiring that all DB migrations be backward-compatible with the previous app version before they can be bundled in a release.' },
      { q: 'What is the expand-contract pattern for column renames?', a: 'Add new column (expand), deploy code writing both columns, backfill, deploy code reading only new column, then drop old column (contract) -- requires 3-5 deploys over multiple days.' },
      { q: 'What makes a database migration backward-compatible?', a: 'Both the old and new application versions can run successfully against the new database state simultaneously -- required for safe rollback and blue-green deployments.' },
      { q: 'When is a database view useful as a migration aid?', a: 'When a column was renamed without the expand-contract pattern, a view presenting the old column name buys time to forward-fix without rolling back the database.' },
    ],
    references: [
...
Enabling the flag caused 240 concurrent DynamoDB GetItem calls per second as each instance evaluated the flag on every incoming request. DynamoDB was configured with provisioned capacity of 100 RCU. The instantaneous spike to 240 RCU triggered throttling. The application interpreted DynamoDB throttling errors as "flag not available" and defaulted to disabled — meaning the feature was effectively off for 11 minutes until the throttling subsided. The irony: a flag change intended to enable a feature disabled it instead.`,
    ...
    quickFire: [
      { q: 'Root cause of the config change outage?', a: 'The application fetched full config from DynamoDB on every flag evaluation instead of using a local cache. Enabling the flag for 240 instances caused 240 concurrent DynamoDB reads per second, exceeding provisioned capacity.' },
      { q: 'What detection would have caught this earlier?', a: 'DynamoDB ProvisionedThroughputExceededException metric alert -- throttling would have been visible immediately when the flag was toggled.' },
      { q: 'Key mitigation applied?', a: 'Fetching config on a schedule with local in-memory cache -- flag evaluations read from the cache (no network call). Added jitter to poll intervals to spread load across all 240 instances.' },
      { q: 'What runbook item was missing?', a: 'A load impact analysis for config changes: estimate peak DynamoDB reads before toggling any flag on a large fleet.' },
      { q: 'What is the correct architecture for feature flag evaluation at scale?', a: 'Fetch the full config from the config service once per interval per instance, cache in memory, evaluate all flags against the cache locally with zero network calls per evaluation.' },
      { q: 'What is staggered config refresh?', a: 'Adding random jitter (e.g., base_interval + random(0, 10s)) to each instance\'s poll interval so all 240 instances do not hit the config service simultaneously.' },
      { q: 'What happens when a config service is unavailable with local caching?', a: 'Serve the last known config (fail-safe) rather than failing open or closed. Stale config for 30-60 seconds is almost always better than failing all requests.' },
    ],
    references: [
...
The application code was deployed before the migration was fixed. It expected all three constraints to be present for its validation logic to work correctly. With only two applied, some validation queries returned incorrect results. The fix required manually identifying which constraints had been applied, removing them to reach a clean state, fixing the orphaned records, and re-running the migration inside a transaction that would roll back all three if any one failed.`,
    ...
    quickFire: [
      { q: 'Root cause of the partial migration state?', a: 'Three ALTER TABLE statements ran outside an explicit transaction. PostgreSQL commits each statement individually -- when the third failed, the first two were already committed with no rollback.' },
      { q: 'What detection would have caught this earlier?', a: 'Pre-migration data validation: SELECT COUNT(*) FROM orders o WHERE NOT EXISTS (SELECT 1 FROM customers c WHERE c.id = o.customer_id) -- finds orphaned rows before the constraint is added.' },
      { q: 'Key mitigation applied?', a: 'Wrapping all DDL statements in a single explicit BEGIN/COMMIT transaction so any failure rolls back all preceding statements to a clean state.' },
      { q: 'What runbook item was missing?', a: 'A pre-flight validation query at the top of every constraint migration that counts rows violating the new constraint and fails with instructions if any are found.' },
      { q: 'What is transactional DDL?', a: 'Running DDL inside an explicit transaction so a failure rolls back all preceding DDL. PostgreSQL supports this; MySQL does not for most DDL statements.' },
      { q: 'What is an idempotent migration?', a: 'A migration that can be safely re-run multiple times, producing the same result -- achieved using ADD COLUMN IF NOT EXISTS, ADD CONSTRAINT IF NOT EXISTS, DROP IF EXISTS.' },
      { q: 'Why should app code never deploy before migration is fully verified?', a: 'If the migration left the DB in a partial state, the app code expecting all changes may produce incorrect results or errors against the incomplete schema.' },
    ],
    references: [
...
This is the same pattern used for JWT algorithm migrations, TLS certificate transitions, and any secret rotation in a live system.`,
    ...
    quickFire: [
      { q: 'Root cause of the split-brain sessions?', a: 'Green used a newly generated JWT signing key while blue still used the old key. Users routed to green had tokens signed by blue -- validation failed and they were logged out.' },
      { q: 'What detection would have caught this earlier?', a: 'A gradual traffic shift with session error rate monitoring -- a spike in 401 errors during the first 10% traffic shift would have caught the key mismatch before full rollout.' },
      { q: 'Key mitigation applied?', a: 'Dual-read key validation -- green was updated to accept tokens signed by either the old or the new key during the transition window.' },
      { q: 'What runbook item was missing?', a: 'A blue-green prerequisite checklist requiring that both environments accept the same session tokens before any traffic is shifted.' },
      { q: 'What is the dual-read pattern for key rotation?', a: 'During transition, the new environment validates tokens signed by both the old and new keys. New tokens use the new key. Old key is removed only after session TTL expires for all old tokens.' },
      { q: 'How long must dual-read be maintained during key rotation?', a: 'At least as long as the session TTL -- if sessions live 24 hours, the old key must remain valid for 24+ hours after the last token was issued with it.' },
      { q: 'Where else does the dual-read pattern apply besides session keys?', a: 'JWT algorithm migrations, TLS certificate transitions, API key rotations, and any secret rotation where existing clients hold the old value and cannot be force-invalidated instantly.' },
    ],
    references: [
...
Recovery required flushing the DNS cache (rndc flush) and repointing the A record to the correct IP.`,
    ...
    quickFire: [
      { q: 'Root cause of the DNS cache poisoning?', a: 'DNSSEC validation was disabled during a BIND upgrade and never re-enabled. A rogue resolver injected a forged A record pointing payments-api.internal to a decommissioned IP with a 7,200s TTL.' },
      { q: 'What detection would have caught this earlier?', a: 'Infrastructure compliance monitoring (AWS Config custom rule or configuration drift detection) checking that DNSSEC validation is enabled on all internal resolvers -- would have flagged the gap within hours.' },
      { q: 'Key mitigation applied?', a: 'Flushing the BIND cache with rndc flush to evict the poisoned record, re-enabling DNSSEC validation, and correcting the A record to the live IP.' },
      { q: 'What runbook item was missing?', a: 'A post-maintenance checklist requiring DNSSEC validation to be re-enabled and verified before closing any DNS infrastructure change ticket.' },
      { q: 'What is DNSSEC?', a: 'Cryptographic signatures on DNS records that allow resolvers to verify records are authentic and have not been tampered with -- prevents cache poisoning attacks.' },
      { q: 'Why do long TTLs amplify DNS poisoning impact?', a: 'A poisoned record with a 7,200s TTL persists in cache for 2 hours with no active intervention required. Short TTLs (30-60s) limit exposure duration.' },
      { q: 'What is service discovery with health checks vs static DNS?', a: 'A registry (Consul, AWS Cloud Map) actively health-checks instances every 10s and removes unhealthy IPs from DNS responses -- stale records expire within seconds, not hours.' },
    ],
    references: [
...
Recovery was a 45-minute process: obtain a new certificate, update the certificate on the API gateway, and force a rotation of TLS sessions across all services.`,
    ...
    quickFire: [
      { q: 'Root cause of the certificate expiry outage?', a: 'A wildcard cert expired 31 days before discovery. The single 7-day expiry alert went to a defunct email alias. TLS session resumption masked the expiry for existing connections -- only new connections failed.' },
      { q: 'What detection would have caught this earlier?', a: 'External certificate monitoring checking the actual TLS handshake from outside the service -- alerts at 90, 30, 14, and 7 days remaining to multiple escalation channels.' },
      { q: 'Key mitigation applied?', a: 'Switching to ACM (AWS Certificate Manager) for all ALB and CloudFront endpoints -- ACM auto-renews 60 days before expiry with zero human intervention required.' },
      { q: 'What runbook item was missing?', a: 'Multiple expiry alert thresholds (90/30/14/7 days) routed to the current on-call rotation, not a static email alias. A single 7-day alert with one destination is a single point of failure.' },
      { q: 'What is TLS session resumption and why did it mask the expiry?', a: 'Reusing an established TLS session via session tickets avoids a full handshake and certificate revalidation. Existing sessions continued working; only new connections triggered certificate validation and failed.' },
      { q: 'What is cert-manager in Kubernetes?', a: 'A Kubernetes add-on that automatically provisions and renews certificates from Let\'s Encrypt or private CAs -- eliminates all manual certificate management for containerized workloads.' },
      { q: 'Why should certificate monitoring check from outside the service?', a: 'External monitoring tests the actual TLS handshake as a client would see it, catching issues (expired cert, wrong cert served, chain incomplete) regardless of how the cert is managed internally.' },
    ],
    references: [
...
The incident had no data security impact for the company, but it violated acceptable-use terms with the carrier and required a formal incident report.`,
    ...
    quickFire: [
      { q: 'Root cause of the BGP route leak?', a: 'A prefix filter on the Direct Connect router was misconfigured to accept a /16 supernet instead of the specific /24, causing some BGP speakers to route adjacent customer traffic through the company\'s link.' },
      { q: 'What detection would have caught this earlier?', a: 'BGP route monitoring tools (BGPMon, RIPE RIS, AWS Network Manager) that alert when unexpected prefixes are announced or when the AS-path for known prefixes changes.' },
      { q: 'Key mitigation applied?', a: 'Correcting the prefix filter to an explicit /24 permit list with a deny-all default, removing the /16 supernet acceptance within 18 minutes of the adjacent customer complaint.' },
      { q: 'What runbook item was missing?', a: 'A peer review requirement for all BGP policy changes -- network config changes have global routing impact and require a second engineer to verify the prefix filter before activation.' },
      { q: 'What is the correct BGP prefix filter design?', a: 'Explicit permit list with deny-all default -- list only the specific CIDRs you own or intend to accept, never use permit-all with deny exceptions or accept supernets.' },
      { q: 'What is a maximum-prefix limit in BGP?', a: 'A BGP session configuration that triggers an alert or drops the session if a peer advertises more prefixes than expected -- catches both misconfiguration on your side and on the peer\'s side.' },
      { q: 'What is AS-path prepending?', a: 'Adding your own AS number to the AS-path multiple times to make a route appear longer and less preferred -- used for traffic engineering to influence which path remote networks prefer.' },
    ],
    references: [
