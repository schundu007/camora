...
Multi-Instance GPU (MIG) partitions a GPU into up to 7 isolated instances, each with dedicated SM slices, L2 cache slices, and HBM slices. MIG provides hardware isolation suitable for multi-tenant serving where QoS guarantees are required. Multi-Process Service (MPS) allows multiple CUDA processes to share the GPU without MIG spatial partitioning, enabling higher occupancy when batches are small. The choice depends on isolation requirement: MIG for strict SLA isolation, MPS for maximizing utilization in trusted workloads.`,
    ...
    quickFire: [
      ...
      { q: 'What is NUMA awareness in GPU systems?', a: 'On multi-socket servers, each CPU socket has its own local memory. GPUs attached to a PCIe bus on NUMA node 0 have lower latency to CPU0 memory. CPU pinning and NUMA-aware allocation reduce PCIe transfer latency.' },
      { q: 'What is the Blackwell FP4 tensor core capability?', a: 'Blackwell 4th-gen tensor cores natively execute FP4 (4-bit float) operations at 2x the throughput of FP8, enabling higher throughput for quantized LLM inference at acceptable accuracy loss.' },
...
The practical consequence: if a kernel has too few threads to fill all SMs, some SMs sit idle and you waste the GPU. If a thread block is too large, register pressure forces reduced occupancy. If branches diverge within a warp, the SM serially executes each divergent path. Optimal kernel design requires choosing block sizes that maximize SM occupancy and avoid divergence, which the profiler reveals via the achieved occupancy metric.`,
      },
      {
...
The HBM traffic becomes O(N * d) -- only proportional to the sequence length times head dimension, not quadratic in sequence length. For N=8192, this is a ~50x reduction in memory reads/writes compared to naive attention. SMEM usage is O(Br * d + Bc * d) which is controlled by the tile size and fits within one SM shared memory.
...
The practical impact: FlashAttention is the standard attention implementation in PyTorch (F.scaled_dot_product_attention), vLLM, TensorRT-LLM, and all production LLM frameworks. Without it, long-context inference (100K+ tokens) would be impractical.`,
      },
      {
...
vLLM, TensorRT-LLM, and SGLang all use CUDA Graph capture for the decode phase (where batch shapes are more predictable) while using eager mode for prefill (variable-length, less latency-critical). This hybrid approach captures ~80-90% of the graph replay benefit while maintaining flexibility for prefill.`,
      },
    ],
...
In FSDP, the forward pass AllGathers weight shards before each layer is computed, and the backward pass ReduceScatters gradients before moving to the previous layer. FSDP2 (PyTorch 2.1+) uses async pre-fetching to overlap the AllGather for the next layer with the compute of the current layer, eliminating most communication stalls.
...
NVIDIA SHARP offloads AllReduce computation to the InfiniBand switch fabric, performing partial reductions at switch aggregation points. This halves the volume of data traversing the full bisection bandwidth and reduces AllReduce latency by up to 2x for large models across many nodes.`,
    ...
    quickFire: [
      ...
      { q: 'What is a pipeline bubble and how does 1F1B help?', a: 'A pipeline bubble is idle time waiting for activations from the previous stage. GPipe runs all microbatches forward then all backward (large bubble). 1F1B interleaves them to reduce the bubble to 1/(number of microbatches).' },
...
Choose DDP when: the model fits in GPU memory, communication overhead must be minimized (DDP AllReduce has lower total volume than FSDP AllGather + ReduceScatter pairs), or you need simplicity.

Choose FSDP when: the model does not fit in GPU memory without sharding, you are training a 10B+ parameter model, or you need zero-redundancy optimizer (ZeRO) semantics for memory efficiency.

FSDP2 (PyTorch 2.1+) adds prefetching the next layer AllGather while computing the current layer (eliminating communication stalls), mixed-precision per-parameter-group, and simpler API. For large model training, FSDP2 with prefetch enabled achieves near-DDP compute efficiency while handling models that DDP cannot accommodate.`,
      },
      {
...
Profiling the overlap: Nsight Systems shows the NCCL stream alongside the compute stream. Gaps where NCCL is not running while the compute stream is active indicate under-utilized communication bandwidth. Gaps where compute is not running while NCCL is active indicate compute being stalled waiting for communication.`,
      },
    ],
...
The practical rule: speculative decoding is a latency optimization for interactive serving at low batch sizes. It trades throughput for latency. At high batch sizes where throughput is the priority, it is counterproductive.`,
...
Code sketch for speculative sampling:
...
- With K=5, alpha=0.8: E[tokens] = 0.8*(1-0.8^5)/0.2 + 1 = 0.8*0.672/0.2 + 1 = 3.69 tokens
...
Sizing: for 100 requests/sec with avg 2048-token prompts and 512-token responses, roughly 8-12 prefill nodes and 4-6 decode nodes provide adequate throughput with sub-500ms TTFT and sub-50ms TPOT, depending on hardware efficiency.`,
      },
    ],
...
During backpropagation, PyTorch retains all intermediate activations from the forward pass in memory to compute gradients. For a transformer with L layers, batch size B, sequence length S, and hidden dimension H, activation memory grows as O(L * B * S * H). For LLaMA-3 70B training with L=80, B=4, S=4096, H=8192: 80 * 4 * 4096 * 8192 * 2 bytes = ~21 GB of activation memory, before accounting for attention maps.
...
Recompilation for dynamic shapes: by default, TorchDynamo recompiles the graph when input shapes change (different batch sizes, different sequence lengths). For a serving scenario with variable length inputs, this causes significant overhead. Fix: pass dynamic=True to torch.compile, which generates shape-generic Triton code. Alternatively, bucket input lengths and compile a separate graph per bucket.
...
FSDP + torch.compile: FSDP2 is officially supported with torch.compile via torch.compile(model, fullgraph=True). The AllGather operations appear as nodes in the compiled graph, enabling the compiler to fuse the post-AllGather operations with the subsequent GEMM.`,
      },
      {
...
- With FSDP2 FULL_SHARD: model_shard = 1120 GB / 16 = 70 GB / 16 = 4.4 GB base
