...
// POST /setup — set goal date, generate prep plan
router.post('/setup', jwtAuth, async (req, res) => {
  try {
    const userId = req.user.id;
    const { goal_date, target_company, target_role } = req.body;
    if (!goal_date) return res.status(400).json({ error: 'goal_date required' });

    const goalDate = new Date(goal_date);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const totalDays = Math.max(1, Math.ceil((goalDate - today) / (1000 * 60 * 60 * 24)));

    // Update user
    await query(
      'UPDATE users SET interview_date = $1, target_company = $2, target_role = $3 WHERE id = $4',
      [goal_date, target_company || null, target_role || null, userId]
    );
    ...
    await query(
      ...
      [userId, goal_date, target_company || null, target_role || null, JSON.stringify(planData), totalDays]
    );

    res.json({ success: true, plan: planData, days_remaining: totalDays });
  } catch (err) {
    console.error('[GoalCountdown] Setup error:', err.message);
    res.status(500).json({ error: 'Failed to set up goal countdown' });
  }
});
...
router.get('/plan', jwtAuth, async (req, res) => {
  try {
    ...
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const goalDate = new Date(plan.interview_date);
...
router.get('/countdown', jwtAuth, async (req, res) => {
  try {
    ...
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const goalDate = new Date(user.interview_date);
    const daysRemaining = Math.max(0, Math.ceil((goalDate - today) / (1000 * 60 * 60 * 24)));
