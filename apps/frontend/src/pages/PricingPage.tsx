    ...
    <div className="min-h-screen" style={{ color: 'var(--text-primary)' }}>
      ...
      <section className="w-full lg:max-w-[70%] mx-auto px-6 py-14">
        <div className="mb-10 text-center">
          <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ background: 'linear-gradient(135deg, #76B900, #76B900, #76B900)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Plans</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-4 items-stretch">
          {plans.map((plan) => {
            ...
            const accent = isPro
              ? { from: '#76B900', via: '#76B900', to: '#76B900', glow: 'rgba(118,185,0,0.4)', checkColor: '#91C733' }
              : isAnnual
              ? { from: '#f59e0b', via: '#fbbf24', to: '#fde68a', glow: 'rgba(245,158,11,0.3)', checkColor: '#fbbf24' }
              : isStarter
              ? { from: '#76B900', via: '#76B900', to: '#91C733', glow: 'rgba(118,185,0,0.15)', checkColor: '#76B900' }
                ...
                <div className="relative flex flex-col flex-1 rounded-[21px] overflow-hidden" style={{ background: isPro ? 'linear-gradient(168deg, rgba(30,27,60,0.98) 0%, rgba(15,14,28,0.99) 100%)' : isAnnual ? 'linear-gradient(168deg, rgba(40,30,15,0.95) 0%, rgba(15,14,22,0.99) 100%)' : isStarter ? 'linear-gradient(168deg, rgba(22,20,40,0.98) 0%, rgba(13,12,22,0.99) 100%)' : 'linear-gradient(168deg, rgba(20,19,30,0.98) 0%, rgba(12,11,20,0.99) 100%)', backdropFilter: 'blur(20px)' }}>
                  ...
                  <div className="relative p-7 pb-0 flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-[13px] font-bold uppercase tracking-[0.12em]" style={{ color: isPro ? '#B8E06C' : isAnnual ? '#fde68a' : isStarter ? '#91C733' : 'rgba(255,255,255,0.5)' }}>{plan.name}</h3>
                      {isPro && <span className="px-3 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-[0.12em]" style={{ background: 'linear-gradient(135deg, #76B900, #76B900)', color: '#fff', boxShadow: '0 2px 12px rgba(118,185,0,0.5), 0 0 20px rgba(118,185,0,0.2)' }}>Most Popular</span>}
                    ...
                    <div className="flex items-baseline gap-1.5">
                      <span className="font-extrabold leading-none tracking-tight" style={{ fontSize: isPro ? '52px' : '46px', ...(isPro ? { background: 'linear-gradient(145deg, #D4F0A0, #B8E06C, #91C733)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', filter: 'drop-shadow(0 0 20px rgba(118,185,0,0.3))' } : isAnnual ? { background: 'linear-gradient(145deg, #fef3c7, #fde68a, #fbbf24)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', filter: 'drop-shadow(0 0 16px rgba(245,158,11,0.25))' } : isStarter ? { background: 'linear-gradient(145deg, rgba(255,255,255,0.95), rgba(255,255,255,0.7))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' } : { color: 'rgba(255,255,255,0.85)' }) }}>{plan.price}</span>
                  ...
                  <div className="p-7 pt-0">
                    <button onClick={() => handleCheckout(plan)} disabled={loading === plan.name} className="pricing-cta w-full py-4 text-sm font-bold rounded-[14px] cursor-pointer transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2" style={isPro ? { background: 'linear-gradient(135deg, #76B900, #7c3aed, #76B900)', color: '#fff', boxShadow: '0 6px 24px rgba(118,185,0,0.45), 0 0 40px rgba(118,185,0,0.15), inset 0 1px 0 rgba(255,255,255,0.15)' } : isAnnual ? { background: 'linear-gradient(135deg, #fbbf24, #f59e0b, #d97706)', color: '#451a03', boxShadow: '0 6px 24px rgba(245,158,11,0.4), 0 0 40px rgba(245,158,11,0.1), inset 0 1px 0 rgba(255,255,255,0.25)' } : isStarter ? { background: 'linear-gradient(135deg, rgba(118,185,0,0.18), rgba(139,92,246,0.12))', color: '#B8E06C', border: '1px solid rgba(118,185,0,0.35)', boxShadow: '0 2px 12px rgba(118,185,0,0.1)' } : { background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.6)', border: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      <section className="px-6 pt-4 pb-16">
        <div className="w-full lg:max-w-[70%] mx-auto">
          <div className="mb-10 text-center">
            <span className="text-xs font-bold uppercase tracking-[0.2em]" style={{ background: 'linear-gradient(135deg, #76B900, #76B900, #76B900)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Why Camora Wins</span>
            <h2 className="mt-3 text-3xl md:text-4xl font-extrabold tracking-tight" style={{ color: 'var(--text-primary)' }}>
            </h2>
          </div>
          ...
          >
            ...
            <div className="absolute -inset-[1px] rounded-[22px] pointer-events-none" style={{ background: 'linear-gradient(135deg, #76B900, #76B900, #76B900, #76B900)', backgroundSize: '300% 300%', animation: 'borderShimmer 4s ease infinite', opacity: 0.4 }} />

            <div className="relative rounded-[21px] overflow-x-auto" style={{ background: 'linear-gradient(168deg, #1a1730 0%, #0e0d18 100%)' }}>
              {/* Top accent bar */}
              <div className="h-[3px] w-full" style={{ background: 'linear-gradient(90deg, #10b981, #76B900, #76B900, #ec4899, #76B900, #10b981)' }} />
                      ...
                      {[
                        ...
                        >
                          <div
                            ...
                            style={col.highlight
                              ? { background: 'linear-gradient(135deg, #D4F0A0, #fff)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: '15px', letterSpacing: '0.02em' }
                          ...
                          <div
                            ...
                            style={col.highlight
                              ? { background: 'linear-gradient(135deg, #91C733, #76B900)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: '20px', filter: 'drop-shadow(0 0 12px rgba(118,185,0,0.5))' }
                  ...
                  <tbody>
                    {COMPARISON.map((row, i) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-4 px-4 text-[13px]" style={{ color: row.unique ? '#B8E06C' : 'rgba(255,255,255,0.75)', fontWeight: row.unique ? 700 : 500, borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.06)' }}>
                              ...
                              {row.unique && (
                                <span className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, rgba(118,185,0,0.3), rgba(139,92,246,0.25))', boxShadow: '0 0 8px rgba(118,185,0,0.3)' }}>
                                  <svg className="w-[10px] h-[10px]" viewBox="0 0 12 12" fill="none">
                                    <path d="M6 1l1.545 3.13L11 4.635 8.5 7.07l.59 3.44L6 8.885 2.91 10.51l.59-3.44L1 4.635l3.455-.505L6 1z" fill="#91C733" />
                                  </svg>
                                </span>
                              )}
                              {row.feature}
                          ...
                          >
                            {row.camora === true
                              ? (
                                <span className="inline-flex items-center justify-center w-8 h-8 rounded-full" style={{ background: 'linear-gradient(135deg, #76B900, #76B900)', boxShadow: '0 0 16px rgba(118,185,0,0.5), 0 4px 12px rgba(118,185,0,0.3)' }}>
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          <div className="rounded-xl p-4 flex items-center justify-between gap-4" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'linear-gradient(135deg, rgba(118,185,0,0.15), rgba(139,92,246,0.15))', color: '#76B900' }}>
            ...
            <a href="https://github.com/schundu007/camora/releases/latest" target="_blank" rel="noopener noreferrer" className="shrink-0 px-4 py-1.5 text-white text-[11px] font-semibold rounded-lg" style={{ background: 'linear-gradient(135deg, #76B900, #76B900)' }}>
              Download
            </a>
          </div>
