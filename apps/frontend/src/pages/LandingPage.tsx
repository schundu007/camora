...
const L = {
  ...
  blue: '#0B5CFF', emerald: '#10b981', radius: '10px',
};
          ...
          return (
            <button key={s.key} onClick={() => setActive(i)}
              ...
              <div className="w-9 h-9 flex items-center justify-center flex-shrink-0"
                style={{ borderRadius: L.radius, background: isActive ? `${L.blue}10` : L.elevated, color: isActive ? L.blue : L.muted }}>
                {s.icon}
              </div>
          ...
          <div className="relative overflow-hidden" style={{ maskImage: 'linear-gradient(90deg, transparent, black 10%, black 90%, transparent)' }}>
            <div className="flex items-center gap-12" style={{ animation: 'scroll-logos 30s linear infinite', width: 'max-content' }}>
              {[...COMPANIES, ...COMPANIES].map((c, i) => (
                <img key={`${c}-${i}`} src={`https://img.logo.dev/${c}.com?token=pk_a8CO5iIAQbGzIhCLGCp2cQ&size=80&format=png`}
                  alt={c} className="h-7 object-contain grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all" loading="lazy" />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          <Reveal className="text-center mb-14">
            <span className="inline-block text-[11px] font-bold tracking-[0.18em] uppercase px-4 py-1.5 mb-5"
              style={{ borderRadius: '999px', background: `${L.blue}0A`, border: `1px solid ${L.blue}20`, color: L.blue, fontFamily: F.mono }}>How it works</span>
              ...
              <Reveal key={f.title} delay={i * 0.04}>
                <div className="p-6 h-full" style={{ borderRadius: '16px', background: L.bg, border: `1px solid ${L.border}` }}>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1" style={{ borderRadius: '6px', background: `${L.blue}08`, color: L.blue, fontFamily: F.mono }}>{f.tag}</span>
                  </div>
            ...
            {PLANS.map((plan, i) => (
              <Reveal key={plan.name} delay={i * 0.06}>
                <div className="relative p-7 h-full flex flex-col" style={{
                  ...
                  border: plan.popular ? `2px solid ${L.blue}` : `1px solid ${L.border}`,
                }}>
                  {plan.popular && (
