...
function diffColor(d) {
  ...
  return { bg: 'rgba(220,38,38,0.12)', text: '#5E9400' };
}
...
function RadarChart({ values, labels, size = 200 }) {
  ...
  return (
    <svg width={size} height={size} style={{ overflow: 'visible' }}>
      ...
      {/* Data shape */}
      <path d={dataPath} fill="rgba(118,185,0,0.15)" stroke="#76B900" strokeWidth={2} />
      {/* Data dots */}
      {dataPoints.map((p, i) => (
        <circle key={i} cx={p[0]} cy={p[1]} r={4} fill="#76B900" stroke="#fff" strokeWidth={2} />
      ))}
      {/* Labels */}
      {labels.map((label, i) => {
...
function Sparkline({ data, width = 100, height = 28, color = '#76B900' }) {
  ...
  return (
    <div className="practice-root" style={{ background: 'transparent', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="w-full lg:max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8" style={{ paddingTop: 32, paddingBottom: 80 }}>
          ...
          {phase === 'setup' && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, padding: '12px 20px', background: 'var(--bg-surface)', border: 'none', borderRadius: 12, boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
                {/* Readiness score */}
                ...
                {CATEGORIES.map(cat => {
                  ...
                  const colors = { coding: '#76B900', 'system-design': '#06b6d4', behavioral: '#f59e0b' };
                  return (
              ...
              <div style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(118,185,0,0.12)', marginBottom: 24 }}>
                ...
                <div style={{ padding: '20px 24px' }}>
                  ...
                  <div style={{ marginBottom: 22 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
                      {MODES.map(m => (
                        <button key={m.id} onClick={() => setMode(m.id)} style={{ padding: '16px 18px', borderRadius: 14, border: mode === m.id ? '2px solid #76B900' : '1px solid var(--border)', background: mode === m.id ? 'var(--accent-subtle)' : 'var(--bg-surface)', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                            <div style={{ width: 28, height: 28, borderRadius: 8, background: mode === m.id ? '#76B900' : 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <Icon name={m.icon} size={14} style={{ color: mode === m.id ? '#fff' : '#94a3b8' }} />
                            </div>
                            <span style={{ fontSize: 14, fontWeight: 700, color: mode === m.id ? '#5E9400' : 'var(--text-primary)' }}>{m.label}</span>
                          </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 22 }}>
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {CATEGORIES.map(c => (
                          <button key={c} onClick={() => setCategory(c)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 10, border: category === c ? '2px solid #76B900' : '1px solid var(--border)', background: category === c ? 'var(--accent-subtle)' : 'var(--bg-surface)', color: category === c ? '#5E9400' : 'var(--text-secondary)', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
                ...
                <div style={{ padding: '16px 24px', background: 'var(--bg-elevated)', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
                  <button onClick={() => startChallenge()} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '13px 36px', background: 'linear-gradient(135deg, #76B900, #76B900)', color: '#fff', fontSize: 15, fontWeight: 700, borderRadius: 12, border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(118,185,0,0.3)', transition: 'transform 0.15s, box-shadow 0.15s' }}>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'grid', gap: 10 }}>
                    {stats.history.slice(0, 10).map((h, i) => {
                      ...
                      return (
                        <div key={i} style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 12, overflow: 'hidden', boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
          ...
          {phase === 'active' && questions[currentIdx] && (
            <div>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {questions.map((_, qi) => (
                      <div key={qi} style={{ width: qi === currentIdx ? 18 : 8, height: 8, borderRadius: 99, background: qi < currentIdx ? '#76B900' : qi === currentIdx ? '#76B900' : 'var(--border)', transition: 'all 0.3s' }} />
                    ))}
                  </div>
                  <InterviewTimer
              ...
              {/* Question card */}
              <div style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 16, padding: 24, marginBottom: 16, boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                  ...
                  {questions[currentIdx].topics?.slice(0, 2).map(t => (
                    <span key={t} style={{ fontSize: 10, fontWeight: 500, padding: '2px 6px', borderRadius: 99, background: 'rgba(124,58,237,0.12)', color: '#76B900' }}>{t}</span>
                  ))}
                </div>
              ...
              {category === 'system-design' && !inlineEval && (() => {
                const SD_SECTIONS = [
                  ...
                  { label: 'Non-Functional Req.', icon: 'shield', color: '#76B900', placeholder: 'Latency, availability, consistency, scale...' },
              ...
              {category === 'behavioral' && !inlineEval && (
                <div style={{ display: 'grid', gap: 8, marginBottom: 8 }}>
                  {['Situation', 'Task', 'Action', 'Result'].map((section, si) => {
                    ...
                    const val = parts[si] || '';
                    const colors = { Situation: '#3b82f6', Task: '#76B900', Action: '#10b981', Result: '#f59e0b' };
                    return (
                      <div key={section}>
                ...
                <div style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 16, padding: 24, marginBottom: 16, boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
                  ...
                  {inlineEval.modelAnswer && (
                    <div style={{ marginTop: 12 }}>
                      <button onClick={() => setShowModelAnswer(showModelAnswer ? null : currentIdx)} style={{ fontSize: 12, fontWeight: 600, color: '#5E9400', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Icon name={showModelAnswer === currentIdx ? 'chevronUp' : 'chevronDown'} size={14} style={{ color: '#5E9400' }} />
                  ...
                  <div style={{ marginTop: 16 }}>
                    <button onClick={moveToNext} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #76B900, #76B900)', color: '#fff', fontSize: 14, fontWeight: 600, borderRadius: 10, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {!inlineEval && (
                <div style={{ display: 'flex', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
                  <button onClick={submitAnswer} disabled={evaluating} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #76B900, #76B900)', color: '#fff', fontSize: 14, fontWeight: 600, borderRadius: 10, border: 'none', cursor: evaluating ? 'wait' : 'pointer', opacity: evaluating ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 6 }}>
                    {evaluating ? (
            ...
            <div>
              ...
              <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)', borderRadius: 20, padding: '40px 32px', marginBottom: 20, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 28, position: 'relative' }}>
                  ...
                  <button onClick={startChallenge} style={{ padding: '11px 22px', background: 'linear-gradient(135deg, #76B900, #76B900)', color: '#fff', fontSize: 13, fontWeight: 600, borderRadius: 10, border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(118,185,0,0.3)', transition: 'all 0.2s' }}>
                    Try Again
                  </button>
              ...
              {resultDimensions && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
                  {DIMENSION_KEYS.map((k, i) => {
                    ...
                    return (
                      <div key={k} style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 14, padding: '16px 18px', boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
              ...
              {/* ── Question Breakdown ── */}
              <div style={{ background: 'var(--bg-surface)', border: 'none', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(118,185,0,0.12)' }}>
                    ...
                    <div key={i} style={{ borderBottom: i < questions.length - 1 ? '1px solid var(--border)' : 'none' }}>
                      <button onClick={() => setExpandedHistory(isExpanded ? null : `result-${i}`)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 24px', background: isExpanded ? 'var(--bg-elevated)' : 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', transition: 'background 0.15s' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 26, height: 26, borderRadius: 8, background: pass ? 'rgba(5,150,105,0.12)' : 'rgba(220,38,38,0.12)', color: pass ? '#76B900' : '#5E9400', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{i + 1}</div>
                          <div>
                      ...
                      {isExpanded && (
                        <div style={{ padding: '0 24px 20px 60px' }}>
                          ...
                          {ma.modelAnswer && (
                            <div style={{ marginTop: 12 }}>
                              <button onClick={() => setShowModelAnswer(showModelAnswer === `r-${i}` ? null : `r-${i}`)} style={{ fontSize: 12, fontWeight: 600, color: '#5E9400', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                                <Icon name={showModelAnswer === `r-${i}` ? 'chevronUp' : 'chevronDown'} size={12} style={{ color: '#5E9400' }} />
                                {showModelAnswer === `r-${i}` ? 'Hide' : 'Show'} Model Answer
                              </button>
                              {showModelAnswer === `r-${i}` && (
                                <div style={{ marginTop: 8, padding: 14, background: 'var(--accent-subtle)', borderRadius: 10, border: '1px solid rgba(118,185,0,0.25)', fontSize: 12, color: 'var(--accent-hover)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                                  {ma.modelAnswer}
                                </div>
                              )}
                            </div>
      ...
      <style>{`
        ...
        textarea:focus { border-color: #76B900 !important; box-shadow: 0 0 0 3px rgba(118,185,0,0.1); }
