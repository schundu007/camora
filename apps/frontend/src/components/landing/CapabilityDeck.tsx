  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes cd-glow-ring { 0% { box-shadow: 0 0 0 0 rgba(6,78,59,0.33); } 80%,100% { box-shadow: 0 0 0 18px rgba(6,78,59,0); } }
...
function SceneLive() {
  ...
  return (
    <div className="grid grid-cols-[1fr,1fr] gap-5" style={{ marginTop: 8 }}>
      ...
      <div style={{ background: 'linear-gradient(180deg, rgba(6,78,59,0.08), rgba(6,78,59,0.02))', border: '1px solid rgba(6,78,59,0.20)', borderRadius: 10, padding: 14, position: 'relative', overflow: 'hidden' }}>
        <span aria-hidden style={{ position: 'absolute', left: 0, right: 0, top: 0, height: 40, background: 'linear-gradient(180deg, transparent, rgba(6,78,59,0.15), transparent)', animation: 'cd-scan 2.6s ease-in-out 0.8s infinite', pointerEvents: 'none' }} />
        <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {chips.map((c, i) => (
            <span key={c} style={{ fontSize: 11, fontWeight: 700, color: ACCENT, background: 'rgba(6,78,59,0.07)', border: '1px solid rgba(6,78,59,0.20)', padding: '4px 10px', borderRadius: 999, fontFamily: MONO, opacity: 0, animation: `cd-pop 0.4s cubic-bezier(0.34,1.56,0.64,1) ${1.2 + i * 0.24}s forwards` }}>
              {c}
            </span>
          ))}
        </div>
        ...
        {STAGES.map((s, i) => (
          <div key={s.n} style={{
            ...
            <span style={{
              ...
              background: 'rgba(6,78,59,0.08)', border: '1px solid rgba(6,78,59,0.33)',
      ...
      <div className="flex items-center gap-1.5 mb-3 flex-wrap">
        {APPROACHES.map((a, i) => (
          <span key={a.label} style={{
            ...
            background: i === tab ? 'rgba(6,78,59,0.08)' : 'transparent',
            border: `1px solid ${i === tab ? 'rgba(6,78,59,0.33)' : '#1E293B'}`,
          ...
          <div style={{ opacity: 0, animation: 'cd-fade-up 0.4s ease-out 0.45s forwards' }}>
            ...
            {[
              ...
              <div key={e.p} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', opacity: 0, animation: `cd-fade-up 0.3s ease-out ${0.6 + i * 0.1}s forwards` }}>
                <span style={{ fontSize: 9, fontWeight: 800, color: e.m === 'POST' ? ACCENT : '#94A3B8', fontFamily: MONO, padding: '1px 5px', border: `1px solid ${e.m === 'POST' ? 'rgba(6,78,59,0.33)' : '#334155'}`, borderRadius: 3, letterSpacing: '0.1em' }}>
                  {e.m}
                </span>
    ...
    <div style={{ marginTop: 8 }}>
      <div style={{ background: 'rgba(30,41,59,0.35)', border: '1px solid #1E293B', borderRadius: 10, padding: 20 }}>
        ...
        <div className="flex items-center justify-between mt-5 pt-4" style={{ borderTop: '1px solid #1E293B', opacity: 0, animation: 'cd-fade-up 0.4s ease-out 1.7s forwards' }}>
          ...
          <div style={{ padding: '8px 16px', borderRadius: 999, background: 'rgba(6,78,59,0.08)', border: '1px solid rgba(6,78,59,0.33)', fontSize: 11, fontWeight: 800, color: ACCENT, fontFamily: MONO, letterSpacing: '0.14em', animation: 'cd-glow-ring 1.6s ease-out 2s infinite' }}>
            READY FOR FAANG
          </div>
        </div>
      </div>
