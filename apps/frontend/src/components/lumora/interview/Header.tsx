...
export function Header({ inputValue, onInputChange, onSubmit, onTranscription, showInputBar = true, activeTab, onTabChange, onToggleSidebar, sidebarOpen }: HeaderProps) {
  ...
  return (
    <header className="z-50 shrink-0" style={{ background: 'linear-gradient(135deg, #36B24E 0%, #0D9488 100%)', borderBottom: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 1px 8px rgba(0,0,0,0.15)', fontFamily: "'Satoshi', sans-serif" }}>
        ...
        <div className="flex-1 flex items-center justify-center px-3 min-w-0">
          ...
          {inputValue && (
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #36B24E, #0D9488)', boxShadow: '0 2px 12px rgba(11,92,255,0.3)' }}
            >
    ...
    <div className="relative">
      ...
      >
        {user.image ? (
          ...
          <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-[9px] font-bold" style={{ background: 'linear-gradient(135deg, #36B24E, #0D9488)' }}>
            {initials}
          </div>
        )}
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-48 rounded-2xl shadow-xl border z-50 py-1 overflow-hidden" style={{ background: 'linear-gradient(180deg, #36B24E 0%, #12121A 100%)', borderColor: 'rgba(255,255,255,0.12)' }}>
