      ...
      <div className="flex items-center h-[42px] overflow-x-auto no-scrollbar">
        ...
        {onToggleSidebar && (
          <button
            ...
            style={{ color: sidebarOpen ? '#F49AA3' : 'rgba(255,255,255,0.6)', borderRight: '1px solid rgba(255,255,255,0.1)' }}
          ...
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => handleTabClick(tab.id)}
              ...
              style={currentTab === tab.id
                ? { background: 'rgba(230,57,70,0.15)', color: '#F49AA3', fontFamily: "'Inter', sans-serif", boxShadow: 'inset 0 0 0 1px rgba(230,57,70,0.25)' }
        ...
        <div className="hidden lg:flex items-center gap-2 px-3 h-full shrink-0" style={{ borderLeft: '1px solid rgba(255,255,255,0.15)' }}>
          <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${
            ...
            'bg-white/20'
          }`} style={status.state === 'ready' ? { boxShadow: '0 0 6px rgba(230,57,70,0.5)' } : {}} />
        ...
        <div className="flex items-center gap-0.5 px-1.5 h-full shrink-0" style={{ borderLeft: '1px solid rgba(255,255,255,0.15)' }}>
          <button onClick={() => setUseSearch(!useSearch)} className="p-1.5 rounded-md transition-all"
            style={useSearch ? { background: 'rgba(230,57,70,0.15)', color: '#F49AA3' } : { color: 'rgba(255,255,255,0.7)' }} title="Search (⌘S)">
            <SearchIcon />
          </button>
      ...
      {showInputBar && <div data-tour="input" className="flex items-center h-[36px]" style={{ borderTop: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.04)' }}>
        <div className="flex-1 flex items-center justify-center px-3 min-w-0">
          ...
          <button
            ...
            style={isExpanded ? { color: '#F49AA3', background: 'rgba(230,57,70,0.15)' } : { color: 'rgba(255,255,255,0.75)' }}
          ...
          {inputValue && (
            <button
              ...
              style={{ background: 'linear-gradient(135deg, var(--cam-primary), var(--cam-primary))', boxShadow: '0 2px 12px rgba(230,57,70,0.3)' }}
            >
