...
export function Header({ inputValue, onInputChange, onSubmit, onTranscription, showInputBar = true, activeTab, onTabChange, onToggleSidebar, sidebarOpen }: HeaderProps) {
  ...
  return (
    <header className="z-50 shrink-0" style={{ background: 'linear-gradient(135deg, #76B900 0%, #76B900 100%)', borderBottom: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 1px 8px rgba(0,0,0,0.15)', fontFamily: "'Satoshi', sans-serif" }}>
      ...
      <div className="flex items-center h-[42px] overflow-x-auto no-scrollbar">
        ...
        {onToggleSidebar && (
          <button
            ...
            style={{ color: sidebarOpen ? '#34d399' : 'rgba(255,255,255,0.6)', borderRight: '1px solid rgba(255,255,255,0.1)' }}
        ...
        <Link to="/" className="flex items-center gap-2 px-4 h-full shrink-0" style={{ borderRight: '1px solid rgba(255,255,255,0.04)' }}>
          <CamoraLogo size={24} />
          <span className="hidden lg:block text-sm font-extrabold tracking-tight" style={{ fontFamily: "'Satoshi', sans-serif", color: '#fff' }}>Camora</span>
        </Link>
          ...
          {TABS.map((tab) => (
            <button key={tab.id} onClick={() => handleTabClick(tab.id)}
              ...
              style={currentTab === tab.id
                ? { background: 'rgba(52,211,153,0.15)', color: '#34d399', fontFamily: "'Satoshi', sans-serif", boxShadow: 'inset 0 0 0 1px rgba(52,211,153,0.25)' }
        ...
        <div className="flex items-center gap-0.5 px-1.5 h-full shrink-0" style={{ borderLeft: '1px solid rgba(255,255,255,0.04)' }}>
          <button onClick={() => setUseSearch(!useSearch)} className="p-1.5 rounded-md transition-all"
            style={useSearch ? { background: 'rgba(52,211,153,0.15)', color: '#34d399' } : { color: 'rgba(255,255,255,0.3)' }} title="Search (⌘S)">
            <SearchIcon />
          </button>
      ...
      {showInputBar && <div data-tour="input" className="flex items-center h-[36px]" style={{ borderTop: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.04)' }}>
        <div className="flex-1 flex items-center justify-center px-3 min-w-0">
          ...
          <button
            ...
            style={isExpanded ? { color: '#34d399', background: 'rgba(52,211,153,0.15)' } : { color: 'rgba(255,255,255,0.5)' }}
          ...
          {inputValue && (
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #76B900, #76B900)', boxShadow: '0 2px 12px rgba(11,92,255,0.3)' }}
            >
    ...
    <div className="relative">
      ...
      >
        {user.image ? (
          ...
          <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-[9px] font-bold" style={{ background: 'linear-gradient(135deg, #76B900, #76B900)' }}>
            {initials}
          </div>
        )}
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-48 rounded-2xl shadow-xl border z-50 py-1 overflow-hidden" style={{ background: 'linear-gradient(180deg, #76B900 0%, #111111 100%)', borderColor: 'rgba(255,255,255,0.12)' }}>
