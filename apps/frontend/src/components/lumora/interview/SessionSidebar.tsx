    ...
    <>
      ...
      <aside
        className={`fixed lg:relative top-0 left-0 h-full z-50 lg:z-auto flex flex-col transition-all duration-300 ease-out ${
          isOpen ? 'translate-x-0 w-[min(280px,80vw)]' : '-translate-x-full lg:translate-x-0 w-0 lg:w-0'
        }`}
        style={{
      ...
      >
        {isOpen && (
          <div className="flex flex-col h-full w-[min(280px,80vw)] overflow-hidden">
            {/* Header */}
              ...
              <div className="flex items-center gap-1">
                {history.length > 0 && (
                  <button
                    ...
                    className="text-[10px] font-mono text-white/25 hover:text-red-400 px-2 py-1 rounded transition-colors"
                    title="Clear all"
                  >
                    Clear
                  </button>
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden py-2">
              {groups.length === 0 ? (
                <div className="px-4 py-8 text-center">
                  <div className="w-10 h-10 mx-auto mb-3 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)' }}>
                  ...
                  <p className="text-xs text-white/20 font-mono">No questions yet</p>
                  <p className="text-[10px] text-white/10 mt-1">Ask a question to start</p>
                </div>
              ) : (
                groups.map(group => (
                  <div key={group.label} className="mb-3">
                    <div className="px-4 py-1.5">
                      <span className="text-[10px] font-mono font-bold text-white/20 uppercase tracking-widest">{group.label}</span>
                    </div>
                      ...
                      >
                        ...
                        {hoveredIdx === item.index && (
                          <button
                            ...
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded text-white/15 hover:text-red-400 hover:bg-red-400/10 transition-colors"
                            title="Remove"
                          >
