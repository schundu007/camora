...
function StreamingDesignView({ blocks }: { blocks: Record<string, ParsedBlock> }) {
  return (
    <div className="flex flex-col gap-2">
      {/* Approach Headline */}
      <div className="flex items-center gap-3 px-3 py-2 rounded-lg border border-[var(--accent)]/20 bg-[rgba(6,78,59,0.04)] shrink-0">
...
function ScaleMathList({ content, isComplete }: { content: string; isComplete: boolean }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {metrics.map((m, i) => (
          <div key={i} className="rounded-lg border border-[rgba(6,78,59,0.15)] bg-[rgba(6,78,59,0.04)] p-3">
