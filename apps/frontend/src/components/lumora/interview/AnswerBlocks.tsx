    ...
    case 'HEADLINE':
      return (
        <div className="relative rounded-xl overflow-hidden animate-fade-up" style={{...style, background: '#F8FAFC', border: '1px solid #E2E8F0', backdropFilter: 'blur(8px)'}}>
          <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: 'linear-gradient(90deg, var(--cam-primary), var(--cam-primary), #F49AA3)' }} />
          <div className="p-5">
            ...
            {lines.map((line, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-3">
                  <span className="flex items-center justify-center w-5 h-5 rounded-md bg-[rgba(230,57,70,0.15)] text-[#F49AA3] text-[10px] font-bold shrink-0 mt-0.5 font-mono">{i + 1}</span>
        ...
        <div className="rounded-xl overflow-hidden animate-fade-up group" style={{...style, background: '#F8FAFC', border: '1px solid #E2E8F0'}}>
          <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[10px] font-bold text-[#F49AA3]/80 uppercase tracking-widest">{lang}</span>
            </div>
...
function ScaleMathList({ content }: { content: string }) {
  ...
  const renderTable = (items: typeof metrics) => (
    <table className="w-full text-left font-mono" style={{ borderCollapse: 'collapse' }}>
      <thead>
        <tr style={{ borderBottom: '1px solid rgba(230,57,70,0.15)' }}>
    ...
    <div className="space-y-4">
      {pairs.map((pair, i) => (
        <div key={i}>
          ...
          <div className="ml-9 border-l-2 border-[rgba(230,57,70,0.3)] pl-4 py-1">
