      ...
      {showEmptyState ? (
        ...
        <div className="flex-1 flex flex-col gap-1 min-h-0 overflow-auto w-full mx-auto px-2 sm:px-3 py-2" style={{ maxWidth: 'min(700px, 100%)' }}>
          ...
          {isStreaming && question && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg shrink-0" style={{ background: 'rgba(6,78,59,0.03)', border: '1px solid rgba(6,78,59,0.13)' }}>
              <div className="relative flex items-center justify-center w-6 h-6 shrink-0">
                <span className="flex items-center justify-center w-6 h-6 rounded text-[10px] font-bold" style={{ background: 'rgba(6,78,59,0.08)', color: 'var(--cam-primary)', fontFamily: 'var(--font-code)' }}>
                  •
                </span>
    ...
    <div className="flex-1 overflow-auto px-4 sm:px-6 md:px-8 py-4 md:py-6">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {COPILOTS.map(cp => (
            <button key={cp.name} onClick={cp.onClick} className="group text-left p-5 rounded-xl transition-all hover:shadow-md hover:-translate-y-0.5" style={{ background: 'rgba(6,78,59,0.04)', border: '1px solid rgba(6,78,59,0.15)' }}>
              <div className="flex items-center gap-2.5 mb-2">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(6,78,59,0.1)' }}>{cp.icon}</div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
          {QUICK_PROMPTS.map(p => (
            <button key={p.text} onClick={() => handlePromptClick(p)} className="text-left px-4 py-3 rounded-xl text-[12px] leading-snug transition-all" style={{ border: '1px solid rgba(6,78,59,0.2)', color: '#475569' }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(6,78,59,0.5)'; e.currentTarget.style.background = 'rgba(6,78,59,0.04)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(6,78,59,0.2)'; e.currentTarget.style.background = 'transparent'; }}>
              {p.text}
            </button>
          ))}
        </div>
