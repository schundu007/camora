      ...
      {showEmptyState ? (
        ...
        <div className="flex-1 flex flex-col gap-1 min-h-0 overflow-auto w-full mx-auto px-2 sm:px-3 py-2" style={{ maxWidth: 'min(700px, 100%)' }}>
            ...
            <div key={idx} className="shrink-0">
              ...
              >
                <span className="flex items-center justify-center w-6 h-6 rounded text-[10px] font-bold shrink-0"
                  style={{ background: '#F9731610', color: '#F97316', fontFamily: 'var(--font-code)' }}>
                  {idx + 1}
                </span>
                <span className="text-[13px] font-medium leading-snug flex-1 truncate" style={{ fontFamily: 'var(--font-sans)', color: '#1e293b' }}>
                  {entry.question}
                </span>
                <svg className="w-3.5 h-3.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#F97316' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          {isStreaming && question && (
            <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg shrink-0" style={{ background: '#F9731608', border: '1px solid #F9731620' }}>
              <div className="relative flex items-center justify-center w-6 h-6 shrink-0">
                <span className="flex items-center justify-center w-6 h-6 rounded text-[10px] font-bold" style={{ background: '#F9731615', color: '#F97316', fontFamily: 'var(--font-code)' }}>
                  {history.length + 1}
                </span>
              ...
              <span className="text-[9px] shrink-0 animate-pulse font-medium" style={{ fontFamily: 'var(--font-code)', color: '#F97316' }}>generating...</span>
            </div>
          )}
          ...
          {history.length > 0 && history.length % 3 === 0 && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
              ...
              <Link to="/capra/prepare" className="text-xs font-bold hover:opacity-90 transition-all" style={{ color: '#F97316' }}>
                Explore 300+ topics →
              </Link>
            </div>
          )}
