...
const C = {
  ...
  text: '#0f172a', muted: '#64748b', accent: '#0B5CFF',
...
export function LumoraCalendar({ onClose }: { onClose: () => void }) {
  ...
  const calendarSrc = email
    ? `https://calendar.google.com/calendar/embed?src=${encodeURIComponent(email)}&mode=${view === 'week' ? 'WEEK' : 'MONTH'}&showTitle=0&showNav=1&showPrint=0&showTabs=0&showCalendars=0&bgcolor=%23ffffff&color=%230B5CFF`
    : '';
    ...
    <div className="h-full flex flex-col" style={{ background: C.base }}>
      ...
      <div className="flex items-center justify-between px-4 h-12 shrink-0" style={{ borderBottom: `1px solid ${C.border}` }}>
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold" style={{ color: C.text, fontFamily: 'var(--font-sans)' }}>Interview Calendar</span>
          {/* View toggle */}
          <div className="flex items-center gap-0.5 p-0.5 rounded-lg" style={{ background: C.elevated }}>
            <button onClick={() => setView('week')}
              className="px-2 py-1 text-[10px] font-bold rounded transition-all"
              style={view === 'week' ? { background: C.accentBg, color: C.accent } : { color: C.muted }}>
              Week
            </button>
            <button onClick={() => setView('month')}
              className="px-2 py-1 text-[10px] font-bold rounded transition-all"
              style={view === 'month' ? { background: C.accentBg, color: C.accent } : { color: C.muted }}>
              Month
            </button>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <a href="https://calendar.google.com" target="_blank" rel="noopener noreferrer"
            className="text-[11px] font-medium transition-colors" style={{ color: C.accent }}>
      ...
      {calendarSrc ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6">
          ...
          <a href="https://calendar.google.com" target="_blank" rel="noopener noreferrer"
            className="px-4 py-2 rounded-xl text-xs font-bold transition-all" style={{ background: '#0B5CFF', color: '#fff' }}>
