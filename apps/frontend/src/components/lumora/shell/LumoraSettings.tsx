                ...
                {voiceMode === 'filter-candidate' ? (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#2D8CFF' }}>Step 1</span>
...
function SettingCard({ icon, title, description, children }: {
  ...
  return (
    <div className="p-4 rounded-xl transition-all hover:shadow-sm" style={{ background: '#ffffff', border: '1px solid #e2e8f0' }}>
      <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ background: '#f1f5f9', color: '#2D8CFF' }}>
        {icon}
      </div>
      <h4 className="text-sm font-bold mb-1" style={{ color: '#2D8CFF', fontFamily: "'Satoshi', sans-serif" }}>{title}</h4>
    ...
    >
      {badge && (
        <span className="absolute top-3 right-3 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full"
          style={{ background: disabled ? '#00000010' : '#2D8CFF15', color: disabled ? '#000000' : '#2D8CFF' }}>
          {badge}
        </span>
      )}
      {/* Radio indicator */}
      <div className="flex items-start gap-3">
        <div className="mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0"
          style={{ borderColor: active ? '#2D8CFF' : '#cbd5e1' }}>
          {active && <div className="w-2 h-2 rounded-full" style={{ background: '#2D8CFF' }} />}
        </div>
        <div>
          <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ background: active ? '#2D8CFF15' : '#f1f5f9', color: active ? '#2D8CFF' : '#94a3b8' }}>
            {icon}
          </div>
