    ...
    <div className="flex flex-col gap-2">
      {sections.map((s) => (
        <div key={s.label} className="rounded-lg overflow-hidden"
          ...
          <div className="flex items-center justify-between px-3 py-1.5" style={{ background: 'rgba(236,72,153,0.06)', borderBottom: '1px solid rgba(236,72,153,0.1)' }}>
        ...
        {sorted.map(s => {
          ...
          return (
            <div key={s.id} className="px-2 py-1.5 rounded-md transition-all"
              style={{
                background: matches ? 'rgba(236,72,153,0.1)' : '#FFFFFF',
                ...
                <div className="flex flex-wrap gap-0.5 shrink-0 pt-0.5">
                  {s.archetypes.slice(0, 2).map(t => (
                    <span key={t} className="text-[7px] font-bold uppercase tracking-wider px-1 py-0.5 rounded"
                      style={{
                        background: (matches && t === activeArchetype) ? 'var(--cam-primary)' : 'rgba(236,72,153,0.15)',
