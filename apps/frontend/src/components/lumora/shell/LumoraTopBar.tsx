...
export function LumoraTopBar({ activeTab, onTranscription, onCapturedProblem, inline = false }: LumoraTopBarProps) {
  ...
  return (
    <header className="flex items-center h-14 px-3 sm:px-4 md:px-6 shrink-0 z-30" style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #F8FAFC 100%)', borderBottom: '3px solid var(--cam-gold-leaf)', boxShadow: '0 4px 18px rgba(5,28,64,0.12)' }}>
      ...
      <div data-tour="audio" className="flex-1 flex items-center justify-center overflow-x-auto">
        <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex items-center gap-2 min-w-[120px] justify-end">
        ...
        <div className="flex items-center gap-2 px-2 lg:px-3 py-1.5 rounded-lg" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }} title={status.message}>
          <div className={`w-2 h-2 rounded-full ${
            status.state === 'ready' ? 'bg-[var(--cam-primary)]' :
            status.state === 'error' ? 'bg-red-400' :
            status.state === 'warn' ? 'bg-amber-400' :
            (status.state === 'listen' || status.state === 'write') ? 'bg-[var(--cam-primary)] animate-pulse' :
            ''
          }`} />
          <span className="hidden lg:inline text-xs font-bold" style={{ fontFamily: "'Inter', sans-serif", color: 'var(--text-secondary)' }}>{status.message}</span>
        </div>

        <ThemeToggleButton />

        {/* Settings gear */}
        <button onClick={() => setShowSettings(true)} className="p-2 rounded-lg transition-colors hover:bg-[var(--bg-elevated)]" style={{ color: 'var(--text-secondary)' }} title="Settings">
