...
export function LumoraTopBar({ activeTab, onTranscription, onCapturedProblem, inline = false }: LumoraTopBarProps) {
  ...
  return (
    <header className="flex items-center h-14 px-3 sm:px-4 md:px-6 shrink-0 z-30" style={{ background: 'linear-gradient(180deg, var(--cam-primary-dk) 0%, var(--cam-primary) 100%)', borderBottom: '2px solid var(--cam-gold-leaf)' }}>
      ...
      <div data-tour="audio" className="flex-1 flex items-center justify-center overflow-x-auto">
        <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.18)' }}>
      ...
      <div className="flex items-center gap-2 min-w-[120px] justify-end">
        ...
        <div className="flex items-center gap-2 px-2 lg:px-3 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.18)' }} title={status.message}>
          ...
          <span className="hidden lg:inline text-xs font-bold" style={{ fontFamily: "'Inter', sans-serif", color: 'rgba(255,255,255,0.85)' }}>{status.message}</span>
        </div>

        <ThemeToggleButton />

        {/* Settings gear */}
        <button onClick={() => setShowSettings(true)} className="p-2 rounded-lg transition-colors hover:bg-white/10" style={{ color: 'rgba(255,255,255,0.85)' }} title="Settings">
