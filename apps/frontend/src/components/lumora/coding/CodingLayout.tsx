...
function useTheme(dark: boolean) {
  if (dark) {
    return {
      ...
      headerBg: 'rgba(59,54,220,0.08)', headerBorder: '#1E40AF',
      headerText: 'var(--cam-primary)', badgeBg: 'rgba(59,54,220,0.12)', badgeText: 'var(--cam-primary)',
  ...
  return {
    ...
    headerBg: 'rgba(59,54,220,0.05)', headerBorder: '#BFDBFE',
    headerText: 'var(--cam-primary)', badgeBg: 'rgba(59,54,220,0.06)', badgeText: 'var(--cam-primary)',
    ...
    <div className={embedded ? 'flex-1 flex flex-col min-h-0' : 'h-screen w-full flex flex-col lumora-app-bg'}>
        ...
        <div className="flex items-center gap-1.5 md:gap-2">
          ...
          {timerDuration > 0 ? (
            <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border text-xs font-mono font-bold transition-colors ${
              ...
              'bg-[rgba(59,54,220,0.15)] border-[rgba(59,54,220,0.3)] text-[#9F9CEC]'
            }`}>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                        ...
                        <div className="space-y-2">
                          ...
                          {imageFile && (
                            <button onClick={handleExtractFromImage} disabled={isProcessing}
                              className="w-full py-2 text-xs font-medium rounded-lg border border-[var(--border)] hover:bg-[rgba(59,54,220,0.04)] disabled:opacity-50 transition-all"
              ...
              <div className="p-2 md:p-3">
                ...
                {(isStreaming || (isLoading && !sd && !parsedBlocks?.length)) && !sd && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(59,54,220,0.08)', border: `1px solid ${t.cardBorder}` }}>
                        ...
                        <div className="rounded-xl overflow-hidden" style={{ background: t.cardBg, border: `1px solid ${t.cardBorder}` }}>
                          ...
                          <div className="p-3 space-y-2">
                            ...
                            {activeSol.narration && (() => {
                              ...
                              const narrLabel = dark ? '#9F9CEC' : 'var(--cam-primary-dk)';
        ...
        <div onMouseDown={() => setIsResizingH(true)}
          className="hidden md:flex w-1.5 bg-[var(--bg-elevated)] hover:bg-[rgba(59,54,220,0.1)] cursor-col-resize transition-colors items-center justify-center group shrink-0">
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ background: t.surfaceBg, color: t.text }}>
            ...
            <div className="flex items-center gap-2">
              ...
              {isTranslating && (
                <span className="flex items-center gap-1 text-[10px] font-semibold" style={{ color: 'var(--cam-primary)' }}>
                  <div className="w-3 h-3 border-2 border-[rgba(59,54,220,0.3)] border-t-[var(--cam-primary)] rounded-full animate-spin" />
                  Translating…
                </span>
              )}
          ...
          {!isOutputCollapsed && (
            <div onMouseDown={() => setIsResizingV(true)}
              className="h-1.5 hover:bg-[rgba(59,54,220,0.1)] cursor-row-resize transition-colors flex justify-center items-center group"
              style={{ background: t.sectionBg }}>
...
function LegacySolutionCards({ blocks, collapsedCards, onToggle, onTestCaseClick }: {
  ...
  const colorMap: Record<string, { header: string; border: string; bg: string; text: string }> = {
    accent: { header: 'bg-[rgba(59,54,220,0.04)]', border: 'border-[rgba(59,54,220,0.15)]', bg: 'bg-white', text: 'text-[var(--accent)]' },
                      ...
                      if (arrowMatch) {
                        return (
                          <button key={i} onClick={() => onTestCaseClick(arrowMatch[1].trim(), arrowMatch[2].trim())}
                            className="w-full text-left px-2 py-1 bg-[rgba(59,54,220,0.04)] border border-[rgba(59,54,220,0.1)] rounded-md hover:border-[var(--accent)] text-[10px] text-gray-600 font-mono hover:text-[var(--accent)] transition-colors">
                            {line}
                          </button>
                        );
                      }
