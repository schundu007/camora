...
/* ── ScreenCaptureButton ────────────────────────────────────────
   "Capture problem" button. Click → window picker opens INSTANTLY
   (no thumbnails, just titles) → click a window → captured at full
   native resolution → OCR → problem text → onCaptured.
   ...
   Two paths:
     ...
     • Browser: navigator.mediaDevices.getDisplayMedia (browser-native
       picker; we have no list-of-windows API). */
...
export default function ScreenCaptureButton({ kind = 'coding', onCaptured, variant = 'icon', className, label }: Props) {
  ...
  // Click handler — open the picker (desktop) or call getDisplayMedia (browser).
