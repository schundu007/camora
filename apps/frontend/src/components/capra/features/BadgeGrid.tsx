...
function BadgeIcon({ name, earned }: { name: string; earned: boolean }) {
  ...
  return (
    <div
      ...
      style={{ background: earned ? 'rgba(34,211,238,0.08)' : 'var(--bg-elevated)' }}
    >
      {renderer(color)}
    </div>
  );
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {badges.map((badge) => (
          <div
            ...
            style={badge.earned
              ? { background: 'var(--bg-surface)', borderColor: 'rgba(34,211,238,0.35)', boxShadow: '0 2px 8px rgba(34,211,238,0.1)' }
