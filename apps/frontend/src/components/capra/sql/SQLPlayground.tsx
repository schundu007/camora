...
export function SQLPlayground({ onClose }: SQLPlaygroundProps) {
  ...
  // ── Render ──────────────────────────────────────────────────────────────
  return (
    <div className="flex h-[calc(100vh-180px)] min-h-[520px] rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
      {/* ── Left Sidebar ─────────────────────────────────────────────── */}
      <div
        className={`flex-shrink-0 border-r border-slate-200 bg-slate-50/80 flex flex-col transition-all duration-200 ${sidebarCollapsed ? 'w-12' : 'w-[260px]'}`}
      >
        {/* Sidebar header */}
              ...
              <div key={cat.id} className="mb-2">
                ...
                {cat.problems.map((p) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 flex items-center gap-2 text-[13px] transition-colors ${
                      p.id === selectedProblemId
                        ? 'bg-teal-50 text-teal-800 border-r-2 border-teal-500'
                  ...
                  >
                    ...
                    <span className="flex-shrink-0 w-4">
                      {solved.has(p.id) ? (
                        ...
                        <span className="block w-3.5 h-3.5 rounded-full border-2 border-slate-300" />
                      )}
                    </span>
                    <span className="flex-1 truncate" style={{ fontFamily: "'Satoshi', sans-serif" }}>
                      {p.id}. {p.title}
                    </span>
          ...
          <div className="px-3 py-2 border-t border-slate-200">
            <div className="flex items-center justify-between text-[11px] text-slate-500 mb-1">
              ...
              <span className="font-bold text-slate-700">
                {solved.size}/{SQL_PROBLEMS.length}
              </span>
            </div>
            <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
              <div
                className="h-full bg-teal-500 rounded-full transition-all duration-300"
                style={{ width: `${(solved.size / SQL_PROBLEMS.length) * 100}%` }}
              />
            </div>
          </div>
        )}
