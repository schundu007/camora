        ...
        {displayPlans.map(plan => {
          ...
          return (
            <div
              ...
              style={{
                background: (isPro || isBest) ? '#D4EEF8' : 'var(--bg-surface)',
                border: (isPro || isBest) ? '2px solid #B0DCF0' : '1px solid var(--border)',
              }}
            >
              <div className="p-3 flex flex-col flex-1">
                {/* Badge */}
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-[10px] font-bold uppercase tracking-wider" style={{ color: (isPro || isBest) ? '#1B6B8A' : 'var(--text-muted)' }}>{plan.name}</h3>
                  {isPro && <span className="px-1.5 py-0.5 rounded-full text-[7px] font-bold text-white" style={{ background: '#1B6B8A' }}>Popular</span>}
                  {isBest && <span className="px-1.5 py-0.5 rounded-full text-[7px] font-bold text-white" style={{ background: '#1B6B8A' }}>Best</span>}
                </div>

                {/* Price */}
                <div className="flex items-baseline gap-0.5 mb-1">
                  <span className="text-xl font-extrabold" style={{ color: (isPro || isBest) ? '#0E7490' : 'var(--text-primary)' }}>{plan.price}</span>
                  {plan.period && <span className="text-[10px]" style={{ color: (isPro || isBest) ? '#1B6B8A' : 'var(--text-muted)' }}>{plan.period}</span>}
                </div>
                {(plan as any).subtitle && <p className="text-[9px] mb-1" style={{ color: (isPro || isBest) ? '#1B6B8A' : 'var(--text-muted)' }}>{(plan as any).subtitle}</p>}

                {/* Features */}
                <ul className="space-y-1 flex-1 mb-3">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-1 text-[10px] leading-tight">
                      <svg className="w-3 h-3 mt-px shrink-0" viewBox="0 0 16 16" fill="none" stroke={(isPro || isBest) ? '#0E7490' : 'var(--accent)'} strokeWidth="2.5"><path d="M13 4L6 11L3 8" strokeLinecap="round" strokeLinejoin="round" /></svg>
                      <span style={{ color: (isPro || isBest) ? '#0E7490' : 'var(--text-secondary)' }}>{f}</span>
                    </li>
                  ))}
                </ul>
                ...
                {(plan as any).upgrade_note && (
                  <p className="text-[8px] mb-2" style={{ color: (isPro || isBest) ? '#0E7490' : 'var(--warning)' }}>{(plan as any).upgrade_note}</p>
                )}
              </div>
